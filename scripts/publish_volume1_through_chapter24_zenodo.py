#!/usr/bin/env python3
"""Publish the admitted O007 305/672 checkpoint in its Zenodo concept.

Only one new version of concept DOI 10.5281/zenodo.22059798 is permitted, and
only from the exact public v0.15 record 10.5281/zenodo.22097858.  The local
Chapter-24 admission/package gate runs before token access or any request.  The
...
import publish_volume1_through_chapter24_github as gate
...
PREDECESSOR_RECORD_ID = 22_097_858
PREDECESSOR_DOI = "10.5281/zenodo.22097858"
PREDECESSOR_VERSION = "0.15.0-v2-through-ch23"
...
PREDECESSOR_PUBLIC_ASSET_ORDER = (
    "00_READ_FIRST_FONDASI_TEORI_UKURAN_V1_DAN_V2_HINGGA_BAB_23.pdf",
    "fondasi-teori-ukuran-v1-dan-v2-hingga-bab23-id-v0.15.0.zip",
    "SHA256SUMS-v0.15.0-v2-through-ch23.txt",
)
...
PREDECESSOR_RECEIPT_RELATIVE = "qa/ZENODO_PUBLICATION_RECEIPT_V0150_V2_THROUGH_CH23.json"
PREDECESSOR_RECEIPT_PATH = ROOT / PREDECESSOR_RECEIPT_RELATIVE
PREDECESSOR_RECEIPT_BYTES = 3_965
PREDECESSOR_RECEIPT_SHA256 = "93339b5ac1fde486151c0455a7cb674069ba48cabd133814ad3b6ed8336eb741"
RECEIPT_RELATIVE = "qa/ZENODO_PUBLICATION_RECEIPT_V0160_V2_THROUGH_CH24.json"
...
def validate_static_contract() -> None:
    ...
    require(gate.VERSION == VERSION, "Chapter-24 gate version differs")
    require(gate.TAG == TAG, "Chapter-24 gate tag differs")
    require(gate.MODEL == MODEL, "Chapter-24 gate model provenance differs")
    require(gate.expected_coverage() == EXPECTED_COVERAGE,
            "Chapter-24 gate coverage differs from 305/672")
...
def asset_bindings(contract: gate.ReleaseContract) -> dict[str, gate.AssetBinding]:
    ...
    for name, binding in bindings.items():
        ...
        require(
            ...
            f"Chapter-24 package-bound asset identity differs: {name}",
        )
    return bindings
...
def metadata() -> dict[str, object]:
    description = (
        "<p><strong>Prarilis parsial: 305 dari 672 halaman resmi korpus "
        ...
        "dan Bab 21–24 secara lengkap.</p>"
        "<p>Cakupan Jilid 2 pada checkpoint ini adalah halaman resmi 1–203; "
        "Bab 24 sendiri mencakup halaman 138–203. Bab 25 dan bagian sesudahnya "
        "belum termasuk; karena itu korpus dua "
    ...
    return {
        ...
        "notes": (
            "Prarilis parsial 305/672 halaman resmi: Jilid 1 lengkap dan "
            "Jilid 2 dari bagian pendahuluan sampai Bab 24 lengkap, mencakup "
            "halaman resmi Jilid 2 nomor 1–203. Bab 25 dan sesudahnya belum "
...
def predecessor_receipt() -> dict[str, Any]:
    require(
        ...
        "immutable Zenodo v0.15 predecessor receipt identity differs",
    )
    try:
        value = json.loads(PREDECESSOR_RECEIPT_PATH.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise PublicationError("Zenodo v0.15 predecessor receipt is unreadable") from exc
    require(isinstance(value, dict), "Zenodo v0.15 predecessor receipt is malformed")
    ...
    require(
        ...
        "Zenodo v0.15 predecessor receipt semantics differ",
    )
    ...
    require(
        ...
        and scope.get("official_pages_complete") == 239
        and scope.get("selected_corpus_pages") == 672
        and scope.get("volume2_first_included_page") == 1
        and scope.get("volume2_last_included_page") == 137
        and scope.get("volume2_included_pages") == 137
        and scope.get("volume2_chapter23_complete") is True
        and scope.get("selected_corpus_complete") is False,
        "Zenodo v0.15 predecessor coverage differs",
    )
    return value
...
def choose_candidate(token: str) -> tuple[dict[str, Any], str, bool]:
    ...
    require(len(exact_public) <= 1, "multiple exact public through-Chapter-24 versions exist")
    if exact_public:
        require(not drafts, "exact public through-Chapter-24 version coexists with a concept draft")
        return exact_public[0], "already_published_exact_through_chapter24", True
    ...
    return draft, "one_newversion_from_22097858", False
...
def write_receipt(
    ...
    value = {
        ...
        "record": {
            ...
            "partial_coverage": "305/672 official pages; Volume II pages 1-203 included",
        },
        "asset_order": [binding.name for binding in contract.assets],
        "assets": assets,
        "transaction_route": "existing_concept_newversion_from_22097858",
        "runtime_route": route,
        "verification": {
    ...
    if RECEIPT_PATH.exists():
        try:
            ...
            raise PublicationError("existing through-Chapter-24 Zenodo receipt is unreadable") from exc
        ...
        require(old == current, "existing through-Chapter-24 Zenodo receipt differs")
...
if __name__ == "__main__":
    try:
        ...
        print(f"ERROR: fail-closed through-Chapter-24 Zenodo publication: {exc}", file=sys.stderr)
        raise SystemExit(1)
