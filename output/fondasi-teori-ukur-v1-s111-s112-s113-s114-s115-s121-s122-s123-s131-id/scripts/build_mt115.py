#!/usr/bin/env python3
"""Deterministically build and package cumulative O007 sections 111-115.

The two passes rebuild the same bounded staging and distribution paths.  The
builder preserves every published S111-through-S114 release artifact, keeps the
...
def deterministic_zip(package: Path, destination: Path) -> None:
    """Write an exact S115 ZIP with the S115 source-date timestamp."""
...
OPTIONAL_QA_INPUTS = {"mt115-visual-browser-qa.json"}
...
def verify_mt115_inputs(lane: Path) -> None:
...
def exact_prior_html(lane: Path, number: str, candidate: Path) -> None:
    ...
    if candidate.read_bytes() != prior.read_bytes():
        raise RuntimeError(
            f"regenerated S{number} HTML differs from the admitted S114 reader"
        )
...
def copy_pdf_inputs(lane: Path, stage: Path) -> None:
    ...
    for name in ("sections111-115-id.tex", "mt113-dvipdfmx-images.tex"):
...
def build_once(
    ...
    reset_directory(lane, stage, "fremlin-v1-s111-s112-s113-s114-s115-id")
    ...
    master = lane / "reader" / "pdf" / "sections111-115-id.tex"
    support = lane / "reader" / "pdf" / "mt113-dvipdfmx-images.tex"
    root_html_source = lane / "reader" / "html" / "index-111-115-id.html"
...
def main() -> int:
    ...
    required_files = [
        ...
        *(lane / "reader" / "static" / name for name in ("reader.css", "reader-v2.css", "reader-v3.css")),
        ...
        lane / "qa" / "mt115-backend-validation.json",
        lane / "qa" / "mt115-semantic-review.json",
        lane / "qa" / "mt115-structural-qa.json",
    ...
    stage = lane / "build" / "fremlin-v1-s111-s112-s113-s114-s115-id"
    ...
    if preserved_before != preserved_after:
        raise RuntimeError("a pre-existing S111-through-S114 release artifact changed")

    qa_dir = lane / "qa"
    shutil.copyfile(package / "BUILD_METADATA.json", qa_dir / "mt115-build-metadata.json")
    shutil.copyfile(package / "PACKAGE_MANIFEST.tsv", qa_dir / "mt115-PACKAGE_MANIFEST.tsv")
    ...
    (qa_dir / "mt115-SHA256SUMS.txt").write_text(
    ...
    write_json(qa_dir / "mt115-build-receipt.json", receipt)
