#!/usr/bin/env python3
"""Build the deterministic reader-first O007 338/672 checkpoint package.

This driver packages complete Volume I and the contiguous Volume II surface
through Section 252 (official Volume II pages 1--236).  Chapter 25 is partial.
It is deliberately
...
import package_volume1_through_chapter24_release as predecessor
...
PDF_RELATIVE = "output/pdf/fondasi-teori-ukuran-jilid-1-dan-jilid-2-hingga-bagian-252-id.pdf"
PDF_SOURCE = ROOT / PDF_RELATIVE
HTML_RELATIVE = "output/fondasi-teori-ukuran-v1-through-s252-id/html"
HTML_ROOT = ROOT / HTML_RELATIVE
PDF_PUBLIC_NAME = "00_READ_FIRST_FONDASI_TEORI_UKURAN_V1_DAN_V2_HINGGA_BAGIAN_252.pdf"
ZIP_NAME = "fondasi-teori-ukuran-v1-dan-v2-hingga-s252-id-v0.17.0.zip"
CHECKSUM_NAME = "SHA256SUMS-v0.17.0-v2-through-s252.txt"
PACKAGE_ROOT = "fondasi-teori-ukuran-v1-dan-v2-hingga-s252-id-v0.17.0"
ZIP_TIMESTAMP = (2026, 8, 26, 0, 0, 0)

ADMISSION_RELATIVE = "qa/through-s252-final-admission.json"
ADMISSION_RECORD_RELATIVE = "00_control/CP0017_THROUGH_S252_ADMISSION.md"
AGGREGATE_RELATIVE = "qa/chapter25-through-s252-aggregate-qa.json"
BACKEND_RELATIVE = "backend/through-s252-backend-validation.json"
PDF_BUILD_RELATIVE = "qa/through-s252-complete-build.json"
PDF_VISUAL_RELATIVE = "qa/through-s252-pdf-visual-qa.json"
HTML_BUILD_RELATIVE = "qa/through-s252-html-build.json"
HTML_BROWSER_RELATIVE = "qa/through-s252-html-browser-qa.json"
PACKAGE_RECEIPT_RELATIVE = "qa/through-s252-release-package.json"
PACKAGE_MANIFEST_RELATIVE = "qa/through-s252-PACKAGE_MANIFEST.tsv"
CHECKSUM_RECEIPT_RELATIVE = "qa/through-s252-SHA256SUMS.txt"
PUBLIC_VALIDATION_RELATIVE = "qa/through-s252-public-overlay-validation.json"
PUBLIC_MANIFEST_RECEIPT_RELATIVE = "qa/through-s252-PUBLIC_SOURCE_TREE_MANIFEST.tsv"
PUBLIC_MAP_RECEIPT_RELATIVE = "qa/through-s252-PUBLIC_SANITIZATION_MAP.json"

ADMISSION_SCHEMA = "o007-fremlin-through-s252-final-admission-v1"
PACKAGE_SCHEMA = "o007-through-s252-release-package-v1"
CURRENT_CATALOG = "backend/catalog-v1.12"

PREDECESSOR_GITHUB_RECEIPT = "qa/PUBLICATION_RECEIPT_V0160_V2_THROUGH_CH24.json"
PREDECESSOR_GITHUB_RECEIPT_IDENTITY = (
    4_616,
    "a1ca79817b26cb0df56c8de085de5c8ddf8e5eec97e89567d137a59bb0ae2dbb",
)
PREDECESSOR_GITHUB_TAG = "v0.16.0-v2-through-ch24"
PREDECESSOR_GITHUB_TAG_COMMIT = "0bd08492b9ed5c31c861dc5f6d45abef452bfbda"
PREDECESSOR_GITHUB_RECEIPT_COMMIT = "1cfefad6e12922bf5b95a4a9551485851a2d64db"
PREDECESSOR_GITHUB_MAIN_COMMIT = "7b219c3a9cd4d83c3c93dfcec85138c4ec60678c"
PREDECESSOR_ZENODO_DOI = "10.5281/zenodo.22103648"
PREDECESSOR_ZENODO_RECEIPT = "qa/ZENODO_PUBLICATION_RECEIPT_V0160_V2_THROUGH_CH24.json"
...
THROUGH_S252_UNIT_STEMS = ("mt25", "mt251", "mt252")
...
VOLUME2_SOURCE_PATHS = (
    ...
    "source/id-ID/vol2-through-s252-id.tex",
)
...
SCRIPT_FILES = (
    *predecessor.SCRIPT_FILES,
    "scripts/aggregate_chapter25_through_s252_qa.py",
    "backend/generate_through_s252_checkpoint.py",
    "backend/validate_through_s252_checkpoint.py",
    "scripts/build_volume1_through_s252.py",
    "scripts/render_volume1_through_s252_html.py",
    "scripts/qa_volume1_through_s252_pdf.py",
    "scripts/qa_volume1_through_s252_html.py",
    "scripts/admit_volume1_through_s252.py",
    "scripts/package_volume1_through_s252_release.py",
    "scripts/publish_volume1_through_s252_github.py",
    "scripts/publish_volume1_through_s252_zenodo.py",
)
...
QA_FILES = (
    *predecessor.QA_FILES,
    ...
    "qa/chapter25/mt25-source-freeze.json",
    *(f"qa/chapter25/{name}-unit-qa.json" for name in THROUGH_S252_UNIT_STEMS),
)

BACKEND_DIRS = (
    *predecessor.BACKEND_DIRS,
    CURRENT_CATALOG,
    *(f"backend/{name}" for name in THROUGH_S252_UNIT_STEMS),
)
...
def validate_html_tree(admission: dict[str, Any]) -> dict[str, int | str]:
    ...
    require(html.get("routes") == 62, "admission HTML route count differs")
    return {"files": file_count, "bytes": byte_count, "manifest_sha256": manifest_sha, "routes": 62}
...
def validate_canonical_replay() -> dict[str, object]:
    ...
    require(
        ...
        "bound THROUGH S252 backend count surfaces are absent",
    )
    generated = subprocess.run(
        [sys.executable, "backend/generate_through_s252_checkpoint.py", "--check"],
        cwd=ROOT, capture_output=True, text=True, encoding="utf-8", errors="strict",
        timeout=300, check=False,
    )
    require(generated.returncode == 0, "canonical THROUGH S252 generator replay failed")
    lines = [line for line in generated.stdout.splitlines() if line]
    require(lines, "canonical THROUGH S252 generator emitted no receipt")
    generation = json.loads(lines[-1])
    require(
        generation.get("written") is False and generation.get("admitted") is False
        and generation.get("boundary_label") == "THROUGH S252"
        and generation.get("chapter25_complete") is False
        and generation.get("chapter25_increment_pages") == "204-236"
        and generation.get("chapter25_increment_unique_official_page_count") == 33
        and generation.get("volume2_contiguous_translated_pages") == "1-236"
        and generation.get("volume2_contiguous_translated_page_count") == 236
        and generation.get("cumulative_completed_official_pages") == 338
        and generation.get("cumulative_active_exercises") == 653
        and generation.get("cumulative_explicit_hints") == 149
        and generation.get("selected_corpus_official_pages") == 672
        and generation.get("inherited_admitted_unit_count") == 28
        and generation.get("inherited_admitted_status") == "admitted"
        and generation.get("inherited_target_admitted") is True
        and generation.get("new_pre_admission_unit_count") == 3
        ...
        "canonical THROUGH S252 generator replay semantics differ",
    )
    generated_units = generation.get("units")
    require(
        isinstance(generated_units, dict)
        and set(generated_units) == set(bound_unit_counts) == set(THROUGH_S252_UNIT_STEMS),
        "canonical THROUGH S252 generator unit surface differs",
    )
    ...
    code = (
        ...
        "import validate_through_s252_checkpoint as v;r=v.validate();"
    ...
    require(validated.returncode == 0, "canonical THROUGH S252 validator replay failed")
    validator_lines = [line for line in validated.stdout.splitlines() if line]
    require(validator_lines, "canonical THROUGH S252 validator emitted no receipt")
    ...
    }, "canonical THROUGH S252 validator replay semantics differ")
...
def validate_admission() -> tuple[dict[str, Any], dict[str, object]]:
    ...
    require(
        ...
        "final admission version or 338/672 boundary differs",
    )
    admission_checks = admission.get("checks")
    all_true(admission_checks, "final admission checks")
    require(
        isinstance(admission_checks, dict)
        and admission_checks.get("inherited_volume2_unit_admission_state_preserved") is True
        and admission_checks.get("new_through_s252_backend_units_admitted_by_cp0017") is True,
        "final admission does not bind the inherited/new unit-state transition",
    )
    require(admission.get("blockers") == [], "final admission reports blockers")
    exact_binding(ADMISSION_RECORD_RELATIVE, admission.get("content_admission"), "CP0017 admission")
    require(MODEL.encode("utf-8") in (ROOT / ADMISSION_RECORD_RELATIVE).read_bytes(),
            "CP0017 omits exact model provenance")

    aggregate = admission.get("independent_aggregate_replay")
    exact_binding(AGGREGATE_RELATIVE, aggregate, "THROUGH S252 aggregate")
    assert isinstance(aggregate, dict)
    require(aggregate.get("pass") is True and aggregate.get("blockers") == [],
            "admission-bound THROUGH S252 aggregate does not pass")
    ...
    require(isinstance(pdf_pages, int) and pdf_pages > 327,
            "cumulative PDF did not extend the predecessor reader")
    ...
    require(
        ...
        and isinstance(output_inventory, dict)
        and schema_records == 3_968
        and isinstance(output_inventory.get("file_count"), int)
        and output_inventory["file_count"] == 101
        and isinstance(output_inventory.get("total_bytes"), int)
        and output_inventory["total_bytes"] > 0
        ...
        and backend.get("inherited_volume2_admitted_units") == 28
        and backend.get("new_units_pre_admission") == 3
    ...
    require(
        ...
        and predecessor_boundary.get("version") == "0.16.0-v2-through-ch24"
        ...
        and predecessor_boundary.get("github_receipt_commit") == PREDECESSOR_GITHUB_RECEIPT_COMMIT
...
- Checkpoint scope: complete Volume I and contiguous Volume II front matter through partial Chapter 25 Section 252, official Volume II pages 1--236; 338 of 672 official corpus pages. Chapter 25 is not complete.
...
- Modification date: 26 August 2026.
...
""".encode("utf-8")
    metadata = {
        "schema": "o007-volume1-through-s252-release-metadata-v1",
        "version": VERSION,
        "tag": TAG,
        ...
        "coverage": {
            ...
            "volume2_chapter25": {
                "status": "partial_through_section_252",
                "official_pages": "204-236",
                "unique_pages": 33,
                "chapter_complete": False,
            },
            "remaining_volume2": {"status": "not_included", "first_official_page": 237},
        },
        "reader": {
        ...
        "lineage": {
            ...
            "github_predecessor_receipt_commit": PREDECESSOR_GITHUB_RECEIPT_COMMIT,
            "zenodo_predecessor_doi": PREDECESSOR_ZENODO_DOI,
        },
        "deterministic_zip_timestamp": "2026-08-26T00:00:00Z",
    }
    closure = {
...
def build(write: bool) -> dict[str, object]:
    ...
    require(
        expected_catalog_resources == 223,
        "validated THROUGH S252 catalog resource count differs",
    )
    ...
    with tempfile.TemporaryDirectory(prefix="o007-v017-through-s252-", dir=ROOT / "tmp") as name:
        temp = Path(name)
        ...
        receipt = {
            ...
            "zenodo_predecessor": {
                "receipt": {
                    ...
                    **identity_bytes((ROOT / PREDECESSOR_ZENODO_RECEIPT).read_bytes()),
                },
            ...
            "package_details": {
                ...
                "fixed_timestamp": "2026-08-26T00:00:00Z", "cumulative_pdf_occurrences": 1,
            ...
            "checks": {
                ...
                "volume2_through_s252_source_identities_exact": True,
        ...
        if write:
            ...
            for target, data in targets.items():
                ...
                temporary = target.with_name(target.name + ".tmp-v017-through-s252")
...
if __name__ == "__main__":
    try:
        ...
        print(f"ERROR: fail-closed THROUGH S252 package: {exc}", file=sys.stderr)
        raise SystemExit(1)
