...
TARGET_LABELS = {
    ...
    "112Ba": "Penggunaan tak hingga", "112Bc": "Deret dalam bilangan real nonnegatif diperluas",
...
def read_correction_rows() -> list[dict[str, str]]:
    ...
    if len(all_rows) != EXPECTED_CORRECTIONS_ROWS or [row["correction_id"] for row in all_rows[:3]] != expected:
        raise ValueError("live cumulative correction ledger does not preserve the exact S112 prefix")
    rows = [row for row in all_rows if row["unit_id"] == UNIT_ID]
    if [row["correction_id"] for row in rows] != expected:
        raise ValueError("S112 correction-ledger row sequence differs")
    return rows
...
DEFINITION_SPECS = [
    ...
    ("IMAGE-MEASURE", "112Xf", "image measure", "ukuran citra"),
]
...
TERM_SPECS = [
    ...
    ("EXTENDED-NONNEGATIVE-REALS", "extended nonnegative reals", "bilangan real nonnegatif diperluas", "technical", []),
    ...
    ("IMAGE-MEASURE", "image measure", "ukuran citra", "preferred", ["IMAGE-MEASURE"]),
...
def build_artifacts(source_bytes: bytes, target_bytes: bytes, corrections_bytes: bytes, source: str, target: str) -> list[dict[str, object]]:
    return [
        ...
        },
        {
            ...
            "bytes": len(corrections_bytes), "sha256": sha256_bytes(corrections_bytes), "rows": 19,
            "verification_status": "current cumulative nineteen-row ledger; exact three-row S112 prefix, target locations, and formula hashes verified",
            "rights_id": RIGHTS_ID,
...
def build_catalog(source_bytes: bytes, target_bytes: bytes, corrections_bytes: bytes, target: str) -> dict[str, list[dict[str, object]]]:
    ...
    resources.extend([
        ...
        },
        {
            ...
            "verification_status": "nineteen cumulative rows; exact three-row S112 prefix and two formula exception hashes verified",
