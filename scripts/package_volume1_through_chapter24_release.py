#!/usr/bin/env python3
"""Build the deterministic reader-first O007 305/672 checkpoint package.

This driver packages complete Volume I and the contiguous Volume II surface
through Chapter 24 (official Volume II pages 1--203).  It is deliberately
...
import package_volume1_through_chapter23_release as predecessor
...
PDF_RELATIVE = "output/pdf/fondasi-teori-ukuran-jilid-1-dan-jilid-2-hingga-bab-24-id.pdf"
PDF_SOURCE = ROOT / PDF_RELATIVE
HTML_RELATIVE = "output/fondasi-teori-ukuran-v1-through-chapter24-id/html"
HTML_ROOT = ROOT / HTML_RELATIVE
PDF_PUBLIC_NAME = "00_READ_FIRST_FONDASI_TEORI_UKURAN_V1_DAN_V2_HINGGA_BAB_24.pdf"
ZIP_NAME = "fondasi-teori-ukuran-v1-dan-v2-hingga-bab24-id-v0.16.0.zip"
CHECKSUM_NAME = "SHA256SUMS-v0.16.0-v2-through-ch24.txt"
PACKAGE_ROOT = "fondasi-teori-ukuran-v1-dan-v2-hingga-bab24-id-v0.16.0"
...
AGGREGATE_RELATIVE = "qa/chapter24-aggregate-qa.json"
BACKEND_RELATIVE = "backend/chapter24-backend-validation.json"
PDF_BUILD_RELATIVE = "qa/through-chapter24-complete-build.json"
PDF_VISUAL_RELATIVE = "qa/through-chapter24-pdf-visual-qa.json"
HTML_BUILD_RELATIVE = "qa/through-chapter24-html-build.json"
HTML_BROWSER_RELATIVE = "qa/through-chapter24-html-browser-qa.json"
PACKAGE_RECEIPT_RELATIVE = "qa/through-chapter24-release-package.json"
PACKAGE_MANIFEST_RELATIVE = "qa/through-chapter24-PACKAGE_MANIFEST.tsv"
CHECKSUM_RECEIPT_RELATIVE = "qa/through-chapter24-SHA256SUMS.txt"
PUBLIC_VALIDATION_RELATIVE = "qa/through-chapter24-public-overlay-validation.json"
PUBLIC_MANIFEST_RECEIPT_RELATIVE = "qa/through-chapter24-PUBLIC_SOURCE_TREE_MANIFEST.tsv"
PUBLIC_MAP_RECEIPT_RELATIVE = "qa/through-chapter24-PUBLIC_SANITIZATION_MAP.json"
...
CURRENT_CATALOG = "backend/catalog-v1.11"

PREDECESSOR_GITHUB_RECEIPT = "qa/PUBLICATION_RECEIPT_V0150_V2_THROUGH_CH23.json"
PREDECESSOR_GITHUB_RECEIPT_IDENTITY = (
    4_187,
    "190972813010bb6f82b83ffd01e5175f857af2f211de5ef35a469040191b7354",
)
PREDECESSOR_GITHUB_TAG = "v0.15.0-v2-through-ch23"
PREDECESSOR_GITHUB_TAG_COMMIT = "181bbb7ae28ac4e8850a005dfc428fe42f67a6b8"
PREDECESSOR_GITHUB_RECEIPT_COMMIT = "6dafc1575460f94f06db9b4c939058a7b97dbf7c"
PREDECESSOR_GITHUB_MAIN_COMMIT = "08e872e60e0268f0e2cfd7cbba0c7d8616b2be62"
PREDECESSOR_ZENODO_DOI = "10.5281/zenodo.22097858"
PREDECESSOR_ZENODO_RECEIPT = "qa/ZENODO_PUBLICATION_RECEIPT_V0150_V2_THROUGH_CH23.json"
...
CHAPTER24_UNIT_STEMS = ("mt24", "mt241", "mt242", "mt243", "mt244", "mt245", "mt246", "mt247")
...
VOLUME2_SOURCE_PATHS = (
    ...
    "source/id-ID/vol2-through-ch24-id.tex",
)
...
SCRIPT_FILES = (
    *predecessor.SCRIPT_FILES,
    "backend/generate_through_chapter24_checkpoint.py",
    "backend/validate_through_chapter24_checkpoint.py",
    "scripts/build_volume1_through_chapter24.py",
    "scripts/render_volume1_through_chapter24_html.py",
    "scripts/qa_volume1_through_chapter24_pdf.py",
    "scripts/qa_volume1_through_chapter24_html.py",
    "scripts/admit_volume1_through_chapter23.py",
    "scripts/admit_volume1_through_chapter24.py",
...
QA_FILES = (
    *predecessor.QA_FILES,
    ...
    CORRECTED_CHAPTER21_SEAL_REPLAY,
    *(f"qa/chapter24/{name}-unit-qa.json" for name in CHAPTER24_UNIT_STEMS),
)

BACKEND_DIRS = (
    *predecessor.BACKEND_DIRS,
    CURRENT_CATALOG,
    *(f"backend/{name}" for name in CHAPTER24_UNIT_STEMS),
)
...
def validate_html_tree(admission: dict[str, Any]) -> dict[str, int | str]:
    ...
    require(html.get("routes") == 59, "admission HTML route count differs")
    return {"files": file_count, "bytes": byte_count, "manifest_sha256": manifest_sha, "routes": 59}
...
def validate_canonical_replay() -> dict[str, object]:
    ...
    require(
        ...
        "bound Chapter 24 backend count surfaces are absent",
    )
    generated = subprocess.run(
        [sys.executable, "backend/generate_through_chapter24_checkpoint.py", "--check"],
        cwd=ROOT, capture_output=True, text=True, encoding="utf-8", errors="strict",
        timeout=300, check=False,
    )
    require(generated.returncode == 0, "canonical Chapter 24 generator replay failed")
    lines = [line for line in generated.stdout.splitlines() if line]
    require(lines, "canonical Chapter 24 generator emitted no receipt")
    generation = json.loads(lines[-1])
    require(
        generation.get("written") is False and generation.get("admitted") is False
        and generation.get("chapter24_pages") == "138-203"
        and generation.get("chapter24_unique_official_page_count") == 66
        and generation.get("volume2_contiguous_translated_pages") == "1-203"
        and generation.get("volume2_contiguous_translated_page_count") == 203
        and generation.get("cumulative_completed_official_pages") == 305
        and generation.get("selected_corpus_official_pages") == 672
        and generation.get("catalog") == catalog_counts,
        "canonical Chapter 24 generator replay semantics differ",
    )
    generated_units = generation.get("units")
    require(
        isinstance(generated_units, dict)
        and set(generated_units) == set(bound_unit_counts) == set(CHAPTER24_UNIT_STEMS),
        "canonical Chapter 24 generator unit surface differs",
    )
    ...
    code = (
        ...
        "import validate_through_chapter24_checkpoint as v;r=v.validate();"
    ...
    require(validated.returncode == 0, "canonical Chapter 24 validator replay failed")
    validator_lines = [line for line in validated.stdout.splitlines() if line]
    require(validator_lines, "canonical Chapter 24 validator emitted no receipt")
    ...
    require(validation == {
        ...
        "schema": "o007-through-chapter24-backend-validation-v1",
        "status": "pass",
        "unit_counts": bound_unit_counts,
    }, "canonical Chapter 24 validator replay semantics differ")
...
def validate_admission() -> tuple[dict[str, Any], dict[str, object]]:
    ...
    require(
        ...
        "final admission version or 305/672 boundary differs",
    )
    all_true(admission.get("checks"), "final admission checks")
    require(admission.get("blockers") == [], "final admission reports blockers")
    exact_binding(ADMISSION_RECORD_RELATIVE, admission.get("content_admission"), "CP0016 admission")
    ...
    exact_binding(AGGREGATE_RELATIVE, aggregate, "Chapter 24 aggregate")
    assert isinstance(aggregate, dict)
    require(aggregate.get("pass") is True and aggregate.get("blockers") == [],
            "admission-bound Chapter 24 aggregate does not pass")
    ...
    require(isinstance(pdf_pages, int) and pdf_pages > 258,
    ...
    require(
        ...
        and isinstance(output_inventory, dict)
        and isinstance(schema_records, int) and schema_records > 5_718
        and isinstance(output_inventory.get("file_count"), int)
        and output_inventory["file_count"] > 188
    ...
    require(
        ...
        and predecessor_boundary.get("version") == "0.15.0-v2-through-ch23"
...
""".encode("utf-8")
    metadata = {
        "schema": "o007-volume1-through-chapter24-release-metadata-v1",
        "version": VERSION,
        "tag": TAG,
        ...
        "coverage": {
            ...
            "remaining_volume2": {"status": "not_included", "first_official_page": 204},
        },
        "reader": {
...
def build(write: bool) -> dict[str, object]:
    ...
    require(
        isinstance(expected_catalog_resources, int) and expected_catalog_resources > 184,
        "validated Chapter 24 catalog resource count did not advance",
    )
    ...
    with tempfile.TemporaryDirectory(prefix="o007-v016-through-ch24-", dir=ROOT / "tmp") as name:
        temp = Path(name)
        ...
        receipt = {
            ...
            "checks": {
                ...
                "volume2_through_chapter24_source_identities_exact": True,
        ...
        if write:
            ...
            for target, data in targets.items():
                ...
                temporary = target.with_name(target.name + ".tmp-v016-through-ch24")
...
if __name__ == "__main__":
    try:
        ...
        print(f"ERROR: fail-closed through-Chapter-24 package: {exc}", file=sys.stderr)
        raise SystemExit(1)
