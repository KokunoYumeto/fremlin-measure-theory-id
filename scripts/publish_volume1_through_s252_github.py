#!/usr/bin/env python3
"""Publish the admitted O007 338/672 checkpoint to the existing GitHub line.
...
1--236 boundary, explicitly partial Chapter 25 through S252.  Preflight
performs no Git command, network request, or credential read.  Publication
stages only the explicit package-bound paths.
"""
...
PREDECESSOR_TAG = "v0.16.0-v2-through-ch24"
PREDECESSOR_BOUNDARY_COMMIT = "0bd08492b9ed5c31c861dc5f6d45abef452bfbda"
PREDECESSOR_RECEIPT_COMMIT = "1cfefad6e12922bf5b95a4a9551485851a2d64db"
PREDECESSOR_MAIN_COMMIT = "7b219c3a9cd4d83c3c93dfcec85138c4ec60678c"
PREDECESSOR_RECEIPT_RELATIVE = "qa/PUBLICATION_RECEIPT_V0160_V2_THROUGH_CH24.json"
PREDECESSOR_RECEIPT_PATH = ROOT / PREDECESSOR_RECEIPT_RELATIVE
PREDECESSOR_RECEIPT_BYTES = 4_616
PREDECESSOR_RECEIPT_SHA256 = "a1ca79817b26cb0df56c8de085de5c8ddf8e5eec97e89567d137a59bb0ae2dbb"
...
PACKAGE_RELATIVE = "qa/through-s252-release-package.json"
PACKAGE_PATH = ROOT / PACKAGE_RELATIVE
TREE_RELATIVE = "qa/THROUGH_S252_RELEASE_TREE.tsv"
TREE_PATH = ROOT / TREE_RELATIVE
RECEIPT_RELATIVE = "qa/PUBLICATION_RECEIPT_V0170_V2_THROUGH_S252.json"
RECEIPT_PATH = ROOT / RECEIPT_RELATIVE
PUBLIC_VALIDATION_RELATIVE = "qa/through-s252-public-overlay-validation.json"
PUBLIC_MANIFEST_RELATIVE = "qa/through-s252-PUBLIC_SOURCE_TREE_MANIFEST.tsv"
PUBLIC_MAP_RELATIVE = "qa/through-s252-PUBLIC_SANITIZATION_MAP.json"
...
BACKEND_VALIDATION_RELATIVE = "backend/through-s252-backend-validation.json"
...
REQUIRED_BOUNDARY_EVIDENCE = {
    ...
    PACKAGE_RELATIVE,
    "backend/through-s252-backend-validation.json",
    "qa/chapter25-through-s252-aggregate-qa.json",
    "qa/through-s252-complete-build.json",
    "qa/through-s252-pdf-visual-qa.json",
    "qa/through-s252-html-build.json",
    "qa/through-s252-html-browser-qa.json",
...
def exact_relative_binding(value: object, label: str) -> tuple[str, dict[str, Any]]:
    """Replay one receipt-bound local file without trusting a pass flag."""
...
def validate_admission_file_bindings(value: dict[str, Any]) -> None:
    """Fail closed if any file admitted by CP0017 has changed since admission."""
    ...
    require(receipts["aggregate"].get("path") == aggregate_relative,
            "admission aggregate path differs")
    require(receipts["backend"].get("path") == BACKEND_VALIDATION_RELATIVE,
            "admission backend path differs")

    units = value.get("unit_receipts")
    require(isinstance(units, list) and len(units) == 3,
    ...
    exact_relative_binding(artifacts.get("cumulative_pdf"), "admitted cumulative PDF")
...
def validate_predecessor_receipt() -> dict[str, Any]:
    require(
        ...
        "immutable GitHub v0.16 predecessor receipt identity differs",
    )
    value = load_json(PREDECESSOR_RECEIPT_PATH, "GitHub v0.16 predecessor receipt")
    require(
        value.get("destination") == "github"
        and value.get("version") == "0.16.0-v2-through-ch24"
        ...
        and value.get("scope", {}).get("official_pages_complete") == 305
        and value.get("scope", {}).get("volume2_last_included_page") == 203
        and value.get("scope", {}).get("volume2_chapter24_complete") is True
        and value.get("release", {}).get("prerelease") is True,
        "GitHub v0.16 predecessor receipt semantics differ",
    )
    return value


def validate_admission() -> dict[str, Any]:
    value = load_json(ADMISSION_PATH, "final through-S252 admission receipt")
    ...
    require(
        ...
        "final through-S252 admission has not been issued",
    )
    ...
    require(
        ...
        "admission version or 338/672 boundary differs",
    )
    ...
    require(
        ...
        and publication.get("zenodo_predecessor_doi") == "10.5281/zenodo.22103648"
    ...
    exact_file(ADMISSION_RECORD_PATH, record, "CP0017 admission record")
...
def validate_package(
    ...
    value = load_json(PACKAGE_PATH, "final through-S252 package receipt")
    ...
    require(tuple(binding.name for binding in assets) == EXPECTED_PUBLIC_ASSET_NAMES,
            "package release-asset names differ from the through-S252 v0.17.0 contract")
    ...
    exact_file(backend_path, backend_row, "package-bound through-S252 backend validation")
    backend = load_json(backend_path, "package-bound through-S252 backend validation")
    require(
        backend.get("schema") == "o007-through-s252-backend-validation-v1"
        and backend.get("status") == "pass"
        and isinstance(backend.get("manifests"), dict)
        and "catalog-v1.12" in backend["manifests"],
        "package backend is not the passing catalog-v1.12 checkpoint",
    )
    ...
    content_relative, content = exact_relative_binding(
        value.get("content_admission"), "package-bound CP0017 admission record",
    )
    require(
        ...
        and all(content.get(key) == admission["content_admission"].get(key)
                for key in ("path", "bytes", "sha256")),
        "package and admission CP0017 bindings disagree",
    )
    aggregate_relative, aggregate = exact_relative_binding(
    ...
    exact_relative_binding(
        value.get("public_overlay_validation"),
    ...
    exact_relative_binding(
        source_tree.get("sanitization_map"), "package public sanitization map",
    )

    zenodo_predecessor = value.get("zenodo_predecessor")
    require(isinstance(zenodo_predecessor, dict),
            "package Zenodo predecessor binding is absent")
    assert isinstance(zenodo_predecessor, dict)
    exact_relative_binding(
        zenodo_predecessor.get("receipt"), "package-bound Zenodo predecessor receipt",
    )
...
def release_name() -> str:
    return "Fondasi Teori Ukuran Bahasa Indonesia — 338/672 halaman"
...
def release_body(pdf_name: str) -> str:
    return (
        ...
        "Cakupan kumulatif: Jilid 1 lengkap serta Jilid 2 dari bagian pendahuluan sampai Bagian 252, "
        "yaitu halaman resmi Jilid 2 nomor 1–236 dan total 338 dari 672 halaman resmi "
        "korpus terpilih. Bab 25 baru parsial; Bagian 253 dan sesudahnya belum termasuk. "
...
def prepare_manifest(contract: ReleaseContract) -> None:
    ...
    temporary = TREE_PATH.with_name(TREE_PATH.name + ".tmp-v017-through-s252")
...
def validate_manifest(contract: ReleaseContract) -> None:
    require(TREE_PATH.is_file() and not TREE_PATH.is_symlink(),
            "through-S252 release-tree manifest is missing")
    bundle = load_privacy_overlay(contract)
    require(TREE_PATH.read_bytes() == public_manifest_payload(contract, bundle),
            "through-S252 public release-tree manifest is stale")
...
def write_pathspec(paths: tuple[str, ...], directory: Path) -> Path:
    ...
    path = directory / "through-s252-paths.nul"
...
def stage_boundary(contract: ReleaseContract) -> tuple[str, str, int]:
    ...
    require(engine.run_git("rev-parse", "HEAD") == PREDECESSOR_MAIN_COMMIT,
            "HEAD is not the exact package-bound pre-v0.17 main commit")
    ...
    with tempfile.TemporaryDirectory(prefix="o007-through-s252-git-") as name:
    ...
    require(engine.run_git("write-tree") != engine.run_git("rev-parse", "HEAD^{tree}"),
            "through-S252 boundary contains no staged change")
    engine.run_git("commit", "--no-verify", "-m", "Admit Indonesian Fremlin checkpoint through Section 252")
...
def anonymous_boundary_verify(
    ...
    require(
        ...
        "public through-S252 tag differs",
    )
    ...
    require(
        ...
        "public through-S252 commit/tree differs",
    )
...
def write_receipt(
    ...
    value = {
        ...
        "verification": {
            ...
            "catalog_v1_12_package_binding_replayed": True,
    ...
    if RECEIPT_PATH.exists():
        ...
        require(old == current, "existing through-S252 GitHub receipt differs")
...
def commit_receipt(token: str, boundary: str, payload: bytes) -> str:
    ...
    if head != boundary:
        ...
        with tempfile.TemporaryDirectory(prefix="o007-through-s252-receipt-") as name:
        ...
        engine.run_git("commit", "--no-verify", "-m", "Record public checkpoint through Section 252")
...
if __name__ == "__main__":
    try:
        ...
        print(f"ERROR: fail-closed through-S252 GitHub publication: {exc}", file=sys.stderr)
        raise SystemExit(1)
