#!/usr/bin/env python3
"""Publish the admitted O007 509/672 checkpoint in its Zenodo concept.

Only one new version of concept DOI 10.5281/zenodo.22059798 is permitted, and
only from exact public record 10.5281/zenodo.22161046.  The local Chapter-27
...
import publish_volume1_through_chapter27_github as gate
...
PREDECESSOR_RECORD_ID = 22_161_046
PREDECESSOR_DOI = "10.5281/zenodo.22161046"
PREDECESSOR_VERSION = "0.19.0-v2-through-ch26"
...
TITLE = (
    ...
    "karya D. H. Fremlin, Jilid 1 lengkap dan Jilid 2 hingga Bab 27 lengkap "
    "(prarilis 509/672 halaman)"
)
PREDECESSOR_TITLE = (
    "Fondasi Teori Ukuran — Adaptasi Bahasa Indonesia dari Measure Theory "
    "karya D. H. Fremlin, Jilid 1 lengkap dan Jilid 2 hingga Bab 26 lengkap "
    "(prarilis 444/672 halaman)"
)
PREDECESSOR_RECEIPT_RELATIVE = "qa/ZENODO_PUBLICATION_RECEIPT_V0190_V2_THROUGH_CH26.json"
PREDECESSOR_RECEIPT_PATH = ROOT / PREDECESSOR_RECEIPT_RELATIVE
PREDECESSOR_RECEIPT_BYTES = 4_571
PREDECESSOR_RECEIPT_SHA256 = (
    "412a8c76c3f08cb26d41c18186f1d257723b1a342e2dc6205881a17d56706f95"
)
RECEIPT_RELATIVE = "qa/ZENODO_PUBLICATION_RECEIPT_V0200_V2_THROUGH_CH27.json"
...
def validate_static_contract() -> None:
    require(gate.VERSION == VERSION, "through-Chapter-27 gate version differs")
    require(gate.TAG == TAG, "through-Chapter-27 gate tag differs")
    require(gate.MODEL == MODEL, "through-Chapter-27 gate model provenance differs")
    require(gate.expected_coverage() == EXPECTED_COVERAGE,
            "through-Chapter-27 gate coverage differs from 509/672")
...
def asset_bindings(contract: gate.ReleaseContract) -> dict[str, gate.AssetBinding]:
    ...
    for name, binding in bindings.items():
        ...
        require(
            ...
            f"through-Chapter-27 package-bound asset identity differs: {name}",
        )
    return bindings
...
def metadata() -> dict[str, object]:
    description = (
        "<p><strong>Prarilis parsial: 509 dari 672 halaman resmi korpus "
        ...
        "dan Bab 21–27 secara lengkap.</p>"
        "<p>Cakupan Jilid 2 pada checkpoint ini adalah halaman resmi 1–407. "
        "Bab 27 lengkap; Bab 28 dan sesudahnya belum termasuk, sehingga korpus "
    ...
    return {
        ...
        "notes": (
            "Prarilis parsial 509/672 halaman resmi: Jilid 1 lengkap dan Jilid 2 "
            "dari bagian pendahuluan sampai akhir Bab 27, mencakup halaman resmi "
            "Jilid 2 nomor 1–407. Bab 27 lengkap; Bab 28 dan sesudahnya belum "
...
def predecessor_receipt() -> dict[str, Any]:
    ...
    try:
        ...
        raise PublicationError("Zenodo v0.19 predecessor receipt is unreadable") from exc
    require(isinstance(value, dict), "Zenodo v0.19 predecessor receipt is malformed")
    ...
    require(
        ...
        "Zenodo v0.19 predecessor receipt semantics differ",
    )
...
def choose_candidate(token: str) -> tuple[dict[str, Any], str, bool]:
    ...
    require(len(exact_public) <= 1,
            "multiple exact public through-Chapter-27 versions exist")
    if exact_public:
        require(not drafts,
                "exact public through-Chapter-27 version coexists with a concept draft")
        return exact_public[0], "already_published_exact_through_chapter27", True
    ...
    return draft, "one_newversion_from_22161046", False
...
def write_receipt(
    ...
    value = {
        ...
        "record": {
            ...
            "partial_coverage": "509/672 official pages; Volume II pages 1-407 included",
        },
        "asset_order": [binding.name for binding in contract.assets],
        "assets": assets,
        "transaction_route": "existing_concept_newversion_from_22161046",
        "runtime_route": route,
        "verification": {
    ...
    if RECEIPT_PATH.exists():
        try:
            ...
            raise PublicationError(
                "existing through-Chapter-27 Zenodo receipt is unreadable"
            ) from exc
        ...
        require(old == current, "existing through-Chapter-27 Zenodo receipt differs")
...
if __name__ == "__main__":
    try:
        ...
        print(f"ERROR: fail-closed through-Chapter-27 Zenodo publication: {exc}",
              file=sys.stderr)
        raise SystemExit(1)
