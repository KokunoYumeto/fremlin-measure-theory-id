#!/usr/bin/env python3
"""Raster, structural, and visual QA for the reader through complete Chapter 27.

The admitted 477-page through-Chapter-26 reader and its passing visual receipt are
...
build_module = load_module("o007_chapter27_build", Path(__file__).with_name("build_volume1_through_chapter27.py"))

PDF = ROOT / "output" / "pdf" / "fondasi-teori-ukuran-jilid-1-dan-jilid-2-hingga-akhir-bab-27-id.pdf"
BUILD_RECEIPT = ROOT / "qa" / "through-chapter27-complete-build.json"
PRIOR_VISUAL_RECEIPT = ROOT / "qa" / "through-chapter26-pdf-visual-qa.json"
PRIOR_PDF = ROOT / "output" / "pdf" / "fondasi-teori-ukuran-jilid-1-dan-jilid-2-hingga-akhir-bab-26-id.pdf"
RECEIPT = ROOT / "qa" / "through-chapter27-pdf-visual-qa.json"
CONTACT_DIR = ROOT / "qa" / "rendered" / "through-chapter27-pdf-visual"
TMP_PARENT = ROOT / "tmp" / "pdfs"

EXPECTED_BUILD_RECEIPT_BYTES: int | None = 106_651
EXPECTED_BUILD_RECEIPT_SHA256: str | None = "5b92a746a76d0fc2fcc9c7eb467595770708e52fe87dc74b640280cff715e827"
EXPECTED_PDF_BYTES: int | None = 3_939_039
EXPECTED_PDF_SHA256: str | None = "48fda0dae726802208056bd3e8a4e3f4713ea45b498c4fe891710f7e2f349466"
EXPECTED_PAGES: int | None = 545
PRIOR_RECEIPT_BYTES = 342_088
PRIOR_RECEIPT_SHA256 = "4944b1c738d0aa849a3a6ebc7e9bfea79d35b10c3200b99b51884384e48f8170"
...
EXPECTED_UNIT_IDS = [row[1] for row in build_module.CHAPTER27_UNITS]
...
def contact_sheet(page_paths: list[Path], page_numbers: list[int], output: Path, sequence: int) -> dict[str, Any]:
    ...
    for index, (path, number) in enumerate(zip(page_paths, page_numbers, strict=True)):
        ...
        label = f"Cumulative {number} | Volume II through Chapter 27 physical {volume2_page}"
...
def verify_build_receipt() -> tuple[dict[str, Any], dict[str, Any], int]:
    require(
        ...
        "Chapter 27 build/PDF constants are unbound; patch them from the completed deterministic build before QA",
    )
    ...
    require(build.get("schema") == "o007-fremlin-volume1-plus-volume2-through-chapter27-pdf-build-v1", "build schema differs")
    ...
    expected_accounting = {
        "volume1_pages": 102,
        "volume2_last_printed_page": 407,
        "selected_total_pages": 509,
    ...
    prefix = reproducibility.get("predecessor_477_page_prefix_preservation", {})
    require(
    ...
    units = build.get("chapter27_unit_receipts")
    require(isinstance(units, list) and [row.get("unit_id") for row in units] == EXPECTED_UNIT_IDS, "Chapter 27 unit order differs")
    for row in units:
...
def audit() -> dict[str, Any]:
    ...
    require(
        prior.get("schema") == "o007-volume1-plus-volume2-through-chapter26-pdf-visual-qa-v1"
    ...
    with tempfile.TemporaryDirectory(prefix="o007-through-chapter27-pdf-", dir=TMP_PARENT) as temp_name:
        temp = Path(temp_name)
        prefix = temp / "page"
        ...
        for offset in range(0, len(inspection_pages), 9):
            ...
            output = CONTACT_DIR / f"through-chapter27-contact-{sequence:02d}.png"
    ...
    automated_checks = {
        ...
        "metadata_scope_model_license_and_509_of_672_exact": not metadata_mismatches,
        "all_pages_a4_zero_rotation": not geometry_mismatches,
        "all_477_predecessor_geometries_exact": not prior_geometry_mismatches,
        ...
        "all_477_predecessor_pixel_hashes_exact": not replay_mismatches,
    ...
    report: dict[str, Any] = {
        "schema": "o007-volume1-plus-volume2-through-chapter27-pdf-visual-qa-v1",
        "status": "automated_pass_visual_inspection_pending",
        "checked_at": "2026-08-29",
        "production_model": PRODUCTION_MODEL,
        "scope": {
            "included": ["Volume I complete", "Volume II official pages 1-407", "Chapter 27 complete through Section 276"],
            "excluded": ["Volume II Chapter 28, appendices, references, and index"],
            "locale": "id-ID",
            "license": "Design Science License for Fremlin-derived material",
            "official_source_page_accounting": "509 of 672 (Volume I 102 + Volume II official pages 1-407)",
        ...
        "prior_boundary_replay": {
            ...
            "all_477_predecessor_pages_reused_pixel_exact": True,
        },
        "inspection_plan": {
        ...
        "next_gate": "Owner inspects every saved through-Chapter27 contact sheet, then finalizes this receipt for admission.",
