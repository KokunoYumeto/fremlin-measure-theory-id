#!/usr/bin/env python3
"""Bounded visual and structural QA for Volume I + Volume II Chapters 21-22.
...
for cumulative pages 111--200, and atomically writes a receipt whose visual
...
PDF = ROOT / "output/pdf/fondasi-teori-ukuran-jilid-1-dan-jilid-2-bab-21-22-id.pdf"
BUILD_RECEIPT = ROOT / "qa/chapters21-22-complete-build.json"
PRIOR_VISUAL_RECEIPT = ROOT / "qa/volume1-pdf-visual-qa.json"
PRIOR_PDF = ROOT / "output/pdf/fondasi-teori-ukuran-jilid-1-id.pdf"
RECEIPT = ROOT / "qa/chapters21-22-pdf-visual-qa.json"
CONTACT_DIR = ROOT / "qa/rendered/chapters21-22-pdf-visual"
TMP_PARENT = ROOT / "tmp/pdfs"

EXPECTED_PDF_BYTES = 1_450_056
EXPECTED_PDF_SHA256 = "3c4a0355569da37bbcb9bd10c58ec97811bddb57b8b67d008dab23bde0da4e33"
EXPECTED_BUILD_BYTES = 22_790
EXPECTED_BUILD_SHA256 = "beea2340dc7e75ba8846acc2b4db6c0f79a226073289f23b0d23044108a630f0"
...
def contact_sheet(
    ...
    for index, (path, number) in enumerate(zip(page_paths, page_numbers, strict=True)):
        ...
        volume2_reader_page = number - PRIOR_PAGES
        label = f"Cumulative page {number} | Volume 2 Chapters 21-22 reader page {volume2_reader_page}"
...
def audit() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(
        prefix="o007-v1-ch21-ch22-pdf-", dir=TMP_PARENT
    ) as temp_name:
        temp = Path(temp_name)
        prefix = temp / "page"
        ...
        for offset in range(0, len(new_renders), batch_size):
            ...
            output = CONTACT_DIR / f"chapters21-22-contact-{sequence:02d}.png"
    ...
    automated_checks = {
        ...
        "pdfinfo_and_pypdf_page_count_200": len(page_records) == EXPECTED_PAGES,
        ...
        "all_200_pages_rendered_at_96dpi": len(page_records) == EXPECTED_PAGES,
        ...
        "new_page_contact_sheets_cover_111_200": (
    ...
    report: dict[str, Any] = {
        "schema": "o007-volume1-plus-volume2-chapters21-22-pdf-visual-qa-v1",
        ...
        "scope": {
            ...
            "excluded": ["Volume II front matter, official pages 1-11", "Volume II Chapters 23-28 and appendices"],
            "locale": "id-ID",
            "license": "Design Science License for Fremlin-derived material",
            "official_source_page_accounting": "186 of 672 (Volume I 102 + Chapter 21 pages 12-54 = 43 + Chapter 22 pages 55-95 = 41)",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--findings",
        default=(
            "All 90 new pages were inspected in the ten contact sheets; no clipping, "
