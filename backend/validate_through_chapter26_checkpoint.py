#!/usr/bin/env python3
"""Fail-closed validator for the cumulative complete-Chapter-26 backend."""
...
import generate_through_chapter26_checkpoint as generator
import validate_volume1_chapter22_checkpoint as common
from o007_backend_core import sha256_bytes


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_RECEIPT = ROOT / "backend/through-chapter26-backend-validation.json"
...
def validate_predecessor_preservation(
    catalog: dict[str, list[dict[str, Any]]], exercise_check: dict[str, Any],
) -> dict[str, Any]:
    prior = {name: load_prior(name) for name in ("corpus", "volumes", "rights", "resources", "units")}
    if catalog["corpus"] != prior["corpus"]:
        raise ValueError("catalog-v1.13 corpus record changed")
    if catalog["rights"] != prior["rights"]:
        raise ValueError("catalog-v1.13 DSL rights record changed")
    ...
    if len(current_units) != len(prior["units"]):
        raise ValueError("catalog-v1.13 unit prefix length changed")
    for before, after in zip(prior["units"], current_units):
        ...
        if after != expected:
            raise ValueError(f"catalog-v1.13 unit changed outside CP0018 admission replay: {unit_id}")
    if exercise_check != {"catalog": "backend/catalog-v1.13", "active_exercises": 757, "changed": False}:
        raise ValueError("catalog-v1.13 predecessor exercise census check differs")

    current_resources = catalog["resources"][:len(prior["resources"])]
    if len(current_resources) != len(prior["resources"]):
        raise ValueError("catalog-v1.13 resource prefix length changed")
    ...
    for before, after in zip(prior["resources"], current_resources):
        ...
        if after != expected:
            raise ValueError(f"catalog-v1.13 resource changed beyond sanctioned snapshot repair: {before['id']}")
    if {row["resource_id"] for row in repairs} != set(generator.INHERITED_SNAPSHOT_SPECS):
        raise ValueError("catalog-v1.13 stale-path repair set differs")
    ...
    return {
        "prior_catalog": "backend/catalog-v1.13",
        ...
        "cp0018_status_promotions": list(generator.PREDECESSOR_PENDING_UNIT_IDS),
        "predecessor_exercise_census_check": exercise_check,
...
def validate_catalog_state(
    ...
    if [row["id"] for row in new_units] != list(generator.COMPLETE_CHAPTER26_NEW_UNIT_IDS):
        raise ValueError("complete-Chapter-26 catalog unit order differs")
    ...
    if len(inherited_ids) != 36 or set(inherited_records) != set(inherited_ids):
    ...
    if new_pages != set(range(288, 343)):
        raise ValueError("complete Chapter 26 official-page union is not exactly 288-342")
    starts = generator.official_section_starts()
    volume2 = next(row for row in catalog["volumes"] if row["id"] == generator.VOLUME_ID)
    expected_ids = inherited_ids + list(generator.COMPLETE_CHAPTER26_NEW_UNIT_IDS)
    if (
        volume2["status"] != "in_progress"
        or volume2["admitted_source_page_span"] != "1-342"
        or volume2["admitted_unique_source_page_count"] != 342
        or volume2["admitted_unit_ids"] != expected_ids
    ):
        raise ValueError("Volume-II complete-Chapter-26 accounting differs")
    ...
    if cumulative_exercises != 841 or cumulative_hints != 196:
        raise ValueError("corrected cumulative exercise/hint census differs")
    chapter_ids = set(generator.COMPLETE_CHAPTER26_NEW_UNIT_IDS)
    chapter_units = [row for row in catalog["units"] if row["id"] in chapter_ids]
    if len(chapter_units) != 7:
        raise ValueError("complete Chapter 26 catalog unit set differs")
    ...
    if chapter_exercises != 84 or chapter_hints != 18:
        raise ValueError("complete Chapter 26 exercise/hint census differs")
    ...
    if not required <= resource_ids:
        raise ValueError("complete-Chapter-26 evidence resources are incomplete")
    return {
        "official_section_starts": starts,
        "boundary_label": "COMPLETE CHAPTER 26",
        "chapter26_complete": True,
        "predecessor_through_chapter25_pages": "1-287",
        "new_chapter26_pages": "288-342",
        "new_unique_official_page_count": len(new_pages),
        "complete_chapter26_pages": "288-342",
        "complete_chapter26_unique_page_count": 55,
        ...
        "pre_chapter26_active_exercises": 757,
        "complete_chapter26_active_exercises": chapter_exercises,
        "cumulative_active_exercises": cumulative_exercises,
        "pre_chapter26_explicit_hints": 178,
        "complete_chapter26_explicit_hints": chapter_hints,
        ...
        "required_complete_chapter26_resource_count": len(required),
    }
...
def validate_unit_closure(
    ...
    if state.config.slug == "mt263":
        segment_id = "O007-FREMLIN-V2-S263-SEG-263G"
        matching_segments = [row for row in datasets["segments"] if row["id"] == segment_id]
        if len(matching_segments) != 1:
            raise ValueError("mt263 must contain exactly one coalesced 263G segment")
        segment = matching_segments[0]
        expected_span = {
            "source_char_start": 24_181,
            "source_char_end": 26_386,
            "target_char_start": 26_161,
            "target_char_end": 28_489,
            "source_line_start": 669,
            "target_line_start": 665,
        }
        if any(segment.get(key) != value for key, value in expected_span.items()):
            raise ValueError("mt263 263G segment does not span the complete adjacent \\dvro branches")
        formula_0452 = next(
            (row for row in datasets["formulas"] if row["id"] == "O007-FREMLIN-V2-S263-FORMULA-0452"),
            None,
        )
        if not formula_0452 or (
            formula_0452.get("segment_id") != segment_id
            or formula_0452.get("source_anchor") != "263G"
            or formula_0452.get("target_anchor") != "263G"
        ):
            raise ValueError("mt263 formula 0452 is not anchored to the complete 263G segment")
...
def validate_correction_and_term_bindings(
    ...
    if {row["id"] for row in correction_records} != expected_ids or len(correction_records) != len(expected_ids):
        raise ValueError("generated complete-Chapter-26 correction set differs")
    ...
    correction_resource = resources["O007-RESOURCE-CH26-COMPLETE-SOURCE-CORRECTIONS"]
    ...
    terminology_resource = resources["O007-RESOURCE-CH26-COMPLETE-TERMINOLOGY-DECISIONS"]
    ...
    return {
        ...
        "chapter26_correction_records": len(correction_records),
...
def validate_model_and_rights(catalog: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    resources = {row["id"]: row for row in catalog["resources"]}
    model = resources["O007-RESOURCE-CH26-COMPLETE-MODEL-PROVENANCE"]
...
def validate() -> dict[str, Any]:
    states, datasets, catalog, snapshots, exercise_check = generator.run()
    ...
    manifests["catalog-v1.14"] = common.validate_output_directory(
        generator.CATALOG, catalog,
        (generator.MODEL_PATH, *tuple(sorted(snapshots)), *tuple(nested_paths)),
    )
    predecessor = validate_predecessor_preservation(catalog, exercise_check)
    ...
    return {
        ...
        "generator": "backend/generate_through_chapter26_checkpoint.py",
