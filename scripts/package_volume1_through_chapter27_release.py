#!/usr/bin/env python3
"""Build the deterministic reader-first O007 509/672 checkpoint package.

This driver packages complete Volume I and the contiguous Volume II surface
through complete Chapter 27 (official Volume II pages 1--407).  It is deliberately
...
import package_volume1_through_chapter26_release as predecessor
...
PDF_RELATIVE = "output/pdf/fondasi-teori-ukuran-jilid-1-dan-jilid-2-hingga-akhir-bab-27-id.pdf"
PDF_SOURCE = ROOT / PDF_RELATIVE
HTML_RELATIVE = "output/fondasi-teori-ukuran-v1-through-chapter27-id/html"
HTML_ROOT = ROOT / HTML_RELATIVE
PDF_PUBLIC_NAME = "00_READ_FIRST_FONDASI_TEORI_UKURAN_V1_DAN_V2_HINGGA_BAB_27.pdf"
ZIP_NAME = "fondasi-teori-ukuran-v1-dan-v2-hingga-bab27-id-v0.20.0.zip"
CHECKSUM_NAME = "SHA256SUMS-v0.20.0-v2-through-ch27.txt"
PACKAGE_ROOT = "fondasi-teori-ukuran-v1-dan-v2-hingga-bab27-id-v0.20.0"
ZIP_TIMESTAMP = (2026, 8, 29, 0, 0, 0)

ADMISSION_RELATIVE = "qa/through-chapter27-final-admission.json"
ADMISSION_RECORD_RELATIVE = "00_control/CP0020_THROUGH_CHAPTER27_ADMISSION.md"
AGGREGATE_RELATIVE = "qa/chapter27-aggregate-qa.json"
BACKEND_RELATIVE = "backend/through-chapter27-backend-validation.json"
PDF_BUILD_RELATIVE = "qa/through-chapter27-complete-build.json"
PDF_VISUAL_RELATIVE = "qa/through-chapter27-pdf-visual-qa.json"
HTML_BUILD_RELATIVE = "qa/through-chapter27-html-build.json"
HTML_READER_RELATIVE = "qa/through-chapter27-html-reader-qa.json"
PACKAGE_RECEIPT_RELATIVE = "qa/through-chapter27-release-package.json"
PACKAGE_MANIFEST_RELATIVE = "qa/through-chapter27-PACKAGE_MANIFEST.tsv"
CHECKSUM_RECEIPT_RELATIVE = "qa/through-chapter27-SHA256SUMS.txt"
PUBLIC_VALIDATION_RELATIVE = "qa/through-chapter27-public-overlay-validation.json"
PUBLIC_MANIFEST_RECEIPT_RELATIVE = "qa/through-chapter27-PUBLIC_SOURCE_TREE_MANIFEST.tsv"
PUBLIC_MAP_RECEIPT_RELATIVE = "qa/through-chapter27-PUBLIC_SANITIZATION_MAP.json"

ADMISSION_SCHEMA = "o007-fremlin-through-chapter27-final-admission-v1"
PACKAGE_SCHEMA = "o007-through-chapter27-release-package-v1"
CURRENT_CATALOG = "backend/catalog-v1.15"

PREDECESSOR_GITHUB_RECEIPT = "qa/PUBLICATION_RECEIPT_V0190_V2_THROUGH_CH26.json"
PREDECESSOR_GITHUB_RECEIPT_IDENTITY = (
    5_057,
    "c18ec05afc6eb9b0af3549845d0c762520769ed21da19a0f1c70ada239046314",
)
PREDECESSOR_GITHUB_TAG = "v0.19.0-v2-through-ch26"
PREDECESSOR_GITHUB_TAG_COMMIT = "4c8c0bbdda7b27c627afa2ab98f5b41515692fac"
PREDECESSOR_ZENODO_DOI = "10.5281/zenodo.22161046"
PREDECESSOR_ZENODO_RECEIPT = "qa/ZENODO_PUBLICATION_RECEIPT_V0190_V2_THROUGH_CH26.json"
...
EXPECTED_PDF_REFLOW_PAGES: int | None = 545
...
CHAPTER27_UNIT_STEMS = ("mt27", "mt271", "mt272", "mt273", "mt274", "mt275", "mt276")
NEW_BACKEND_UNIT_STEMS = CHAPTER27_UNIT_STEMS
EXPECTED_CHAPTER27_UNIT_IDS = {
    "mt27": "O007-FREMLIN-V2-CH27-INTRO",
    **{f"mt{section}": f"O007-FREMLIN-V2-S{section}" for section in range(271, 277)},
}
...
VOLUME2_SOURCE_PATHS = (
    ...
    "source/id-ID/mt266.tex",
    "source/id-ID/mt27.tex", "source/id-ID/mt271.tex", "source/id-ID/mt272.tex",
    "source/id-ID/mt273.tex", "source/id-ID/mt274.tex", "source/id-ID/mt275.tex",
    "source/id-ID/mt276.tex", "source/id-ID/vol2-through-chapter27-id.tex",
)
...
SCRIPT_FILES = (
    *predecessor.SCRIPT_FILES,
    "scripts/freeze_chapter27_source.py",
    "scripts/aggregate_chapter27_qa.py",
    "backend/generate_through_chapter27_checkpoint.py",
    "backend/validate_through_chapter27_checkpoint.py",
    "scripts/build_volume1_through_chapter27.py",
    "scripts/render_volume1_through_chapter27_html.py",
    "scripts/qa_volume1_through_chapter27_pdf.py",
    "scripts/qa_volume1_through_chapter27_html.py",
    "scripts/admit_volume1_through_chapter27.py",
    "scripts/package_volume1_through_chapter27_release.py",
    "scripts/publish_volume1_through_chapter27_github.py",
    "scripts/publish_volume1_through_chapter27_zenodo.py",
)
...
QA_FILES = (
    *predecessor.QA_FILES,
    ...
    "qa/chapter27/mt27-source-freeze.json",
    *(f"qa/chapter27/{name}-unit-qa.json" for name in CHAPTER27_UNIT_STEMS),
    *(f"qa/chapter27/{name}-source-anomaly-adjudication.md" for name in CHAPTER27_UNIT_STEMS[1:]),
    *(f"qa/chapter27/{name}-independent-semantic-review.md" for name in CHAPTER27_UNIT_STEMS[1:]),
)
...
def validate_html_tree(admission: dict[str, Any]) -> dict[str, int | str]:
    ...
    require(isinstance(routes, list) and len(routes) == len(set(routes)) == 81,
            "HTML reader route inventory differs")
    receipt_routes = len(routes)
    require(receipt_routes == 81 and html.get("routes") == receipt_routes,
...
def validate_canonical_replay() -> dict[str, object]:
    ...
    require(
        ...
        "bound THROUGH CHAPTER27 backend count surfaces are absent",
    )
    generated = subprocess.run(
        [sys.executable, "backend/generate_through_chapter27_checkpoint.py", "--check"],
        cwd=ROOT, capture_output=True, text=True, encoding="utf-8", errors="strict",
        timeout=300, check=False,
    )
    require(generated.returncode == 0, "canonical THROUGH CHAPTER27 generator replay failed")
    lines = [line for line in generated.stdout.splitlines() if line]
    require(lines, "canonical THROUGH CHAPTER27 generator emitted no receipt")
    generation = json.loads(lines[-1])
    require(
        generation.get("written") is False and generation.get("admitted") is False
        and generation.get("boundary_label") == "COMPLETE CHAPTER 27"
        and generation.get("chapter27_complete") is True
        and generation.get("chapter27_pages") == "343-407"
        and generation.get("chapter27_unique_official_page_count") == 65
        and generation.get("volume2_contiguous_translated_pages") == "1-407"
        and generation.get("volume2_contiguous_translated_page_count") == 407
        and generation.get("cumulative_completed_official_pages") == 509
        and generation.get("complete_chapter27_active_exercises") == 111
        and generation.get("complete_chapter27_explicit_hints") == 37
        and generation.get("cumulative_active_exercises") == 952
        and generation.get("cumulative_explicit_hints") == 233
        and generation.get("selected_corpus_official_pages") == 672
        and generation.get("inherited_admitted_unit_count") == 43
        ...
        "canonical THROUGH CHAPTER27 generator replay semantics differ",
    )
    ...
    require(
        ...
        "canonical THROUGH CHAPTER27 generator unit surface differs",
    )
    ...
    code = (
        ...
        "import validate_through_chapter27_checkpoint as v;r=v.validate();"
    ...
    require(validated.returncode == 0, "canonical THROUGH CHAPTER27 validator replay failed")
    validator_lines = [line for line in validated.stdout.splitlines() if line]
    require(validator_lines, "canonical THROUGH CHAPTER27 validator emitted no receipt")
    ...
    require(validation == {
        ...
        "schema": "o007-through-chapter27-backend-validation-v1",
        "status": "pass",
        "unit_counts": bound_unit_counts,
    }, "canonical THROUGH CHAPTER27 validator replay semantics differ")
...
def validate_admission() -> tuple[dict[str, Any], dict[str, object]]:
    ...
    require(
        ...
        "final admission version or 509/672 boundary differs",
    )
    ...
    exact_binding(ADMISSION_RECORD_RELATIVE, admission.get("content_admission"), "CP0020 admission")
    require(MODEL.encode("utf-8") in (ROOT / ADMISSION_RECORD_RELATIVE).read_bytes(),
            "CP0020 omits exact model provenance")
    predecessor_admission = admission.get("predecessor_admission")
    exact_binding("qa/through-chapter26-final-admission.json", predecessor_admission,
                  "through-Chapter-26 predecessor admission")

    unit_rows = admission.get("unit_receipts")
    require(isinstance(unit_rows, list) and len(unit_rows) == len(EXPECTED_CHAPTER27_UNIT_IDS),
            "final admission Chapter-27 unit-receipt inventory differs")
    ...
    for position, row in enumerate(unit_rows):
        ...
        stem = next((name for name, expected in EXPECTED_CHAPTER27_UNIT_IDS.items()
                     if expected == unit_id), None)
        require(stem is not None, f"unexpected final admission unit ID: {unit_id}")
        exact_binding(f"qa/chapter27/{stem}-unit-qa.json", row,
    ...
    require(observed_units == set(EXPECTED_CHAPTER27_UNIT_IDS.values()),
            "final admission Chapter-27 unit IDs differ")

    aggregate = admission.get("independent_aggregate_replay")
    exact_binding(AGGREGATE_RELATIVE, aggregate, "THROUGH CHAPTER27 aggregate")
    assert isinstance(aggregate, dict)
    require(aggregate.get("pass") is True and aggregate.get("blockers") == [],
            "admission-bound THROUGH CHAPTER27 aggregate does not pass")
    ...
    require(EXPECTED_PDF_IDENTITY is not None and EXPECTED_PDF_REFLOW_PAGES is not None,
            "Chapter 27 PDF identity/page constants are unbound; patch them after the deterministic build")
    require(live_identity(PDF_RELATIVE) == EXPECTED_PDF_IDENTITY,
            "cumulative PDF differs from the fixed Chapter-27 artifact")
    ...
    require(
        ...
        and isinstance(output_inventory, dict)
        and schema_records == 6_464
        and output_inventory.get("file_count") == 217
        and output_inventory.get("total_bytes") == 10_818_056
        and catalog_counts.get("units") == 77
        and catalog_counts.get("resources") == 292
        ...
        and backend.get("inherited_volume2_admitted_units") == 43
    ...
    require(
        ...
        and predecessor_boundary.get("version") == "0.19.0-v2-through-ch26"
...
""".encode("utf-8")
    metadata = {
        "schema": "o007-volume1-through-chapter27-release-metadata-v1",
        "version": VERSION,
        "tag": TAG,
        ...
        "coverage": {
            ...
            "remaining_volume2": {"status": "not_included", "first_official_page": 408},
        },
        "reader": {
...
def expected_coverage() -> dict[str, object]:
    return {
        ...
        "volume2_last_included_page": 407,
        "volume2_included_pages": 407,
        ...
        "volume2_chapter27_complete": True,
        "volume2_chapter27_status": "complete",
        "chapter27_first_included_page": 343,
        "chapter27_last_included_page": 407,
        "chapter27_included_pages": 65,
        "next_not_included_page": 408,
    }
...
def build(write: bool) -> dict[str, object]:
    ...
    require(
        expected_catalog_resources == 292,
        "validated THROUGH CHAPTER27 catalog resource count differs",
    )
    ...
    with tempfile.TemporaryDirectory(prefix="o007-v020-through-chapter27-", dir=ROOT / "tmp") as name:
        temp = Path(name)
        ...
        receipt = {
            ...
            "checks": {
                ...
                "volume2_through_chapter27_source_identities_exact": True,
        ...
        if write:
            ...
            for target, data in targets.items():
                ...
                temporary = target.with_name(target.name + ".tmp-v020-through-chapter27")
...
if __name__ == "__main__":
    try:
        ...
        print(f"ERROR: fail-closed THROUGH CHAPTER27 package: {exc}", file=sys.stderr)
        raise SystemExit(1)
