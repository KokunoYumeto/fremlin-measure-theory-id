#!/usr/bin/env python3
"""Build the cumulative Indonesian Volume I + Volume II Chapter 22 PDF.

The builder is deliberately closed over the frozen Fremlin mt2.2016 archive,
its source manifest, the separately frozen 2016 build support, nine explicit
id-ID overlays, and the admitted complete Volume I PDF.  Chapter 22 is built
...
MASTER = "vol2-ch22-id.tex"
MASTER_BYTES = 927
MASTER_SHA256 = "47ae09ac4f589b581e89c9320d76f1da38d4e3006b3f2d41c0534b63ef8008be"
CHAPTER_PDF_NAME = "fondasi-teori-ukuran-jilid-2-bab-22-id.pdf"
...
BUILD_NAMES = ("volume2-chapter22-id-pass-a", "volume2-chapter22-id-pass-b")
...
COMBINED_METADATA = {
    ...
    "/CreationDate": "D:20260824000000Z",
    "/ModDate": "D:20260824000000Z",
...
def verify_driver_contract(driver: Path) -> None:
    assert_file(driver, MASTER_BYTES, MASTER_SHA256, "Chapter 22 driver")
    ...
    for surface in required_surfaces:
        require(surface in text, f"Chapter 22 driver scope/metadata surface differs: {surface!r}")
    expected_order = [f"\\input {name}" for name in ("mt22", "mt221", "mt222", "mt223", "mt224", "mt225", "mt226")]
    offsets = [text.index(value) for value in expected_order]
    require(offsets == sorted(offsets), "Chapter 22 localized source order differs")
...
def build_once(lane: Path, name: str, env: dict[str, str]) -> dict[str, Any]:
    ...
    require(dvi.is_file() and dvi.stat().st_size > 0, "TeX did not create the Chapter 22 DVI")
    ...
    require(missing_characters == 0, "missing character in Chapter 22 TeX log")
    ...
    require(printed_folios[0] == OFFICIAL_CHAPTER22_FIRST_PAGE, "Chapter 22 first printed folio differs")
    require(
        printed_folios == list(range(printed_folios[0], printed_folios[0] + len(printed_folios))),
        "Chapter 22 printed folios are not contiguous",
    )
    require(
        OFFICIAL_CHAPTER22_LAST_PAGE in printed_folios,
        "Chapter 22 build does not span the complete official 55-95 folio range",
    )

    pdf_command = ["dvipdfmx", "-o", CHAPTER_PDF_NAME, dvi.name]
    dvipdfmx_stdout = run(pdf_command, stage, stage / "dvipdfmx.stdout.log", env)
    pdf = stage / CHAPTER_PDF_NAME
    require(pdf.is_file() and pdf.stat().st_size > 0, "dvipdfmx did not create the Chapter 22 PDF")
    require(pdf.read_bytes().startswith(b"%PDF-"), "Chapter 22 output lacks a PDF signature")
    ...
    return {
        ...
        "commands": {"tex": tex_command, "dvipdfmx": pdf_command, "pdfinfo": ["pdfinfo", CHAPTER_PDF_NAME]},
...
def write_pypdf_combination(volume1: Path, chapter22: Path, output: Path) -> str:
    import pypdf
    ...
    chapter_reader = PdfReader(chapter22)
    writer = PdfWriter()
    writer.append(volume1_reader, outline_item="Jilid 1 - lengkap", import_outline=True)
    writer.append(chapter_reader, outline_item="Jilid 2 - Bab 22", import_outline=True)
...
def combine_twice(
    ...
    pypdf_a = lane / "build" / "volume1-chapter22-pypdf-pass-a.pdf"
    pypdf_b = lane / "build" / "volume1-chapter22-pypdf-pass-b.pdf"
    for path in (pypdf_a, pypdf_b):
        reset_build_file(lane, path, path.name)

    version_a = write_pypdf_combination(volume1, chapter22, pypdf_a)
    version_b = write_pypdf_combination(volume1, chapter22, pypdf_b)
...
def main() -> int:
    ...
    require(first["dvi"]["sha256"] == second["dvi"]["sha256"], "two clean Chapter 22 DVI builds differ")
    require(first["dvi"]["bytes"] == second["dvi"]["bytes"], "two clean Chapter 22 DVI sizes differ")
    require(first["pdf"]["sha256"] == second["pdf"]["sha256"], "two clean Chapter 22 PDF builds differ")
    require(first["pdf"]["bytes"] == second["pdf"]["bytes"], "two clean Chapter 22 PDF sizes differ")
    require(first["pdf"]["pages"] == second["pdf"]["pages"], "Chapter 22 physical page counts differ")
    require(first["printed_folios"] == second["printed_folios"], "Chapter 22 printed folios differ")

    inputs_after_build = snapshot_inputs(lane)
    require(inputs_before == inputs_after_build, "build inputs changed during the Chapter 22 reproducibility proof")

    volume1 = lane / "output" / "pdf" / VOLUME1_PDF_NAME
    chapter22 = lane / second["pdf"]["path"]
    ...
    combined_build, combination = combine_twice(lane, volume1, chapter22, env)
    ...
    combined_info = pdfinfo(
        ...
        lane / "build" / "volume1-chapter22-combined-pdfinfo.stdout.log",
        env,
    )
    require(
    ...
    receipt: dict[str, Any] = {
        "schema": "o007-fremlin-volume1-plus-volume2-chapter22-pdf-build-v1",
        "pass": True,
        ...
        "scope": {
            ...
            "excluded_at_this_boundary": ["Volume II Chapter 21", "Volume II Chapters 23-28 and appendices"],
        ...
        "pagination": {
            ...
            "physical_reflow_accounting": {
                ...
                "chapter22_pdf_pages": second["pdf"]["pages"],
            ...
            "chapter22_printed_folios": second["printed_folios"],
        },
        ...
        "checks": {
            ...
            "only_nine_explicit_localized_overlays": True,
            "only_established_volwp_legacy_substitution": True,
            "authority_and_live_source_bytes_unchanged": True,
            "chapter22_driver_exact": True,
            "chapter21_absent_and_labeled": True,
        ...
        "next_gate": "Render and inspect every cumulative PDF page, run reader QA, then admit and publish the substantial Chapter 22 boundary.",
    }
    receipt_path = lane / "qa" / "chapter22-complete-build.json"
