#!/usr/bin/env python3
"""Build the cumulative Indonesian reader through complete Volume II Chapter 26.

The implementation extends the complete Chapter 25 build contract across the
Chapter 26 introduction and Sections 261-266.  It builds the complete localized
...
suffix to the 419-page Chapter 25 predecessor reader.  Every predecessor page is
...
MASTER = "vol2-through-chapter26-id.tex"
MASTER_BYTES = 1_711
MASTER_SHA256 = "bd51118fa231aa6e587d85348175de1ee2e9744eabaa6780139d45cc56c1267c"
VOLUME2_PDF_NAME = "fondasi-teori-ukuran-jilid-2-hingga-akhir-bab-26-id.pdf"
OUTPUT_NAME = "fondasi-teori-ukuran-jilid-1-dan-jilid-2-hingga-akhir-bab-26-id.pdf"
BUILD_NAMES = ("volume2-through-chapter26-id-pass-a", "volume2-through-chapter26-id-pass-b")

PRIOR_PDF_NAME = "fondasi-teori-ukuran-jilid-1-dan-jilid-2-hingga-akhir-bab-25-id.pdf"
PRIOR_PDF_BYTES = 2_967_476
PRIOR_PDF_SHA256 = "11c9af2cae2f0bd63cff2c8be3d511e88105fbcbf3b34888887abcf28669e8d2"
PRIOR_BUILD_RECEIPT_NAME = "through-chapter25-complete-build.json"
PRIOR_BUILD_RECEIPT_BYTES = 79_893
PRIOR_BUILD_RECEIPT_SHA256 = "c2c67672d201796a24fa1f9663e0847533079f4f92c7fc616d717a90e6364365"
PRIOR_VISUAL_RECEIPT_NAME = "through-chapter25-pdf-visual-qa.json"
PRIOR_VISUAL_RECEIPT_BYTES = 299_281
PRIOR_VISUAL_RECEIPT_SHA256 = "3bd3d546011deb5e74a9d6e23988b9a3b1c502c2e8008cf0f8c78a557c4e5811"
PRIOR_PHYSICAL_PAGES = 419
...
OFFICIAL_VOLUME2_LAST_PAGE = 342
OFFICIAL_SELECTED_PAGES = 444
OFFICIAL_CHAPTER26_FIRST_PAGE = 288
OFFICIAL_CHAPTER26_LAST_PAGE = 342

CHAPTER26_AGGREGATE_NAME = "chapter26-aggregate-qa.json"
CHAPTER26_AGGREGATE_BYTES = 13_904
CHAPTER26_AGGREGATE_SHA256 = "f321d81eb6510683487f62970be689e1e53f10269dbfd9c9e53af1871969cc13"
...
COMBINED_METADATA = {
    "/Title": "Fondasi Teori Ukuran - Jilid 1 lengkap dan Jilid 2 hingga akhir Bab 26",
    "/Author": (
    ...
    "/Subject": (
        ...
        "(102 halaman resmi) dan Jilid 2 halaman resmi 1-342; Bab 26 lengkap"
    ),
    "/Keywords": (
        "teori ukuran, perubahan variabel, Jacobian, ukuran Hausdorff, "
        "teori ukuran geometris, id-ID, O007, Design Science License"
    ),
    ...
    "/CorpusOfficialPages": "444/672",
    "/CoverageStatus": (
        "Jilid 1 lengkap; Jilid 2 halaman resmi 1-342; "
        "Bab 26 lengkap hingga Bagian 266"
    ),
...
def verify_chapter26_aggregate(path: Path, chapter26: list[dict[str, Any]]) -> dict[str, Any]:
    base.assert_file(
        path,
        CHAPTER26_AGGREGATE_BYTES,
        CHAPTER26_AGGREGATE_SHA256,
        "Chapter 26 aggregate QA",
    )
    payload = json.loads(path.read_text(encoding="utf-8", errors="strict"))
    require(payload.get("schema") == "o007-fremlin-chapter26-aggregate-qa-v1", "Chapter 26 aggregate schema differs")
    require(payload.get("pass") is True, "Chapter 26 aggregate does not pass")
    checks = payload.get("checks")
    require(isinstance(checks, dict) and checks and all(value is True for value in checks.values()), "Chapter 26 aggregate checks differ")
    scope = payload.get("scope")
    require(
        isinstance(scope, dict)
        and scope.get("official_pages") == "288-342"
        and scope.get("official_page_count") == 55
        and scope.get("candidate_cumulative_official_pages") == "444/672",
        "Chapter 26 aggregate scope differs",
    )
    ...
    require(
        ...
        "Chapter 26 aggregate census differs",
    )
    aggregate_units = payload.get("units")
    require(isinstance(aggregate_units, list) and len(aggregate_units) == len(chapter26), "Chapter 26 aggregate unit inventory differs")
    for expected, actual in zip(chapter26, aggregate_units, strict=True):
        require(actual.get("unit_id") == expected["unit_id"], f"Chapter 26 aggregate unit ID differs: {expected['stem']}")
        require(actual.get("checks_all_true") is True, f"Chapter 26 aggregate unit checks differ: {expected['stem']}")
        for key in ("source", "target", "qa_receipt"):
            bound = actual.get(key)
            current = expected[key]
            require(isinstance(bound, dict), f"Chapter 26 aggregate {expected['stem']} {key} absent")
            require(
                base.normalized_receipt_path(bound.get("path"), f"aggregate {expected['stem']} {key}") == current["path"],
                f"Chapter 26 aggregate {expected['stem']} {key} path differs",
            )
            for identity_key in ("bytes", "sha256"):
                require(
                    bound.get(identity_key) == current[identity_key],
                    f"Chapter 26 aggregate {expected['stem']} {key} {identity_key} differs",
                )
...
def localized_identities() -> tuple[
    ...
    for row in (*chapter24, *chapter25, *chapter26):
        identities[f"{row['stem']}.tex"] = (row["target"]["bytes"], row["target"]["sha256"])
    identities[MASTER] = (MASTER_BYTES, MASTER_SHA256)
    require(len(identities) == 48, "localized overlay inventory differs")
    return identities, chapter24, chapter25, chapter26


def verify_driver_contract(path: Path) -> None:
    base.assert_file(path, MASTER_BYTES, MASTER_SHA256, "through-Chapter26 driver")
    text = path.read_text(encoding="utf-8", errors="strict")
    required = (
        "% sampai akhir Bagian 266.",
        "/Subject (Adaptasi Bahasa Indonesia dari Measure Theory, Volume 2, halaman resmi 1-342, lengkap hingga akhir Bab 26)",
        PRODUCTION_MODEL,
    )
    for surface in required:
        require(surface in text, f"driver metadata surface differs: {surface!r}")
    for folio in (12, 55, 96, 138, 204, 288):
...
def verify_prior_receipt(path: Path, build_receipt: Path) -> dict[str, Any]:
    base.assert_file(
        ...
        "complete Chapter 25 build receipt",
    )
    base.assert_file(path, PRIOR_VISUAL_RECEIPT_BYTES, PRIOR_VISUAL_RECEIPT_SHA256, "complete Chapter 25 visual receipt")
    payload = json.loads(path.read_text(encoding="utf-8", errors="strict"))
    pages = payload.get("all_page_raster_audit", {}).get("pages", [])
    bound_build = payload.get("inputs", {}).get("build_receipt", {})
    require(
        payload.get("schema") == "o007-volume1-plus-volume2-through-chapter25-pdf-visual-qa-v1"
        ...
        "complete Chapter 25 visual receipt is not reusable",
    )
    require(
        ...
        "complete Chapter 25 visual receipt does not bind the exact build receipt",
    )
    return record(path)
...
def snapshot_inputs() -> dict[str, Any]:
    ...
    chapter26_aggregate = ROOT / "qa" / CHAPTER26_AGGREGATE_NAME
    ...
    base.assert_file(prior_pdf, PRIOR_PDF_BYTES, PRIOR_PDF_SHA256, "complete Chapter 25 reader")
    ...
    identities, chapter24, chapter25, chapter26 = localized_identities()
    aggregate_record = verify_chapter26_aggregate(chapter26_aggregate, chapter26)
    ...
    paths.extend(ROOT / row["qa_receipt"]["path"] for row in (*chapter24, *chapter25, *chapter26))
    return {
        ...
        "chapter26_aggregate": aggregate_record,
...
def overlay_localized_sources(stage: Path) -> list[dict[str, Any]]:
    identities, _, _, _ = localized_identities()
    localized = ROOT / "source" / "id-ID"
...
def build_once(name: str, env: dict[str, str]) -> dict[str, Any]:
    ...
    require(reset_targets == [55, 96, 138, 204, 288], f"printed-folio resets differ: {reset_targets}")
    chapter21_offset = folios.index(12)
    chapter22_offset, chapter23_offset, chapter24_offset, chapter25_offset, chapter26_offset = resets
    require(
        chapter21_offset < chapter22_offset < chapter23_offset < chapter24_offset < chapter25_offset < chapter26_offset,
    ...
    ranges = {
        ...
        "chapter26_complete": (folios[chapter26_offset:], 288, 342),
    }
    ...
    return {
        ...
        "printed_folios": {
            ...
            "anchor_targets": [12, 55, 96, 138, 204, 288],
            "reset_targets": reset_targets,
            **range_records,
            "official_range_1_342_present": True,
        },
        "tex": {
...
def verify_rebuilt_volume2_prefix(prior: Path, rebuilt_volume2: Path) -> dict[str, Any]:
    ...
    # The Chapter 25 predecessor retained two earlier transition pages from its
    # own predecessor-preserving assembly: the former Chapter 24 terminal page
    # (Volume II physical 217) and the S252 terminal page (physical 253).  They
    # differ from a fresh full-Volume-II build only in shipout/header state.
    # The old Chapter 25 terminal page (physical 309) is likewise shipped under
    # a different terminal/continuation state.  All three are discarded.  The
    # other 306 predecessor content streams and all 309 page geometries must be
    # exact before the copied Chapter 26 suffix begins at physical page 310.
    transition_pages = (217, 253, 309)
    ...
    # whitespace leaves one expected semantic mismatch: the S252 running header.
    require(
    ...
    return {
        ...
        "normalized_text_exact_except_s252_running_header": True,
        "discarded_chapter24_and_chapter25_terminal_transition_text_and_geometry_exact": True,
        "discarded_s252_transition_normalized_body_and_geometry_exact": True,
        "discarded_s252_transition_header": {"predecessor": expected_header, "rebuilt": actual_header},
...
def write_combined(prior: Path, rebuilt_volume2: Path, output: Path) -> str:
    ...
    writer.append(
        ...
        outline_item="Jilid 2 - Bab 26 lengkap",
...
def combine_twice(prior: Path, rebuilt_volume2: Path) -> tuple[Path, dict[str, Any]]:
    outputs = (
        ROOT / "build" / "volume1-through-chapter26-pypdf-pass-a.pdf",
        ROOT / "build" / "volume1-through-chapter26-pypdf-pass-b.pdf",
    )
    versions: list[str] = []
...
def main() -> int:
    ...
    combined_info = base.pdfinfo(
        ...
        ROOT / "build" / "volume1-through-chapter26-combined-pdfinfo.stdout.log",
        env,
    )
    appended_pages = second["pdf"]["pages"] - PRIOR_VOLUME2_PHYSICAL_PAGES
    require(appended_pages > 0, "no new Chapter 26 surfaces were appended")
    ...
    receipt: dict[str, Any] = {
        "schema": "o007-fremlin-volume1-plus-volume2-through-chapter26-pdf-build-v1",
        "pass": True,
        ...
        "scope": {
            ...
            "included": [
                "Volume I complete",
                "Volume II official pages 1-287 complete",
                "Volume II Chapter 26 introduction and Sections 261-266 complete, official pages 288-342",
            ],
            "excluded_at_this_boundary": ["Volume II Chapters 27-28 and appendices"],
            "volume2_official_pages_1_342_complete": True,
            "chapter26_status": "complete through Section 266",
            "corpus_official_pages": "444/672",
        ...
        "pagination": {
            "official_source_accounting": {
                ...
                "chapter26_first_printed_page": OFFICIAL_CHAPTER26_FIRST_PAGE,
                "chapter26_last_printed_page_at_boundary": OFFICIAL_CHAPTER26_LAST_PAGE,
                "chapter26_pages_at_boundary": 55,
                "selected_total_pages": OFFICIAL_SELECTED_PAGES,
                "full_corpus_pages": 672,
                "equation": "102 + 342 = 444",
            },
            ...
            "volume2_through_chapter26_printed_folios": second["printed_folios"],
        },
        "inputs": inputs_before,
        "chapter26_unit_receipts": inputs_before["chapter26_units"],
        "chapter26_aggregate_qa": inputs_before["chapter26_aggregate"],
        "builds": [first, second],
        "reproducibility": {
            ...
            "predecessor_419_page_prefix_preservation": prior_prefix,
        },
        "environment": {
        ...
        "checks": {
            "all_seven_chapter26_unit_receipts_present_and_pass": True,
            "chapter26_aggregate_bound_and_pass": True,
            ...
            "exact_through_chapter26_driver_and_folio_anchors": True,
            "official_volume2_folio_range_1_342_present": True,
            "official_accounting_102_plus_342_equals_444": True,
            ...
            "rebuilt_volume2_first_309_predecessor_pages_exact_except_three_discarded_transition_pages": True,
            "all_419_predecessor_content_text_geometry_fingerprints_exact": True,
        ...
        "next_gate": "Render all pages, replay the 419-page prior raster prefix, and prepare every appended contact sheet for owner visual inspection.",
    }
    receipt_path = ROOT / "qa" / "through-chapter26-complete-build.json"
