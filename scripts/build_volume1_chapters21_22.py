#!/usr/bin/env python3
"""Build the cumulative Indonesian Volume I + Volume II Chapters 21-22 PDF.

The builder is deliberately closed over the frozen Fremlin mt2.2016 archive,
its source manifest, the separately frozen 2016 build support, sixteen explicit
id-ID overlays, and the admitted complete Volume I PDF.  Chapters 21-22 are built
twice in clean, bounded staging directories.  No canonical output is admitted
unless the two DVI and PDF files are byte-identical and the live inputs remain
unchanged.  Visual admission and publication are downstream operations.
"""
...
MASTER = "vol2-ch21-ch22-id.tex"
MASTER_BYTES = 1_137
MASTER_SHA256 = "1a979bd293a4a2d70f820d3c495a60de31f460683f3857d2c56edcbf370dfbed"
CHAPTERS_PDF_NAME = "fondasi-teori-ukuran-jilid-2-bab-21-22-id.pdf"
...
OUTPUT_NAME = "fondasi-teori-ukuran-jilid-1-dan-jilid-2-bab-21-22-id.pdf"
...
LOCALIZED_OVERLAY_IDENTITIES = {
    "id-overrides.tex": (1_890, "8310f167fed7bdfe8e7fb1ddd1ccf33ba34c4999d36849b55b4471019eec9e16"),
...
BUILD_NAMES = ("volume2-chapters21-22-id-pass-a", "volume2-chapters21-22-id-pass-b")

VOLWP_ACTIVATION_BEFORE = b"  \\usegraphicx\r\n  }"
VOLWP_ACTIVATION_AFTER = b"  \\atUEssex\r\n  }"

COMBINED_METADATA = {
    "/Title": "Fondasi Teori Ukuran - Jilid 1 lengkap dan Jilid 2 Bab 21-22",
    "/Author": (
    ...
    "/Subject": (
        ...
        "(102 halaman resmi) dan Jilid 2 Bab 21-22 (halaman resmi 12-95); "
        "halaman awal Jilid 2 (1-11) belum termasuk"
    ),
    "/Keywords": (
        "teori ukuran, integrasi, taksonomi ruang ukur, Teorema Dasar "
        "Kalkulus, id-ID, O007, Design Science License"
    ),
    ...
    "/CoverageStatus": "Jilid 1 lengkap; Jilid 2 Bab 21-22; halaman awal Jilid 2 (1-11) belum termasuk",
...
def verify_driver_contract(driver: Path) -> None:
    assert_file(driver, MASTER_BYTES, MASTER_SHA256, "Chapters 21-22 driver")
    text = driver.read_text(encoding="utf-8", errors="strict")
    required_surfaces = (
        "Halaman awal Jilid 2 (1--11) belum",
    ...
    for surface in required_surfaces:
        require(surface in text, f"Chapters 21-22 driver scope/metadata surface differs: {surface!r}")
    expected_order = [
        f"\\input {name}"
        for name in (
    ...
    require(offsets == sorted(offsets), "Chapters 21-22 localized source order differs")
...
def build_once(lane: Path, name: str, env: dict[str, str]) -> dict[str, Any]:
    ...
    require(dvi.is_file() and dvi.stat().st_size > 0, "TeX did not create the Chapters 21-22 DVI")
    ...
    require(missing_characters == 0, "missing character in Chapters 21-22 TeX log")
    ...
    require(printed_folios[0] == OFFICIAL_CHAPTER21_FIRST_PAGE, "Chapter 21 first printed folio differs")
    resets = [
        index
        for index in range(1, len(printed_folios))
        if printed_folios[index] <= printed_folios[index - 1]
    ]
    require(len(resets) == 1, "expected exactly one chapter-boundary folio reset")
    chapter22_offset = resets[0]
    chapter21_folios = printed_folios[:chapter22_offset]
    chapter22_folios = printed_folios[chapter22_offset:]
    require(
    ...
    require(pdf.is_file() and pdf.stat().st_size > 0, "dvipdfmx did not create the Chapters 21-22 PDF")
    require(pdf.read_bytes().startswith(b"%PDF-"), "Chapters 21-22 output lacks a PDF signature")
    ...
    return {
        ...
        "printed_folios": {
            ...
            "official_range_12_95_present": True,
        },
        "tex": {
...
def write_pypdf_combination(volume1: Path, chapters21_22: Path, output: Path) -> str:
    import pypdf
    ...
    chapter_reader = PdfReader(chapters21_22)
    writer = PdfWriter()
    writer.append(volume1_reader, outline_item="Jilid 1 - lengkap", import_outline=True)
    writer.append(chapter_reader, outline_item="Jilid 2 - Bab 21-22", import_outline=True)
...
def combine_twice(
    ...
    pypdf_a = lane / "build" / "volume1-chapters21-22-pypdf-pass-a.pdf"
    pypdf_b = lane / "build" / "volume1-chapters21-22-pypdf-pass-b.pdf"
    for path in (pypdf_a, pypdf_b):
        reset_build_file(lane, path, path.name)

    version_a = write_pypdf_combination(volume1, chapters21_22, pypdf_a)
    version_b = write_pypdf_combination(volume1, chapters21_22, pypdf_b)
...
def main() -> int:
    ...
    require(first["dvi"]["sha256"] == second["dvi"]["sha256"], "two clean Chapters 21-22 DVI builds differ")
    require(first["dvi"]["bytes"] == second["dvi"]["bytes"], "two clean Chapters 21-22 DVI sizes differ")
    require(first["pdf"]["sha256"] == second["pdf"]["sha256"], "two clean Chapters 21-22 PDF builds differ")
    require(first["pdf"]["bytes"] == second["pdf"]["bytes"], "two clean Chapters 21-22 PDF sizes differ")
    require(first["pdf"]["pages"] == second["pdf"]["pages"], "Chapters 21-22 physical page counts differ")
    require(first["printed_folios"] == second["printed_folios"], "Chapters 21-22 printed folios differ")

    inputs_after_build = snapshot_inputs(lane)
    require(inputs_before == inputs_after_build, "build inputs changed during the Chapters 21-22 reproducibility proof")

    volume1 = lane / "output" / "pdf" / VOLUME1_PDF_NAME
    chapters21_22 = lane / second["pdf"]["path"]
    ...
    combined_build, combination = combine_twice(lane, volume1, chapters21_22, env)
    ...
    combined_info = pdfinfo(
        ...
        lane / "build" / "volume1-chapters21-22-combined-pdfinfo.stdout.log",
        env,
    )
    require(
    ...
    receipt: dict[str, Any] = {
        "schema": "o007-fremlin-volume1-plus-volume2-chapters21-22-pdf-build-v1",
        "pass": True,
        ...
        "scope": {
            ...
            "locale": "id-ID",
            "included": ["Volume I complete", "Volume II Chapter 21 complete", "Volume II Chapter 22 complete"],
            "excluded_at_this_boundary": ["Volume II front matter, official pages 1-11", "Volume II Chapters 23-28 and appendices"],
        ...
        "pagination": {
            "official_source_accounting": {
                ...
                "equation": "102 + 43 + 41 = 186",
            },
            "physical_reflow_accounting": {
                "volume1_pdf_pages": volume1_info["pages"],
                "chapters21_22_pdf_pages": second["pdf"]["pages"],
            ...
            "chapters21_22_printed_folios": second["printed_folios"],
        },
        "inputs": inputs_before,
        "builds": [first, second],
        "reproducibility": {
            "clean_chapter_build_count": 2,
            "chapter_dvi_byte_exact": True,
            "chapter_pdf_byte_exact": True,
            "chapter_dvi_sha256": second["dvi"]["sha256"],
            "chapter_pdf_sha256": second["pdf"]["sha256"],
        ...
        "checks": {
            ...
            "only_sixteen_explicit_localized_overlays": True,
            "only_established_volwp_legacy_substitution": True,
            "authority_and_live_source_bytes_unchanged": True,
            "chapters21_22_driver_exact": True,
            "volume2_front_matter_pages_1_11_absent_and_labeled": True,
            ...
            "chapter_pdf_signature_valid": True,
        ...
        "next_gate": "Render and inspect every cumulative PDF page, run reader QA, then admit and publish the substantial Chapters 21-22 boundary.",
    }
    receipt_path = lane / "qa" / "chapters21-22-complete-build.json"
