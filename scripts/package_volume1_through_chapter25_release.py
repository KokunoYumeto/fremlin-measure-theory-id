#!/usr/bin/env python3
"""Build the deterministic reader-first O007 389/672 checkpoint package.

This driver packages complete Volume I and the contiguous Volume II surface
through complete Chapter 25 (official Volume II pages 1--287).  It is deliberately
...
import package_volume1_through_s252_release as predecessor
...
PDF_RELATIVE = "output/pdf/fondasi-teori-ukuran-jilid-1-dan-jilid-2-hingga-akhir-bab-25-id.pdf"
PDF_SOURCE = ROOT / PDF_RELATIVE
HTML_RELATIVE = "output/fondasi-teori-ukuran-v1-through-chapter25-id/html"
HTML_ROOT = ROOT / HTML_RELATIVE
PDF_PUBLIC_NAME = "00_READ_FIRST_FONDASI_TEORI_UKURAN_V1_DAN_V2_HINGGA_BAB_25.pdf"
ZIP_NAME = "fondasi-teori-ukuran-v1-dan-v2-hingga-bab25-id-v0.18.0.zip"
CHECKSUM_NAME = "SHA256SUMS-v0.18.0-v2-through-ch25.txt"
PACKAGE_ROOT = "fondasi-teori-ukuran-v1-dan-v2-hingga-bab25-id-v0.18.0"
...
BACKEND_RELATIVE = "backend/through-chapter25-backend-validation.json"
PDF_BUILD_RELATIVE = "qa/through-chapter25-complete-build.json"
PDF_VISUAL_RELATIVE = "qa/through-chapter25-pdf-visual-qa.json"
HTML_BUILD_RELATIVE = "qa/through-chapter25-html-build.json"
HTML_READER_RELATIVE = "qa/through-chapter25-html-reader-qa.json"
PACKAGE_RECEIPT_RELATIVE = "qa/through-chapter25-release-package.json"
PACKAGE_MANIFEST_RELATIVE = "qa/through-chapter25-PACKAGE_MANIFEST.tsv"
CHECKSUM_RECEIPT_RELATIVE = "qa/through-chapter25-SHA256SUMS.txt"
PUBLIC_VALIDATION_RELATIVE = "qa/through-chapter25-public-overlay-validation.json"
PUBLIC_MANIFEST_RECEIPT_RELATIVE = "qa/through-chapter25-PUBLIC_SOURCE_TREE_MANIFEST.tsv"
PUBLIC_MAP_RECEIPT_RELATIVE = "qa/through-chapter25-PUBLIC_SANITIZATION_MAP.json"
...
CURRENT_CATALOG = "backend/catalog-v1.13"

PREDECESSOR_GITHUB_RECEIPT = "qa/PUBLICATION_RECEIPT_V0170_V2_THROUGH_S252.json"
PREDECESSOR_GITHUB_RECEIPT_IDENTITY = (
    4_882,
    "57720a0fa79edb64224cb9a23084c123657addd20af7c09845c06dd2923572c2",
)
PREDECESSOR_GITHUB_TAG = "v0.17.0-v2-through-s252"
PREDECESSOR_GITHUB_TAG_COMMIT = "d7d35539f7b11274a7ff202ce24ee8aef26c5550"
PREDECESSOR_GITHUB_RECEIPT_COMMIT = "8b05306b63ec563a9953ad3e6d1407795eb4c53f"
PREDECESSOR_GITHUB_MAIN_COMMIT = "d51e0b357190f566791e46579fab53771d98ea43"
PREDECESSOR_ZENODO_DOI = "10.5281/zenodo.22105474"
PREDECESSOR_ZENODO_RECEIPT = "qa/ZENODO_PUBLICATION_RECEIPT_V0170_V2_THROUGH_S252.json"
...
CHAPTER25_UNIT_STEMS = ("mt25", "mt251", "mt252", "mt253", "mt254", "mt255", "mt256", "mt257")
NEW_BACKEND_UNIT_STEMS = ("mt253", "mt254", "mt255", "mt256", "mt257")
EXPECTED_CHAPTER25_UNIT_IDS = {
    "mt25": "O007-FREMLIN-V2-C25-INTRO",
    **{f"mt{section}": f"O007-FREMLIN-V2-S{section}" for section in range(251, 258)},
}
...
VOLUME2_SOURCE_PATHS = (
    ...
    "source/id-ID/vol2-through-chapter25-id.tex",
)
...
SCRIPT_FILES = (
    *predecessor.SCRIPT_FILES,
    "scripts/aggregate_chapter25_complete_qa.py",
    "backend/generate_through_chapter25_checkpoint.py",
    "backend/validate_through_chapter25_checkpoint.py",
    "scripts/build_volume1_through_chapter25.py",
    "scripts/render_volume1_through_chapter25_html.py",
    "scripts/qa_volume1_through_chapter25_pdf.py",
    "scripts/qa_volume1_through_chapter25_html.py",
    "scripts/admit_volume1_through_chapter25.py",
...
QA_FILES = (
    ...
    *(f"qa/chapter25/{name}-unit-qa.json" for name in CHAPTER25_UNIT_STEMS),
)
...
def validate_html_tree(admission: dict[str, Any]) -> dict[str, int | str]:
    ...
    require(isinstance(routes, list) and len(routes) == len(set(routes)) == 67,
            "HTML reader route inventory differs")
    receipt_routes = len(routes)
    require(receipt_routes == 67 and html.get("routes") == receipt_routes,
...
def validate_canonical_replay() -> dict[str, object]:
    ...
    require(
        ...
        "bound THROUGH CHAPTER25 backend count surfaces are absent",
    )
    generated = subprocess.run(
        [sys.executable, "backend/generate_through_chapter25_checkpoint.py", "--check"],
        cwd=ROOT, capture_output=True, text=True, encoding="utf-8", errors="strict",
        timeout=300, check=False,
    )
    require(generated.returncode == 0, "canonical THROUGH CHAPTER25 generator replay failed")
    lines = [line for line in generated.stdout.splitlines() if line]
    require(lines, "canonical THROUGH CHAPTER25 generator emitted no receipt")
    generation = json.loads(lines[-1])
    require(
        generation.get("written") is False and generation.get("admitted") is False
        and generation.get("boundary_label") == "COMPLETE CHAPTER 25"
        and generation.get("chapter25_complete") is True
        and generation.get("chapter25_pages") == "204-287"
        and generation.get("chapter25_unique_official_page_count") == 84
        and generation.get("volume2_contiguous_translated_pages") == "1-287"
        and generation.get("volume2_contiguous_translated_page_count") == 287
        and generation.get("cumulative_completed_official_pages") == 389
        and generation.get("complete_chapter25_active_exercises") == 156
        and generation.get("complete_chapter25_explicit_hints") == 35
        and generation.get("cumulative_active_exercises") == 757
        and generation.get("cumulative_explicit_hints") == 178
        and generation.get("selected_corpus_official_pages") == 672
        and generation.get("inherited_admitted_unit_count") == 31
        and generation.get("inherited_snapshot_count") == 2
        and generation.get("new_pre_admission_unit_count") == 5
        ...
        "canonical THROUGH CHAPTER25 generator replay semantics differ",
    )
    ...
    require(
        ...
        "canonical THROUGH CHAPTER25 generator unit surface differs",
    )
    ...
    code = (
        ...
        "import validate_through_chapter25_checkpoint as v;r=v.validate();"
    ...
    require(validated.returncode == 0, "canonical THROUGH CHAPTER25 validator replay failed")
    validator_lines = [line for line in validated.stdout.splitlines() if line]
    require(validator_lines, "canonical THROUGH CHAPTER25 validator emitted no receipt")
    ...
    }, "canonical THROUGH CHAPTER25 validator replay semantics differ")
...
def validate_admission() -> tuple[dict[str, Any], dict[str, object]]:
    ...
    require(
        ...
        "final admission version or 389/672 boundary differs",
    )
    ...
    exact_binding(ADMISSION_RECORD_RELATIVE, admission.get("content_admission"), "CP0018 admission")
    ...
    exact_binding("qa/through-s252-final-admission.json", predecessor_admission,
                  "through-S252 predecessor admission")

    unit_rows = admission.get("unit_receipts")
    require(isinstance(unit_rows, list) and len(unit_rows) == len(EXPECTED_CHAPTER25_UNIT_IDS),
            "final admission Chapter-25 unit-receipt inventory differs")
    ...
    for position, row in enumerate(unit_rows):
        ...
        stem = next((name for name, expected in EXPECTED_CHAPTER25_UNIT_IDS.items()
                     if expected == unit_id), None)
        require(stem is not None, f"unexpected final admission unit ID: {unit_id}")
        exact_binding(f"qa/chapter25/{stem}-unit-qa.json", row,
    ...
    require(observed_units == set(EXPECTED_CHAPTER25_UNIT_IDS.values()),
            "final admission Chapter-25 unit IDs differ")

    aggregate = admission.get("independent_aggregate_replay")
    exact_binding(AGGREGATE_RELATIVE, aggregate, "THROUGH CHAPTER25 aggregate")
    assert isinstance(aggregate, dict)
    require(aggregate.get("pass") is True and aggregate.get("blockers") == [],
            "admission-bound THROUGH CHAPTER25 aggregate does not pass")
    ...
    require(live_identity(PDF_RELATIVE) == EXPECTED_PDF_IDENTITY,
            "cumulative PDF differs from the fixed Chapter-25 artifact")
    ...
    require(
        ...
        and backend.get("inherited_volume2_admitted_units") == 31
        and backend.get("new_units_pre_admission") == 5
    ...
    require(
        ...
        and predecessor_boundary.get("version") == "0.17.0-v2-through-s252"
    ...
    require(live_identity(PREDECESSOR_GITHUB_RECEIPT) == PREDECESSOR_GITHUB_RECEIPT_IDENTITY,
            "immutable GitHub v0.17 predecessor receipt differs")
    require(
    ...
    require(
        ...
        and zenodo_predecessor.get("version") == "0.17.0-v2-through-s252"
        and zenodo_predecessor.get("record", {}).get("doi") == PREDECESSOR_ZENODO_DOI,
        "immutable Zenodo v0.17 predecessor receipt differs",
    )
...
- Checkpoint scope: complete Volume I and contiguous Volume II front matter through complete Chapter 25, official Volume II pages 1--287; 389 of 672 official corpus pages.
...
""".encode("utf-8")
    metadata = {
        "schema": "o007-volume1-through-chapter25-release-metadata-v1",
        "version": VERSION,
        "tag": TAG,
        ...
        "coverage": {
            ...
            "remaining_volume2": {"status": "not_included", "first_official_page": 288},
        },
        "reader": {
        ...
        "deterministic_zip_timestamp": "2026-08-28T00:00:00Z",
    }
    closure = {
...
def build(write: bool) -> dict[str, object]:
    ...
    require(
        expected_catalog_resources == 242,
        "validated THROUGH CHAPTER25 catalog resource count differs",
    )
    ...
    with tempfile.TemporaryDirectory(prefix="o007-v018-through-chapter25-", dir=ROOT / "tmp") as name:
        temp = Path(name)
        ...
        receipt = {
            ...
            "package_details": {
                ...
                "fixed_timestamp": "2026-08-28T00:00:00Z", "cumulative_pdf_occurrences": 1,
            ...
            "checks": {
                ...
                "volume2_through_chapter25_source_identities_exact": True,
        ...
        if write:
            ...
            for target, data in targets.items():
                ...
                temporary = target.with_name(target.name + ".tmp-v018-through-chapter25")
...
if __name__ == "__main__":
    try:
        ...
        print(f"ERROR: fail-closed THROUGH CHAPTER25 package: {exc}", file=sys.stderr)
        raise SystemExit(1)
