#!/usr/bin/env python3
"""Build the deterministic cumulative Volume I + Volume II Chapters 21-22 reader.
...
Fourteen source-bound Chapters 21-22 routes are then rendered from the canonical
...
OUTPUT = ROOT / "output" / "fondasi-teori-ukuran-v1-ch21-ch22-id" / "html"
RECEIPT = ROOT / "qa" / "chapters21-22-html-build.json"
...
UNIT_CONFIG: dict[str, dict[str, Any]] = {
    "21": {
        "unit_id": "O007-FREMLIN-V2-CH21",
        "title": "Bab 21 — Taksonomi ruang ukur",
        "marker": "Saya memulai jilid ini dengan sebuah `bab berbintang'",
        "official_page": 12,
        "chapter_pages": [12, 54],
    },
    "211": {
        ...
        "title": "Definisi",
        "marker": "Saya mulai dengan suatu daftar definisi",
        "official_page": 12,
        "chapter_pages": [12, 54],
    },
    "212": {
        ...
        "title": "Ruang lengkap",
        "marker": "Dalam dua bagian berikutnya dari bab ini",
        "official_page": 17,
        "chapter_pages": [12, 54],
    },
    "213": {
        "unit_id": "O007-FREMLIN-V2-S213",
        "title": "Ruang semihingga, ditentukan secara lokal, dan dapat dilokalkan",
        "marker": "Pada bagian ini saya menghimpun berbagai fakta berguna",
        "official_page": 23,
        "chapter_pages": [12, 54],
    },
    "214": {
        ...
        "title": "Subruang",
        "marker": "Dalam \\S131 saya menjelaskan suatu konstruksi ukuran subruang",
        "official_page": 34,
        "chapter_pages": [12, 54],
    },
    "215": {
        "unit_id": "O007-FREMLIN-V2-S215",
        "title": "Ruang sigma-hingga dan prinsip penghabisan",
        "marker": "Saya menyisipkan bagian singkat ini",
        "official_page": 44,
        "chapter_pages": [12, 54],
        "title_math": r"\sigma",
        "title_math_before": "Ruang ",
        "title_math_after": "-hingga dan prinsip penghabisan",
    },
    "216": {
        "unit_id": "O007-FREMLIN-V2-S216",
        "title": "Contoh-contoh",
        "marker": "Sudah menjadi kebiasaan umum",
        "official_page": 48,
        "chapter_pages": [12, 54],
    ...
    "221": {
        ...
        "chapter_pages": [55, 95],
        "title_math": r"\Bbb R",
        "title_math_before": "Teorema Vitali di ",
        "title_math_after": "",
...
def preprocess_source(unit: str, source: str) -> str:
    prepared = source
    if unit == "21":
        marker = UNIT_CONFIG[unit]["marker"]
        if prepared.count(marker) != 1:
            raise ValueError("Chapter 21 introduction marker differs")
        prepared = prepared[prepared.index(marker):]
        if prepared.count(r"\discrpage") != 1:
            raise ValueError("Chapter 21 final print-page control differs")
        prepared = prepared.replace(r"\discrpage", "", 1)
    if unit == "211":
        command = r"\ifwithproofs"
        if prepared.count(command) != 1:
            raise ValueError("S211 proof-conditional surface differs")
        start = prepared.index(command)
        proof, proof_end = read_group(prepared, start + len(command))
        alternative = r"\else"
        if not prepared.startswith(alternative, proof_end):
            raise ValueError("S211 proof-conditional alternative differs")
        fallback, fallback_end = read_group(prepared, proof_end + len(alternative))
        if not prepared.startswith(r"\fi", fallback_end):
            raise ValueError("S211 proof-conditional close differs")
        prepared = prepared[:start] + proof + prepared[fallback_end + len(r"\fi"):]
    if unit == "212":
        if prepared.count(r"\grheada") != 4:
            raise ValueError("S212 print-heading controls differ")
        if prepared.count(r"\grheadb") != 4:
            raise ValueError("S212 continuation-heading controls differ")
        prepared = prepared.replace(r"\grheada", "")
        prepared = prepared.replace(r"\grheadb", "")
    if unit == "213":
        if prepared.count(r"\leaveitout") != 1:
            raise ValueError("S213 reader inclusion control differs")
        prepared = prepared.replace(r"\leaveitout", "", 1)
    if unit == "214":
        if prepared.count(r"\BanG") != 1:
            raise ValueError("S214 alternate contradiction glyph differs")
        prepared = prepared.replace(r"\BanG", r"\Bang", 1)
        editorial = r"\dvAformerly"
        if prepared.count(editorial) != 1:
            raise ValueError("S214 former-source-ID marker differs")
        start = prepared.index(editorial)
        _former_id, end = read_group(prepared, start + len(editorial))
        prepared = prepared[:start] + prepared[end:]
    if unit == "216":
        style_control = r"\ifdim\pagewidth>467pt\fontdimen3\tenbf=3pt\fontdimen4\tenbf=2pt\fi"
        if prepared.count(style_control) != 1:
            raise ValueError("S216 font-spacing control differs")
        prepared = prepared.replace(style_control, "", 1)
        for control in (r"\fontdimen3\tenbf=1.92pt", r"\fontdimen4\tenbf=1.28pt"):
            if prepared.count(control) != 1:
                raise ValueError("S216 restored font-spacing control differs")
            prepared = prepared.replace(control, "", 1)
        conditional = r"\ifdim\pagewidth>400pt"
        if prepared.count(conditional) != 1:
            raise ValueError("S216 responsive display conditional differs")
        start = prepared.index(conditional)
        else_start = prepared.index(r"\else", start)
        end = prepared.index(r"\fi", else_start) + len(r"\fi")
        wide_branch = prepared[start + len(conditional):else_start]
        prepared = prepared[:start] + wide_branch + prepared[end:]
    if unit == "22":
        marker = r"\newchapter{22}"
        if prepared.count(marker) != 1:
...
def patch_unit_page(
    ...
    # Some source running titles contain the first active math atom.  The
    ...
    if title_math:
        ...
        new = (
            f'<h1>{html.escape(config["title_math_before"])}'
            f'<span class="math inline" data-source-tex="{source_attr}">'
            f"\\({html.escape(title_math)}\\)</span>"
            f'{html.escape(config["title_math_after"])}</h1>'
        )
        if rendered.count(old) != 1:
            raise ValueError(f"S{unit} mathematical reader-title surface differs")
...
def render_units(
    ...
    for unit, config in UNIT_CONFIG.items():
        ...
        argv = [
            "render_volume1_chapters21_22_html.py",
            str(prepared),
            str(output),
...
def root_document(id_routes: dict[str, str]) -> str:
    metadata = {
        ...
        "corpus_id": "O007-FREMLIN",
        "locale": "id-ID",
        "coverage_status": "complete-volume-1-plus-complete-volume-2-chapters-21-22",
        "official_pages_complete": 186,
        "corpus_official_pages": 672,
        "volume_1_status": "complete",
        "volume_2_front_matter_pages_1_11_status": "pending",
        "volume_2_chapter_21_status": "complete",
        "volume_2_chapter_22_status": "complete",
        "volume_2_chapter_21_source_pages": [12, 54],
    ...
    chapter21_cards = cards("21", "21")
    chapter22_cards = cards("22", "22")
    return f'''<!doctype html>
<html lang="id-ID">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <meta name="generator" content="O007 cumulative Volume I and Volume II Chapters 21-22 offline reader">
...
<header class="book-header">
  ...
  <p><em>Pembaca kumulatif Bahasa Indonesia: Volume 1 lengkap + Volume 2 Bab 21–22 lengkap</em></p>
</header>
<main id="isi">
<section class="edition-status" aria-label="Status edisi">
  <div><strong>186 / 672</strong>halaman resmi selesai</div>
  <div><strong>Volume 1</strong>lengkap, 102 halaman resmi</div>
  <div><strong>Bab 21–22</strong>Volume 2, halaman sumber 12–95</div>
</section>
<section class="content-block"><h2>Mulai membaca</h2>
  ...
  <a href="21/index.html">Mulai Volume 2, Bab 21</a> ·
  <a href="22/index.html">Langsung ke Volume 2, Bab 22</a></p>
</section>
...
<section class="toc-group"><h2>Volume 2 — Fondasi Luas</h2>
  <article class="toc-card"><h3><a href="21/index.html">Bab 21 — Taksonomi ruang ukur</a></h3>
    <p>Terjemahan lengkap bab, halaman sumber resmi 12–54.</p></article>
  {chapter21_cards}
...
<section class="content-block"><h2>Status korpus</h2>
  <p>Pembaca ini mencakup Volume 1 lengkap serta Volume 2 Bab 21–22 lengkap: 186 dari 672 halaman resmi. Halaman awal Volume 2 (1–11), Bab 23–28, dan lampiran Volume 2 belum tersedia.</p>
...
def verify_inline_javascript(root: Path, pages: list[Path]) -> dict[str, Any]:
    ...
    for route in NEW_ROUTES:
        if scripts_by_route.get(route) != 1:
            raise ValueError(f"Chapters 21-22 inline JavaScript surface differs: {route}")
    ...
    if {row[0] for row in snapshots} != set(NEW_ROUTES) or len(snapshots) != 14:
        raise ValueError("Node did not evaluate exactly fourteen Chapters 21-22 macro configs")
    ...
    return {
        ...
        "chapters21_22_macro_configs_evaluated": len(snapshots),
...
def verify_site(root: Path, units: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    if routes != set(ROUTE_ORDER) or len(pages) != 42:
...
def build_once(
    ...
    return {
        "schema": "o007-volume1-chapters21-22-html-build-v1",
        "status": "pass",
        "coverage": {
            "official_pages_complete": 186,
            "corpus_official_pages": 672,
            "volume_1": "complete",
            "volume_2_front_matter_pages_1_11": "pending",
...
def safe_replace_tree(source: Path, destination: Path) -> None:
    expected_parent = (ROOT / "output" / "fondasi-teori-ukuran-v1-ch21-ch22-id").resolve()
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="o007-v1-ch21-ch22-html-", dir=ROOT / "tmp") as temp_name:
        temp = Path(temp_name)
