#!/usr/bin/env python3
"""Publish the admitted O007 239/672 checkpoint to the existing GitHub line.
...
1--137 boundary.  Preflight performs no Git command, network request, or
...
PREDECESSOR_TAG = "v0.14.0-v2-ch21-ch22"
PREDECESSOR_BOUNDARY_COMMIT = "d31490adfe313f92705e44985f93d09c7e70bdfc"
PREDECESSOR_MAIN_COMMIT = "4dee75d89f3b6f11f70afb116e05b8ec314c9f44"
PREDECESSOR_RECEIPT_RELATIVE = "qa/PUBLICATION_RECEIPT_V0140_V2_CH21_CH22.json"
PREDECESSOR_RECEIPT_PATH = ROOT / PREDECESSOR_RECEIPT_RELATIVE
PREDECESSOR_RECEIPT_BYTES = 4_269
PREDECESSOR_RECEIPT_SHA256 = "4c130cce18421fe27fd56380c53dbc52310ce37023d5d159846681240c872eca"
...
PACKAGE_RELATIVE = "qa/through-chapter23-release-package.json"
PACKAGE_PATH = ROOT / PACKAGE_RELATIVE
TREE_RELATIVE = "qa/THROUGH_CHAPTER23_RELEASE_TREE.tsv"
TREE_PATH = ROOT / TREE_RELATIVE
RECEIPT_RELATIVE = "qa/PUBLICATION_RECEIPT_V0150_V2_THROUGH_CH23.json"
RECEIPT_PATH = ROOT / RECEIPT_RELATIVE
PUBLIC_VALIDATION_RELATIVE = "qa/through-chapter23-public-overlay-validation.json"
PUBLIC_MANIFEST_RELATIVE = "qa/through-chapter23-PUBLIC_SOURCE_TREE_MANIFEST.tsv"
PUBLIC_MAP_RELATIVE = "qa/through-chapter23-PUBLIC_SANITIZATION_MAP.json"
...
REQUIRED_BOUNDARY_EVIDENCE = {
    ...
    PACKAGE_RELATIVE,
    "backend/chapter23-backend-validation.json",
    "qa/chapter23-aggregate-qa.json",
    "qa/through-chapter23-complete-build.json",
    "qa/through-chapter23-pdf-visual-qa.json",
    "qa/through-chapter23-html-build.json",
    "qa/through-chapter23-html-browser-qa.json",
...
def validate_predecessor_receipt() -> dict[str, Any]:
    require(
        ...
        "immutable GitHub v0.14 predecessor receipt identity differs",
    )
    value = load_json(PREDECESSOR_RECEIPT_PATH, "GitHub v0.14 predecessor receipt")
    require(
        value.get("destination") == "github"
        and value.get("version") == "0.14.0-v2-ch21-ch22"
        ...
        "GitHub v0.14 predecessor receipt semantics differ",
    )
    return value


def validate_admission() -> dict[str, Any]:
    value = load_json(ADMISSION_PATH, "final through-Chapter-23 admission receipt")
    ...
    require(
        ...
        "final through-Chapter-23 admission has not been issued",
    )
    ...
    require(
        ...
        "admission version or 239/672 boundary differs",
    )
    ...
    require(
        ...
        and publication.get("zenodo_predecessor_doi") == "10.5281/zenodo.22088384"
    ...
    exact_file(ADMISSION_RECORD_PATH, record, "CP0015 admission record")
...
def validate_package(
    ...
    value = load_json(PACKAGE_PATH, "final through-Chapter-23 package receipt")
...
def release_name() -> str:
    return "Fondasi Teori Ukuran Bahasa Indonesia — 239/672 halaman"
...
def release_body(pdf_name: str) -> str:
    return (
        ...
        "Cakupan kumulatif: Jilid 1 lengkap serta Jilid 2 dari bagian pendahuluan sampai Bab 23 "
        "lengkap, yaitu halaman resmi Jilid 2 nomor 1–137 dan total 239 dari 672 halaman resmi "
        "korpus terpilih. Bab 24 dan bagian sesudahnya belum termasuk; korpus dua jilid belum selesai. "
...
def prepare_manifest(contract: ReleaseContract) -> None:
    ...
    temporary = TREE_PATH.with_name(TREE_PATH.name + ".tmp-v015-through-ch23")
...
def validate_manifest(contract: ReleaseContract) -> None:
    require(TREE_PATH.is_file() and not TREE_PATH.is_symlink(),
            "through-Chapter-23 release-tree manifest is missing")
    bundle = load_privacy_overlay(contract)
    require(TREE_PATH.read_bytes() == public_manifest_payload(contract, bundle),
            "through-Chapter-23 public release-tree manifest is stale")
...
def write_pathspec(paths: tuple[str, ...], directory: Path) -> Path:
    ...
    path = directory / "through-chapter23-paths.nul"
...
def stage_boundary(contract: ReleaseContract) -> tuple[str, str, int]:
    ...
    require(engine.run_git("rev-parse", "HEAD") == PREDECESSOR_MAIN_COMMIT,
            "HEAD is not the exact public v0.14 main")
    ...
    with tempfile.TemporaryDirectory(prefix="o007-through-ch23-git-") as name:
    ...
    require(engine.run_git("write-tree") != engine.run_git("rev-parse", "HEAD^{tree}"),
            "through-Chapter-23 boundary contains no staged change")
    engine.run_git("commit", "--no-verify", "-m", "Admit Indonesian Fremlin checkpoint through Chapter 23")
...
def write_receipt(
    ...
    if RECEIPT_PATH.exists():
        ...
        require(old == current, "existing through-Chapter-23 GitHub receipt differs")
...
def commit_receipt(token: str, boundary: str, payload: bytes) -> str:
    ...
    if head != boundary:
        ...
        with tempfile.TemporaryDirectory(prefix="o007-through-ch23-receipt-") as name:
        ...
        engine.run_git("commit", "--no-verify", "-m", "Record public checkpoint through Chapter 23")
...
if __name__ == "__main__":
    try:
        ...
        print(f"ERROR: fail-closed through-Chapter-23 GitHub publication: {exc}", file=sys.stderr)
        raise SystemExit(1)
