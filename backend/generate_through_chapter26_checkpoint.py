#!/usr/bin/env python3
"""Deterministic cumulative O007 backend through complete Volume-II Chapter 26.

This extends the admitted ``catalog-v1.13`` boundary with the complete
Chapter-26 introduction and Sections 261--266.  The immutable source and
...
PREVIOUS_CATALOG = BACKEND / "catalog-v1.13"
...
PREDECESSOR_ADMISSION = ROOT / "qa/through-chapter25-final-admission.json"
AGGREGATE_RECEIPT = ROOT / "qa/chapter26-aggregate-qa.json"
...
COMPLETE_CHAPTER26_NEW_UNIT_IDS = (
...
INHERITED_SNAPSHOT_SPECS = {
    "O007-RESOURCE-CH25-COMPLETE-SOURCE-CORRECTIONS": {
        "path": SNAPSHOT_DIR / "inherited-v1.13-through-chapter25-source-corrections.csv",
        "bytes": 119_360,
        "sha256": "a7180658db9639d7e10bc2425746c0fc67f0f94979eb16106f928af6628c4e2e",
        "lines": 245,
    },
    "O007-RESOURCE-CH25-COMPLETE-TERMINOLOGY-DECISIONS": {
        "path": SNAPSHOT_DIR / "inherited-v1.13-through-chapter25-terminology-decisions.md",
        "bytes": 26_512,
        "sha256": "431244697e4d08820f4230822001c064a59f92e76ac4df337c4df39754bbda0e",
    },
}
...
class UnitConfig:
    ...
    def receipt_path(self) -> Path:
        return ROOT / f"qa/chapter26/{self.slug}-unit-qa.json"
...
UNITS = (
    UnitConfig(
        "mt26", COMPLETE_CHAPTER26_NEW_UNIT_IDS[0], "Change of Variable in the Integral",
        "Perubahan variabel dalam integral", "288", 1, 4_248,
        "883d2fe0e6bc1013dc001dbc45124d0b349b961b83e01acc554f7efe6a9abdb3",
    ),
    UnitConfig(
        "mt261", COMPLETE_CHAPTER26_NEW_UNIT_IDS[1], "Vitali's theorem in R^r",
        "Teorema Vitali di R^r", "288-295", 8, 32_141,
        "84766c4dd0c601713d2417a59c1db0853d0257e0e5f373a03427ddd42b33887d",
        terms=(
            ("VITALI-THEOREM", "Vitali's theorem", "teorema Vitali", "preferred"),
            ("VITALI-RELATION", "Vitali relation", "relasi Vitali", "preferred"),
            ("FINE-RELATION", "fine relation", "relasi halus", "preferred"),
        ),
    ),
    UnitConfig(
        "mt262", COMPLETE_CHAPTER26_NEW_UNIT_IDS[2], "Lipschitz and differentiable functions",
        "Fungsi Lipschitz dan diferensiabilitas", "296-307", 12, 50_250,
        "8b3f83a867f4984ae6d7455ed746a124dd36c56bc379624c5c8fb8e9afd2dfa9",
        terms=(
            ("LIPSCHITZ-FUNCTION", "Lipschitz function", "fungsi Lipschitz", "preferred"),
            ("DIFFERENTIABLE", "differentiable", "terdiferensialkan", "preferred"),
            ("PARTIAL-DERIVATIVE", "partial derivative", "turunan parsial", "preferred"),
        ),
    ),
    UnitConfig(
        "mt263", COMPLETE_CHAPTER26_NEW_UNIT_IDS[3], "Differentiable transformations in R^r",
        "Transformasi terdiferensialkan di R^r", "308-319", 12, 52_275,
        "debe20ba70cbc7663f5e653d79ca216973d653e818121f565c650300804b7cd5",
        terms=(
            ("DIFFERENTIABLE-TRANSFORMATION", "differentiable transformation", "transformasi terdiferensialkan", "preferred"),
            ("JACOBIAN", "Jacobian", "Jacobian", "preferred"),
            ("CHANGE-OF-VARIABLE", "change of variable", "perubahan variabel", "preferred"),
        ),
    ),
    UnitConfig(
        "mt264", COMPLETE_CHAPTER26_NEW_UNIT_IDS[4], "Hausdorff measures",
        "Ukuran Hausdorff", "320-329", 10, 44_157,
        "c94668c8ac7d23e80377a8a159a0e12c368f21bc2ebbc2cea637fe9afad83464",
        terms=(
            ("HAUSDORFF-OUTER-MEASURE", "Hausdorff outer measure", "ukuran luar Hausdorff", "preferred"),
            ("HAUSDORFF-MEASURE", "Hausdorff measure", "ukuran Hausdorff", "preferred"),
            ("HAUSDORFF-DIMENSION", "Hausdorff dimension", "dimensi Hausdorff", "preferred"),
        ),
    ),
    UnitConfig(
        "mt265", COMPLETE_CHAPTER26_NEW_UNIT_IDS[5], "Surface measures",
        "Ukuran Hausdorff dari permukaan", "330-337", 8, 32_003,
        "09d94421b9537a68427446749f66e0f9fbcdb7543f48ff2e6e3ff3f9b10b44c4",
        terms=(
            ("SURFACE-MEASURE", "surface measure", "ukuran permukaan", "preferred"),
            ("UNIT-SPHERE", "unit sphere", "permukaan bola satuan", "preferred"),
            ("R-SPHERE", "r-sphere", "permukaan bola berdimensi r", "preferred"),
        ),
    ),
    UnitConfig(
        "mt266", COMPLETE_CHAPTER26_NEW_UNIT_IDS[6], "The Brunn--Minkowski inequality",
        "Ketaksamaan Brunn--Minkowski", "338-342", 5, 12_637,
        "ad2c977b007b6ff77bbd0685e91659a9daf69cb71604a589a9cf58d407255e35",
        terms=(
            ("BRUNN-MINKOWSKI", "Brunn--Minkowski inequality", "ketaksamaan Brunn--Minkowski", "preferred"),
            ("CONVEX-SET", "convex set", "himpunan konveks", "preferred"),
        ),
    ),
)
...
def predecessor_admitted_unit_ids(catalog: dict[str, list[dict[str, Any]]]) -> tuple[str, ...]:
    ...
    if not (
        receipt.get("schema") == "o007-fremlin-through-chapter25-final-admission-v1"
        ...
        and boundary.get("version") == "0.18.0-v2-through-ch25"
        and boundary.get("official_pages", {}).get("cumulative_complete") == 389
        and backend.get("catalog") == "backend/catalog-v1.13"
    ...
    if len(admitted_ids) != 36 or len(set(admitted_ids)) != 36 or set(admitted_ids) != set(units):
        raise ValueError("CP0018 Volume-II admitted-unit prefix is not exactly 36 units")
    if not set(PREDECESSOR_PENDING_UNIT_IDS) <= set(admitted_ids):
        raise ValueError("CP0018 complete-Chapter-25 unit IDs are absent")
    return admitted_ids
...
def promote_predecessor_admission_state(catalog: dict[str, list[dict[str, Any]]]) -> tuple[str, ...]:
    ...
    for unit_id in admitted_ids:
        ...
        if record.get("status") == "in_progress" and record.get("target_admitted") is False:
            ...
            raise ValueError(f"unexpected catalog-v1.13 admission state: {unit_id}")
    if tuple(promoted) != PREDECESSOR_PENDING_UNIT_IDS:
        raise ValueError(f"CP0018 admission replay surface differs: {promoted}")
    return admitted_ids
...
def verify_predecessor_exercise_census(catalog: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    if count != 757:
        raise ValueError("catalog-v1.13 cumulative exercise census differs")
    return {"catalog": "backend/catalog-v1.13", "active_exercises": count, "changed": False}


def planned_inherited_snapshots() -> dict[Path, bytes]:
    correction_lines = CORRECTIONS_PATH.read_bytes().splitlines(keepends=True)
    correction_spec = INHERITED_SNAPSHOT_SPECS["O007-RESOURCE-CH25-COMPLETE-SOURCE-CORRECTIONS"]
    if len(correction_lines) < int(correction_spec["lines"]):
        raise SystemExit("current correction ledger cannot recover the catalog-v1.13 prefix")
    snapshots = {
        Path(correction_spec["path"]): b"".join(correction_lines[:int(correction_spec["lines"])]),
        Path(INHERITED_SNAPSHOT_SPECS["O007-RESOURCE-CH25-COMPLETE-TERMINOLOGY-DECISIONS"]["path"]):
            TERMINOLOGY_PATH.read_bytes()[:int(INHERITED_SNAPSHOT_SPECS["O007-RESOURCE-CH25-COMPLETE-TERMINOLOGY-DECISIONS"]["bytes"])],
    }
...
def repair_inherited_resource_paths(resources: list[dict[str, Any]], snapshots: dict[Path, bytes]) -> list[dict[str, Any]]:
    ...
    if seen != set(INHERITED_SNAPSHOT_SPECS):
        raise ValueError("catalog-v1.13 mutable-path repair surface differs")
    return resources
...
def verify_receipt(config: UnitConfig, target_bytes: bytes) -> tuple[dict[str, Any], UnitConfig]:
    ...
    if (
        ...
        raise SystemExit(f"required Chapter 26 receipt does not pass: {config.receipt_path.relative_to(ROOT)}")
...
def official_section_starts() -> dict[str, int]:
    ...
    starts: dict[str, int] = {}
    for anchor in ("261", "262", "263", "264", "265", "266"):
    ...
    expected = {"261": 288, "262": 296, "263": 308, "264": 320, "265": 330, "266": 338}
    if starts != expected or "\\chapintrosection{5.9.03}{288}{}" not in text or "\\chapintrosection{26.8.13}{343}{}" not in text:
        raise SystemExit(f"official complete-Chapter-26 page boundary differs: {starts}")
    return starts
...
def validate_correction_coverage(states: list[engine.UnitState]) -> None:
    expected_counts = {"mt26": 0, "mt261": 3, "mt262": 7, "mt263": 7, "mt264": 16, "mt265": 5, "mt266": 6}
    for state in states:
...
def verify_inputs() -> tuple[list[engine.UnitState], list[dict[str, str]], dict[Path, bytes]]:
    ...
    if not (
        aggregate.get("schema") == "o007-fremlin-chapter26-aggregate-qa-v1"
        ...
        and aggregate.get("census", {}).get("accepted_source_corrections") == 44
    ):
        raise SystemExit("complete Chapter 26 aggregate receipt differs")
...
def build_terms(state: engine.UnitState) -> list[dict[str, Any]]:
    ...
    for record in records:
        record["provenance"]["source_resource_ids"] = ["O007-RESOURCE-CH26-COMPLETE-TERMINOLOGY-DECISIONS"]
    return records
...
def build_corrections(state: engine.UnitState) -> list[dict[str, Any]]:
    ...
    for record in records:
        record["provenance"]["source_resource_ids"] = ["O007-RESOURCE-CH26-COMPLETE-SOURCE-CORRECTIONS"]
    return records
...
def artifact_record(state: engine.UnitState, suffix: str, kind: str, path: Path, verification: str) -> dict[str, Any]:
    ...
    return {
        ...
        "provenance": engine.provenance("complete-chapter26-artifact-witness", "exact bounded complete-Chapter-26 backend input"),
    }
...
def build_event(state: engine.UnitState, datasets: dict[str, list[dict[str, Any]]]) -> list[dict[str, Any]]:
    ...
    counts.update({
        "complete_chapter26_unique_official_pages": 55,
        "complete_chapter26_active_exercises": 84,
        "complete_chapter26_explicit_hints": 18,
        "volume2_frontmatter_through_chapter26_pages": 342,
        "cumulative_completed_official_pages": 444,
    ...
    return [{
        ...
        "id": f"{state.config.unit_id}-QA-CH26-COMPLETE-BACKEND-{EVENT_DATE.replace('-', '')}",
        "unit_id": state.config.unit_id, "event_kind": "complete-chapter26-semantic-backend-replay",
        "event_date": EVENT_DATE, "outcome": "pass",
        "validator": "backend/validate_through_chapter26_checkpoint.py",
        "checks": {
            ...
            "catalog_v1_13_prefix_preserved_except_cp0018_admission_promotion_and_mutable-resource_snapshots": True,
            "backend_checkpoint_not_reader_admission": True,
        },
        "counts": counts,
        "provenance": engine.provenance(
            "deterministic-qa-event",
            f"Complete Chapter 26 cumulative backend checkpoint; {MODEL_TEXT.strip()}.",
            [receipt_resource_id(state.config), "O007-RESOURCE-CH26-COMPLETE-MODEL-PROVENANCE"],
        ),
    }]
...
def build_segments(state: engine.UnitState) -> None:
    ...
    records = state.segments
    counts: dict[str, int] = defaultdict(int)
    for record in records:
        counts[str(record["id"])] += 1
    duplicates = {record_id for record_id, count in counts.items() if count > 1}
    expected = {"O007-FREMLIN-V2-S263-SEG-263G"} if state.config.slug == "mt263" else set()
    if duplicates != expected:
        raise ValueError(f"{state.config.slug} unexpected segment-ID duplication: {sorted(duplicates)}")
    if not duplicates:
        return
    duplicate_id = "O007-FREMLIN-V2-S263-SEG-263G"
    duplicate_records = sorted(
        (record for record in records if str(record["id"]) == duplicate_id),
        key=lambda record: int(record["source_char_start"]),
    )
    if len(duplicate_records) != 2:
        raise ValueError("mt263 expected exactly two adjacent source branches for 263G")
    first, second = duplicate_records
    if (
        int(first["source_char_end"]) != int(second["source_char_start"])
        or int(first["target_char_end"]) != int(second["target_char_start"])
        or first["semantic_anchor"] != second["semantic_anchor"]
        or first["source_anchor"] != second["source_anchor"]
    ):
        raise ValueError("mt263 263G source branches are not an adjacent duplicate-anchor pair")
    merged = engine.make_segment(
        state,
        str(first["semantic_anchor"]),
        str(first["source_anchor"]),
        str(first["anchor_kind"]),
        (int(first["source_char_start"]), int(second["source_char_end"])),
        (int(first["target_char_start"]), int(second["target_char_end"])),
    )
    merged["source_label"] = str(first["source_label"])
    merged["target_label"] = str(first["target_label"])
    active: list[dict[str, Any]] = []
    inserted = False
    for record in records:
        if str(record["id"]) == duplicate_id:
            if not inserted:
                active.append(merged)
                inserted = True
            continue
        active.append(record)
    for order, record in enumerate(active, 1):
        record["order"] = order
    state.segments = active
    state.segment_map = {str(record["semantic_anchor"]): record for record in active}
    state.source_ranges = sorted(
        (int(record["source_char_start"]), int(record["source_char_end"]), str(record["semantic_anchor"]))
        for record in active
    )
    state.target_ranges = sorted(
        (int(record["target_char_start"]), int(record["target_char_end"]), str(record["semantic_anchor"]))
        for record in active
    )
...
def configure_engine() -> None:
    ...
    engine.intro_start = unit_backend.intro_start
...
def resource_record(
    ...
    *, rows: int | None = None, source_ids: list[str] | None = None,
    ...
    record: dict[str, Any] = {
        ...
        "provenance": engine.provenance(
            "complete-chapter26-cumulative-backend-checkpoint",
...
def build_resources(
    ...
    boundary_rows = [row for row in corrections if row.get("unit_id") in COMPLETE_CHAPTER26_NEW_UNIT_IDS]
    if len(boundary_rows) != 44:
        raise ValueError("complete Chapter 26 correction ledger selection is not exactly 44 rows")
    boundary_source_ids = [source_resource_id(state.config) for state in states]
    additions: list[dict[str, Any]] = [
        resource_record(
            "O007-RESOURCE-CH26-COMPLETE-SOURCE-CORRECTIONS", "source-correction-ledger", CORRECTIONS_PATH,
            "exact cumulative source-to-target correction ledger through complete Volume-II Chapter 26",
            f"{len(corrections)} unique rows; every hash-bound Chapter 26 correction matches the receipt exception union",
            rows=len(corrections), source_ids=boundary_source_ids,
        ),
        resource_record(
            "O007-RESOURCE-CH26-COMPLETE-TERMINOLOGY-DECISIONS", "terminology-decision-log", TERMINOLOGY_PATH,
            "current Indonesian terminology decisions through complete Chapter 26",
            "current exact bytes; preferred Chapter 26 mathematical terms explicit",
            source_ids=boundary_source_ids,
        ),
        {
            "schema_version": SCHEMA_VERSION, "record_type": "resource",
            "id": "O007-RESOURCE-CH26-COMPLETE-MODEL-PROVENANCE", "resource_kind": "model-provenance-note",
            ...
            "relation": "explicit model provenance for the complete-Chapter-26 cumulative backend",
            "verification_status": "exact required model identification",
            "provenance": engine.provenance("model-provenance", MODEL_TEXT.strip()),
        },
        resource_record(
            "O007-RESOURCE-CH26-COMPLETE-AGGREGATE-QA", "aggregate-qa-receipt", AGGREGATE_RECEIPT,
            "complete Chapter 26 source-target aggregate with corrected exercise and hint censuses",
            "pass=true; complete Chapter 26 has 84 exercises and 18 hints",
            source_ids=boundary_source_ids,
        ),
    ]
    for state in states:
    ...
    for record in additions:
        if record["id"] in existing:
            raise ValueError(f"new resource collides with catalog-v1.13: {record['id']}")
...
def unit_record(state: engine.UnitState, formulas: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if state.corrections:
        source_ids.append("O007-RESOURCE-CH26-COMPLETE-SOURCE-CORRECTIONS")
    provenance_ids = [
        ...
        "O007-RESOURCE-CH26-COMPLETE-MODEL-PROVENANCE",
        "O007-RESOURCE-CH26-COMPLETE-AGGREGATE-QA",
    ]
    if state.corrections:
        provenance_ids.append("O007-RESOURCE-CH26-COMPLETE-SOURCE-CORRECTIONS")
    return {
        ...
        "provenance": engine.provenance(
            "source-derived-complete-chapter26-backend-checkpoint",
...
def build_catalog(
    ...
    volume2 = next(record for record in catalog["volumes"] if record["id"] == VOLUME_ID)
    volume2.update({
        "status": "in_progress", "admitted_source_page_span": "1-342",
        "admitted_unique_source_page_count": 342,
        "admitted_unit_ids": previous_ids + list(COMPLETE_CHAPTER26_NEW_UNIT_IDS),
        "provenance": engine.provenance(
            "volume2-frontmatter-through-complete-chapter26-backend-checkpoint",
            f"Volume-II front matter through complete Chapter 26 covers pages 1-342 as complete translated units; corpus progress is 444 of 672 official pages; reader/build admission remains external; {MODEL_TEXT.strip()}.",
            [
                "O007-RESOURCE-MT2-ARCHIVE", "O007-RESOURCE-SOURCE-MANIFEST",
                "O007-RESOURCE-MT02-OFFICIAL-CONTENTS", "O007-RESOURCE-CH26-COMPLETE-SOURCE-CORRECTIONS",
                "O007-RESOURCE-CH26-COMPLETE-TERMINOLOGY-DECISIONS", "O007-RESOURCE-CH26-COMPLETE-MODEL-PROVENANCE",
                "O007-RESOURCE-CH26-COMPLETE-AGGREGATE-QA",
    ...
    if not set(previous_ids + list(COMPLETE_CHAPTER26_NEW_UNIT_IDS)) <= ids:
        raise ValueError("cumulative Volume-II unit closure is incomplete")
    if sum(len(record.get("exercise_ids", [])) for record in catalog["units"]) != 841:
        raise ValueError("corrected cumulative active-exercise census is not 841")
    if sum(int(record.get("explicit_hint_count", 0)) for record in catalog["units"]) != 196:
        raise ValueError("cumulative explicit-hint census is not 196")
...
def main() -> int:
    ...
    print(json.dumps({
        ...
        "boundary_label": "COMPLETE CHAPTER 26", "chapter26_pages": "288-342",
        "chapter26_unique_official_page_count": 55, "chapter26_complete": True,
        "volume2_contiguous_translated_pages": "1-342",
        "volume2_contiguous_translated_page_count": 342,
        "cumulative_completed_official_pages": 444,
        "complete_chapter26_active_exercises": 84,
        "cumulative_active_exercises": 841,
        "complete_chapter26_explicit_hints": 18,
        "cumulative_explicit_hints": 196,
        "selected_corpus_official_pages": 672,
        "inherited_admitted_unit_count": 36,
