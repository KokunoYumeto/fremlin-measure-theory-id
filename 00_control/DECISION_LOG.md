...
## 2026-08-21 — D001: Preserve the reconstruction record

`TASK_RECONSTRUCTION_20260821.md` remains immutable historical evidence of the
earlier missing-selection blocker. It is not deleted or rewritten merely
because the blocker is now resolved.

## 2026-08-21 — D002: Accept the root corpus selection

The root handoff selects D. H. Fremlin, *Measure Theory*, complete Volumes 1–2
for the existing O007 task. This replaces the reconstruction-only pause with
production. No new/replacement task is created.

## 2026-08-21 — D003: Fix the corpus boundary

Translate all source units in Volumes 1–2. Exclude Volumes 3–5 and do not merge
Cabral, Erdman, Random, RFA, Axler, Stacks, or other comparator/donor material
into the assigned corpus. Curricular route labels may classify content but may
not delete it.

## 2026-08-21 — D004: Freeze authority, modernize outside it

The two official archives, expanded source trees, manifests, and exact DSL are
read-only authority. Build, semantic, accessibility, and typographic
modernizations belong in derived source/build layers with explicit deltas.

## 2026-08-21 — D005: Use locale-neutral stable IDs

The canonical backend record streams use stable `O007-FREMLIN-*` IDs that do
not encode the target locale. `id-ID` is a record attribute. JSONL is canonical
and CSV is a deterministic projection for interchange.

## 2026-08-21 — D006: Keep provenance components separate

Fremlin-derived translation stays under the DSL. Bundled third-party support
files retain their own notices. New mastery/backend material is explicitly
original and separately licensed; it supplements rather than substitutes for
source exercises and hints.

## 2026-08-21 — D007: Translation dominates the workflow

The first production cursor is the complete `mt111.tex` section,
`O007-FREMLIN-V1-S111`. Freeze/build/QA work supports unit admission but must
not become a planning or audit loop. Only evidence-backed completion advances
the cursor.

## 2026-08-21 — D008: Bound Git and upstream communication

Use only narrow corpus-local Git operations and push substantial verified
boundaries under standing authorization. Do not contact upstream during
production. After full corpus completion, at most one separately authorized,
concise, deduplicated, high-confidence report may be sent with the required
signature.

## 2026-08-21 — D009: Bind the corrected source census

The live root handoff supersedes its earlier mechanical census. The authority
for planning is 1,094 unique active exercise/problem IDs: 198 in Volume 1 and
896 in Volume 2. Explicit active hint macros remain 276: 55 and 221. Section
111 remains eleven exercises and three explicit hints. Do not propagate the
superseded pre-correction figures.

## 2026-08-21 — D010: Admit complete Section 111 and advance in source order

The complete `mt111.tex` translation is admitted as
`O007-FREMLIN-V1-S111`. Structural replay preserves every formula, source
anchor, exercise, hint, prooflet, and reference relation. The PDF, offline HTML,
machine backend, editable source, licenses, and deterministic package pass the
...
## 2026-08-23 — D020: Correct the production cadence and complete the Chapter 13 translation batch

The active objective was replaced with a comprehensive 3,999-character goal
that preserves the complete 672-page corpus and the admitted S132 boundary but
ends the per-section build/publication loop. Its durable mirror is
`00_control/CURRENT_GOAL_AND_WORKFLOW.md` (4,232 bytes; SHA-256
`6f3e0fd84cc751fa3acf4316b0e64c9109a0452ddf971110eefea741f087be77`).
Translation now proceeds in contiguous chapter or major-section batches with
light per-section checks; backend consolidation, cumulative readers, visual
QA, packaging, admission, and publication occur once per substantial boundary.

The first corrected-cadence translation boundary is now complete but not yet
admitted: the missing Chapter 13 introduction plus complete sections 133–136.
All five target members pass exact structural and normalized-mathematics QA.
Their SHA-256 values are `8eaa400c1ee8ec70ff08dcd3c6ca9029584c0b8113968aa6bab546eff564994a`
(mt13), `b965f3a8673f161ba2b372d698754f27545708f62fa7e52765f03a08d7d4605d`
(mt133), `18b99df4efc21ea4e1c6b31e561021fa8d5fac730772a3acad96f2dc5923c367`
(mt134), `8e4eeb3d864f81fe6b27be59ee145d0bb5ca3ad5e01e279f951c922ca7ec965a`
(mt135), and `aadd0bdbb660d8843ed83189eb0f0362f2b5aed22b42544f4deac57f382eec92`
(mt136). The consolidated semantic receipt is
`qa/mt133-mt136-semantic-review.json` (3,809 bytes; SHA-256
`907b78b41fa85cd7d1b784646ed0adb372f60cdc1003ac85e59e065b9c50a9b3`).

High-confidence authority errors found during review are explicit in
`00_control/SOURCE_CORRECTIONS.csv` through `O007-CORR-0026`, including the
wrong primed family and undefined measure symbol in mt134. The working batch
has frozen official coverage through page 90, so final admission will raise
the cumulative union from pages 10–62 to pages 10–90 (81 of 672) only after
the consolidated backend, PDF/offline HTML, visual/browser evidence, and
deterministic package all pass. Zenodo S132 will not be retried separately;
the next admitted cumulative version will extend the existing concept.

## 2026-08-24 — D021: Admit the complete Chapter 13 boundary and correct the next cursor

The complete Chapter 13 closure batch is admitted: missing introduction
`mt13.tex` plus Sections 133–136, with prior S111–S132 material preserved.
The cumulative official union is pages 10–90, 81 of the selected 672 pages.
Exact evidence is `00_control/CP0011_MT136_ADMISSION.md`. The admitted reader
is a 93-page A4 PDF, 704,002 bytes / SHA-256
`9afb9bca0bf6e1116ac4aae673392478a191198c9a3f75dd591493ac3e7d3adf`.
Final package and ZIP identities are held in the external admission record and
final receipts rather than this packaged decision snapshot, avoiding a
self-referential checksum.
All-page PDF inspection, all-route desktop/mobile browser replay, 3,461-record
backend validation, and two-pass final package reproduction pass. The final
build and reader receipts are admitted and publication-ready.

A direct replay of `authority/fremlin/source/mt1.2011/vol1.tex` proves that the
inherited `mt137.tex` next-cursor claim was false: no such authority member
exists. Source order after `mt136.tex` is `mt1a.tex`, `mt1a1.tex`, `mt1a2.tex`,
`mt1a3.tex`, `mt1conc.tex`, `mt1r.tex`, and then the shared index driver
`mti.tex`. The next production cursor is therefore the Volume 1 appendix
introduction `mt1a.tex`. Existing Indonesian drafts for the six Volume 1-local
...
`6e865eecf0c946ddbbf9a5202b812304638a28ef`). Anonymous readback matched the
translation, unit receipt, anomaly receipt, semantic receipt, Chapter 25
freeze, and durable reader bytes exactly. Sanitized evidence is
