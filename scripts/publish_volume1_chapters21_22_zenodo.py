#!/usr/bin/env python3
"""Publish the admitted O007 186/672 Chapters 21--22 checkpoint to Zenodo.

Only one new version of the established concept 10.5281/zenodo.22059798 is
permitted, and only from the exact public v0.13.0 Chapter 22 predecessor.  The
local Chapter 21--22 admission/package gate runs before token access or any
request.  The public transaction remains reader-first with exactly three
assets and concludes with anonymous byte/hash readback of every asset.
"""
...
import publish_volume1_chapters21_22_github as gate
...
PREDECESSOR_RECORD_ID = 22_086_976
PREDECESSOR_DOI = "10.5281/zenodo.22086976"
PREDECESSOR_VERSION = "0.13.0-v2-ch22"
...
PREDECESSOR_RECEIPT_RELATIVE = "qa/ZENODO_PUBLICATION_RECEIPT_V0130_V2_CH22.json"
PREDECESSOR_RECEIPT_PATH = ROOT / PREDECESSOR_RECEIPT_RELATIVE
PREDECESSOR_RECEIPT_BYTES = 3_619
PREDECESSOR_RECEIPT_SHA256 = "eeab39a61f330dcbbce1ddaad3a5b7c41c79bf61ca64d5eec26f431b410590ff"
RECEIPT_RELATIVE = "qa/ZENODO_PUBLICATION_RECEIPT_V0140_V2_CH21_CH22.json"
...
def metadata() -> dict[str, object]:
    description = (
        "<p><strong>Prarilis parsial: 186 dari 672 halaman resmi korpus "
        "terpilih.</strong> Deposit ini memuat adaptasi Bahasa Indonesia "
        "lengkap dari D. H. Fremlin, <em>Measure Theory</em>, Volume 1: "
        "<em>The Irreducible Minimum</em>, serta Volume 2 Bab 21: "
        "<em>Taxonomy of Measure Spaces</em> dan Bab 22: "
        "<em>The Fundamental Theorem of Calculus</em>.</p>"
        "<p>Cakupan Jilid 2 pada checkpoint ini adalah halaman resmi 12–95. "
        "Bagian pendahuluan Jilid 2 pada halaman 1–11 belum termasuk; karena "
        "itu korpus dua jilid belum selesai. PDF pembaca adalah berkas "
        "pertama, diikuti ZIP deterministik yang dapat dilanjutkan dan "
        "berkas checksum SHA-256.</p>"
    ...
    return {
        ...
        "notes": (
            "Prarilis parsial 186/672 halaman resmi: Jilid 1 lengkap dan "
            "Jilid 2 Bab 21–22 lengkap, mencakup halaman resmi Jilid 2 nomor "
            "12–95. Bagian pendahuluan Jilid 2 pada halaman 1–11 belum "
            "termasuk. MathJax 3.2.2 tetap komponen terpisah berlisensi "
            "Apache-2.0."
        ),
        "related_identifiers": [
            {
...
def predecessor_receipt() -> dict[str, Any]:
    require(
        ...
        "immutable Zenodo v0.13.0 predecessor receipt identity differs",
    )
    try:
        value = json.loads(PREDECESSOR_RECEIPT_PATH.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise PublicationError("Zenodo v0.13.0 predecessor receipt is unreadable") from exc
    require(isinstance(value, dict), "Zenodo v0.13.0 predecessor receipt is malformed")
    ...
    require(
        ...
        "Zenodo v0.13.0 predecessor receipt semantics differ",
    )
...
def choose_candidate(token: str) -> tuple[dict[str, Any], str, bool]:
    ...
    for key in ("latest", "latest_draft"):
        ...
        if isinstance(url, str):
            ...
            require(isinstance(value, dict) and isinstance(value.get("id"), int), f"Zenodo predecessor {key} link is malformed")
    ...
    require(len(exact_public) <= 1, "multiple exact public Chapters 21--22 versions exist")
    if exact_public:
        require(not drafts, "exact public Chapters 21--22 version coexists with a concept draft")
        return exact_public[0], "already_published_exact_chapters21_22", True
    ...
    require(isinstance(draft, dict) and isinstance(draft.get("id"), int), "Zenodo newversion draft is absent")
    assert isinstance(draft, dict)
    engine.assert_concept(draft, "Zenodo newversion draft")
    require(transport.deposition_state(draft, "Zenodo newversion draft") == "draft", "Zenodo newversion did not create one draft")
    return draft, "one_newversion_from_22086976", False
...
def write_receipt(
    ...
    value = {
        ...
        "record": {
            ...
            "partial_coverage": "186/672 official pages; Volume II pages 12-95 included; pages 1-11 absent",
        },
        "asset_order": [binding.name for binding in contract.assets],
        "assets": assets,
        "transaction_route": "existing_concept_newversion_from_22086976",
        "runtime_route": route,
        "verification": {
    ...
    if RECEIPT_PATH.exists():
        try:
            ...
            raise PublicationError("existing Chapters 21--22 Zenodo receipt is unreadable") from exc
        ...
        require(old == comparable, "existing Chapters 21--22 Zenodo receipt differs")
...
def main() -> int:
    ...
    parser.add_argument("--preflight", action="store_true", help="validate all local contracts without credentials or network")
...
if __name__ == "__main__":
    try:
        ...
        print(f"ERROR: fail-closed Chapters 21--22 Zenodo publication: {exc}", file=sys.stderr)
        raise SystemExit(1)
