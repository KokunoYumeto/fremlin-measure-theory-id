#!/usr/bin/env python3
"""Publish the admitted O007 509/672 checkpoint to its existing GitHub line.
...
complete Chapter 27 boundary.  ``--preflight`` performs no Git command,
...
PREDECESSOR_TAG = "v0.19.0-v2-through-ch26"
PREDECESSOR_BOUNDARY_COMMIT = "4c8c0bbdda7b27c627afa2ab98f5b41515692fac"
PREDECESSOR_RECEIPT_COMMIT = "c66445e4dbe745bb448ddd2d81be1df7dbe24490"
# The v0.19 receipt commit was followed by the already-public D050 metadata
# commit.  That exact commit is the current receipt-bound main parent for v0.20.
PREDECESSOR_MAIN_COMMIT = "1f6de4f1e4b9f63c5435be8d2c87f748c6a0077b"
PREDECESSOR_RECEIPT_RELATIVE = "qa/PUBLICATION_RECEIPT_V0190_V2_THROUGH_CH26.json"
PREDECESSOR_RECEIPT_PATH = ROOT / PREDECESSOR_RECEIPT_RELATIVE
PREDECESSOR_RECEIPT_BYTES = 5_057
...
PACKAGE_RELATIVE = "qa/through-chapter27-release-package.json"
PACKAGE_PATH = ROOT / PACKAGE_RELATIVE
TREE_RELATIVE = "qa/THROUGH_CHAPTER27_RELEASE_TREE.tsv"
TREE_PATH = ROOT / TREE_RELATIVE
RECEIPT_RELATIVE = "qa/PUBLICATION_RECEIPT_V0200_V2_THROUGH_CH27.json"
RECEIPT_PATH = ROOT / RECEIPT_RELATIVE
PUBLIC_VALIDATION_RELATIVE = "qa/through-chapter27-public-overlay-validation.json"
PUBLIC_MANIFEST_RELATIVE = "qa/through-chapter27-PUBLIC_SOURCE_TREE_MANIFEST.tsv"
PUBLIC_MAP_RELATIVE = "qa/through-chapter27-PUBLIC_SANITIZATION_MAP.json"
...
BACKEND_VALIDATION_RELATIVE = "backend/through-chapter27-backend-validation.json"
...
PDF_RELATIVE = (
    "output/pdf/fondasi-teori-ukuran-jilid-1-dan-jilid-2-hingga-akhir-bab-27-id.pdf"
)
EXPECTED_PDF_BYTES = 3_939_039
EXPECTED_PDF_SHA256 = "48fda0dae726802208056bd3e8a4e3f4713ea45b498c4fe891710f7e2f349466"
EXPECTED_PDF_REFLOW_PAGES = 545
...
EXPECTED_PUBLIC_ASSET_NAMES = (
    "00_READ_FIRST_FONDASI_TEORI_UKURAN_V1_DAN_V2_HINGGA_BAB_27.pdf",
    "fondasi-teori-ukuran-v1-dan-v2-hingga-bab27-id-v0.20.0.zip",
    "SHA256SUMS-v0.20.0-v2-through-ch27.txt",
)
...
REQUIRED_BOUNDARY_EVIDENCE = {
    ...
    AGGREGATE_RELATIVE,
    "qa/through-chapter27-complete-build.json",
    "qa/through-chapter27-pdf-visual-qa.json",
    HTML_BUILD_RELATIVE,
...
def validate_admission_file_bindings(value: dict[str, Any]) -> None:
    ...
    require(pdf.get("pages") == EXPECTED_PDF_REFLOW_PAGES,
            "admitted cumulative PDF reflow-page count differs")
...
def validate_predecessor_receipt() -> dict[str, Any]:
    require(
        ...
        "immutable GitHub v0.19 predecessor receipt identity differs",
    )
    value = load_json(PREDECESSOR_RECEIPT_PATH, "GitHub v0.19 predecessor receipt")
    scope = value.get("scope", {})
    require(
        value.get("destination") == "github"
        and value.get("version") == "0.19.0-v2-through-ch26"
        ...
        and scope.get("official_pages_complete") == 444
        and scope.get("volume2_last_included_page") == 342
        and scope.get("volume2_chapter26_complete") is True
        and value.get("release", {}).get("prerelease") is True,
        "GitHub v0.19 predecessor receipt semantics differ",
    )
    return value


def validate_admission() -> dict[str, Any]:
    value = load_json(ADMISSION_PATH, "final through-Chapter-27 admission receipt")
    ...
    require(
        ...
        "final through-Chapter-27 admission has not been issued",
    )
    ...
    require(
        ...
        "admission version or 509/672 boundary differs",
    )
    ...
    require(
        ...
        and publication.get("zenodo_predecessor_doi") == "10.5281/zenodo.22161046"
    ...
    exact_file(ADMISSION_RECORD_PATH, record, "CP0020 admission record")
...
def validate_package(
    ...
    value = load_json(PACKAGE_PATH, "final through-Chapter-27 package receipt")
    ...
    require(tuple(binding.name for binding in assets) == EXPECTED_PUBLIC_ASSET_NAMES,
            "package release-asset names differ from the Chapter-27 v0.20.0 contract")
    reader = assets[0]
    require(
    ...
    exact_file(backend_path, backend_row, "package-bound Chapter-27 backend validation")
    backend = load_json(backend_path, "package-bound Chapter-27 backend validation")
    require(
        backend.get("schema") == "o007-through-chapter27-backend-validation-v1"
        and backend.get("status") == "pass" and backend.get("pass") is True
        and backend.get("catalog_counts", {}).get("resources") == 292
        and backend.get("catalog_counts", {}).get("units") == 77
        and backend.get("schema_valid_record_count") == 6_464
        and backend.get("output_inventory", {}).get("file_count") == 217
        and backend.get("output_inventory", {}).get("total_bytes") == 10_818_056
        and isinstance(backend.get("manifests"), dict)
        and "catalog-v1.15" in backend["manifests"],
        "package backend is not the passing catalog-v1.15 checkpoint",
    )
    ...
    content_relative, content = exact_relative_binding(
        value.get("content_admission"), "package-bound CP0020 admission record",
    )
    require(
        ...
        and all(content.get(key) == admission["content_admission"].get(key)
                for key in ("path", "bytes", "sha256")),
        "package and admission CP0020 bindings disagree",
    )
    aggregate_relative, aggregate = exact_relative_binding(
    ...
    require(isinstance(zenodo, dict) and zenodo.get("doi") == "10.5281/zenodo.22161046",
    ...
    require(
        zenodo_relative == "qa/ZENODO_PUBLICATION_RECEIPT_V0190_V2_THROUGH_CH26.json"
        and (zenodo_receipt.get("bytes"), zenodo_receipt.get("sha256"))
        == (4_571, "412a8c76c3f08cb26d41c18186f1d257723b1a342e2dc6205881a17d56706f95"),
...
def release_name() -> str:
    return "Fondasi Teori Ukuran Bahasa Indonesia — 509/672 halaman"
...
def release_body(pdf_name: str) -> str:
    return (
        ...
        "Cakupan kumulatif: Jilid 1 lengkap serta Jilid 2 dari bagian pendahuluan sampai akhir Bab 27, "
        "yaitu halaman resmi Jilid 2 nomor 1–407 dan total 509 dari 672 halaman resmi korpus terpilih. "
        "Bab 27 lengkap; Bab 28, lampiran, referensi, dan indeks belum termasuk. PDF pembaca ditempatkan pertama, "
...
def stage_boundary(contract: ReleaseContract) -> tuple[str, str, int]:
    ...
    require(engine.run_git("rev-parse", "HEAD") == PREDECESSOR_MAIN_COMMIT,
            "HEAD is not the exact receipt-bound pre-v0.20 main commit")
    ...
    with tempfile.TemporaryDirectory(prefix="o007-through-chapter27-git-") as name:
    ...
    require(engine.run_git("write-tree") != engine.run_git("rev-parse", "HEAD^{tree}"),
            "through-Chapter-27 boundary contains no staged change")
    engine.run_git("commit", "--no-verify", "-m",
                   "Admit Indonesian Fremlin checkpoint through complete Chapter 27")
...
def write_receipt(
    ...
    value = {
        ...
        "verification": {
            ...
            "catalog_v1_15_package_binding_replayed": True,
            "canonical_private_evidence_files_mutated": False,
            "volume2_contiguous_pages_1_407_disclosed": True,
    ...
    if RECEIPT_PATH.exists():
        ...
        require(old == current, "existing through-Chapter-27 GitHub receipt differs")
        return RECEIPT_PATH.read_bytes()
    temporary = RECEIPT_PATH.with_name(RECEIPT_PATH.name + ".tmp-v020-through-chapter27")
...
def commit_receipt(token: str, boundary: str, payload: bytes) -> str:
    ...
    if head != boundary:
        ...
        with tempfile.TemporaryDirectory(prefix="o007-through-chapter27-receipt-") as name:
        ...
        engine.run_git("commit", "--no-verify", "-m",
                       "Record public checkpoint through complete Chapter 27")
...
def _configure_driver() -> None:
    values = {
        ...
        "EXPECTED_PDF_REFLOW_PAGES": EXPECTED_PDF_REFLOW_PAGES,
...
if __name__ == "__main__":
    try:
        ...
        print(f"ERROR: fail-closed through-Chapter-27 GitHub publication: {exc}",
              file=sys.stderr)
        raise SystemExit(1)
