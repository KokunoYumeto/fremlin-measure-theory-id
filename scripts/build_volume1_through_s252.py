#!/usr/bin/env python3
"""Build the cumulative Indonesian reader through Volume II Section 252.

The implementation reuses the already admitted Chapter 24 build primitives,
but closes over a new exact driver and the three passing Chapter 25 unit
receipts (introduction, 251, and 252).  It builds the complete localized
Volume II surface twice from the frozen mt2.2016 archive, verifies deterministic
output and official folio anchors, then appends only the newly exposed Volume II
suffix to the admitted 327-page predecessor reader.  Thus every predecessor
page is structurally reused, while the downstream raster QA proves those pages
pixel-exact and inspects every appended surface.
"""
...
SOURCE_DATE_EPOCH = "1787702400"  # 2026-08-26T00:00:00Z
PRODUCTION_MODEL = "OpenAI Codex gpt-5.6-sol, Ultra"

MASTER = "vol2-through-s252-id.tex"
MASTER_BYTES = 1_574
MASTER_SHA256 = "396e01d0880722a2d4bd9a9fa90b2dd691f4ab814fa198ed3087ff93819f3938"
VOLUME2_PDF_NAME = "fondasi-teori-ukuran-jilid-2-hingga-bagian-252-id.pdf"
OUTPUT_NAME = "fondasi-teori-ukuran-jilid-1-dan-jilid-2-hingga-bagian-252-id.pdf"
BUILD_NAMES = ("volume2-through-s252-id-pass-a", "volume2-through-s252-id-pass-b")

PRIOR_PDF_NAME = "fondasi-teori-ukuran-jilid-1-dan-jilid-2-hingga-bab-24-id.pdf"
PRIOR_PDF_BYTES = 2_176_212
PRIOR_PDF_SHA256 = "7c03fa2e673f0a3e617401e8894c6d2e56956d39cec4c14e60006f4ff26ec446"
PRIOR_VISUAL_RECEIPT_NAME = "through-chapter24-pdf-visual-qa.json"
PRIOR_VISUAL_RECEIPT_BYTES = 242_482
PRIOR_VISUAL_RECEIPT_SHA256 = "69603741629ff53431291d1065d66eeea93c2f647d9d260e401ce13a1b646444"
PRIOR_PHYSICAL_PAGES = 327
...
OFFICIAL_VOLUME2_LAST_PAGE = 236
OFFICIAL_SELECTED_PAGES = 338
OFFICIAL_CHAPTER25_FIRST_PAGE = 204
OFFICIAL_CHAPTER25_LAST_PAGE = 236
...
COMBINED_METADATA = {
    "/Title": "Fondasi Teori Ukuran - Jilid 1 lengkap dan Jilid 2 hingga Bagian 252",
    "/Author": (
    ...
    "/Subject": (
        ...
        "(102 halaman resmi) dan Jilid 2 halaman resmi 1-236; Bab 25 "
        "parsial hingga Bagian 252"
    ),
    "/Keywords": (
        "teori ukuran, ukuran hasil kali, teorema Fubini, teorema Tonelli, "
        "integral berulang, id-ID, O007, Design Science License"
    ),
    ...
    "/Volume2OfficialPages": "1-236",
    "/Chapter21OfficialPages": "12-54",
    "/Chapter22OfficialPages": "55-95",
    "/Chapter23OfficialPages": "96-137",
    "/Chapter24OfficialPages": "138-203",
    "/Chapter25OfficialPages": "204-236 (parsial, sampai Bagian 252)",
    "/CorpusOfficialPages": "338/672",
    "/CoverageStatus": (
        "Jilid 1 lengkap; Jilid 2 halaman resmi 1-236; "
        "Bab 25 parsial hingga Bagian 252"
    ),
...
def localized_identities() -> tuple[dict[str, tuple[int, str]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    require(len(identities) == 36, "localized overlay inventory differs")
    return identities, chapter24, chapter25


def verify_driver_contract(path: Path) -> None:
    base.assert_file(path, MASTER_BYTES, MASTER_SHA256, "through-S252 driver")
...
def verify_prior_receipt(path: Path) -> dict[str, Any]:
    base.assert_file(path, PRIOR_VISUAL_RECEIPT_BYTES, PRIOR_VISUAL_RECEIPT_SHA256, "admitted Chapter 24 visual receipt")
    payload = json.loads(path.read_text(encoding="utf-8", errors="strict"))
    pages = payload.get("all_page_raster_audit", {}).get("pages", [])
    require(
        payload.get("schema") == "o007-volume1-plus-volume2-through-chapter24-pdf-visual-qa-v1"
        ...
        "admitted Chapter 24 visual receipt is not reusable",
    )
    return record(path)
...
def snapshot_inputs() -> dict[str, Any]:
    ...
    base.assert_file(prior_pdf, PRIOR_PDF_BYTES, PRIOR_PDF_SHA256, "admitted Chapter 24 reader")
...
def build_once(name: str, env: dict[str, str]) -> dict[str, Any]:
    ...
    ranges = {
        ...
        "chapter25_partial_through_s252": (folios[chapter25_offset:], 204, 236),
    }
    ...
    return {
        ...
        "printed_folios": {
            ...
            **range_records,
            "official_range_1_236_present": True,
        },
        "tex": {
...
def verify_rebuilt_volume2_prefix(prior: Path, rebuilt_volume2: Path) -> dict[str, Any]:
    ...
    # The prior standalone driver ended with \end; the extended driver closes
    # the same terminal Chapter 24 page with \varfrnewpage before resetting the
    # folio to 204.  That changes only the disposable rebuilt terminal page's
    # content stream/placement.  It is never copied: the admitted predecessor
    # page is retained instead, and appending starts at physical page 218.
    require(expected[:-1] == actual[:-1], "rebuilt Volume II prefix differs before its discarded terminal page")
    terminal_expected = expected[-1]
    terminal_actual = actual[-1]
    for key in ("text_sha256", "media_box", "crop_box", "rotation"):
        require(terminal_expected[key] == terminal_actual[key], f"discarded terminal Chapter 24 page {key} differs")
    require(terminal_expected["content_sha256"] != terminal_actual["content_sha256"], "expected terminal driver-transition delta is absent")
    encoded = json.dumps(actual[:-1], ensure_ascii=False, separators=(",", ":"), sort_keys=True).encode("utf-8")
    return {
        "page_count": PRIOR_VOLUME2_PHYSICAL_PAGES,
        "byte_structurally_exact_prefix_pages": PRIOR_VOLUME2_PHYSICAL_PAGES - 1,
        "discarded_rebuilt_terminal_page": PRIOR_VOLUME2_PHYSICAL_PAGES,
        "discarded_terminal_page_text_and_geometry_exact": True,
        "copied_suffix_starts_at_rebuilt_physical_page": PRIOR_VOLUME2_PHYSICAL_PAGES + 1,
        "exact_prefix_fingerprint_sha256": hashlib.sha256(encoded).hexdigest(),
    }
...
def write_combined(prior: Path, rebuilt_volume2: Path, output: Path) -> str:
    ...
    writer.append(
        ...
        outline_item="Jilid 2 - Bab 25 hingga Bagian 252",
...
def combine_twice(prior: Path, rebuilt_volume2: Path) -> tuple[Path, dict[str, Any]]:
    outputs = (
        ROOT / "build" / "volume1-through-s252-pypdf-pass-a.pdf",
        ROOT / "build" / "volume1-through-s252-pypdf-pass-b.pdf",
    )
    versions: list[str] = []
...
def verify_prior_prefix(prior: Path, combined: Path) -> dict[str, Any]:
    ...
    require(expected == actual, "combined reader does not preserve all 327 predecessor pages")
...
def main() -> int:
    ...
    combined_info = base.pdfinfo(
        ...
        ROOT / "build" / "volume1-through-s252-combined-pdfinfo.stdout.log",
        env,
    )
    appended_pages = second["pdf"]["pages"] - PRIOR_VOLUME2_PHYSICAL_PAGES
    require(appended_pages > 0, "no new Section 25 surfaces were appended")
    ...
    receipt: dict[str, Any] = {
        "schema": "o007-fremlin-volume1-plus-volume2-through-s252-pdf-build-v1",
        "pass": True,
        ...
        "scope": {
            ...
            "excluded_at_this_boundary": ["Volume II Sections 253 onward, Chapters 26-28, and appendices"],
            "volume2_official_pages_1_236_complete": True,
            "chapter25_status": "partial through complete Section 252",
            "corpus_official_pages": "338/672",
        ...
        "pagination": {
            "official_source_accounting": {
                ...
                "chapter25_pages_at_boundary": 33,
            ...
            "volume2_through_s252_printed_folios": second["printed_folios"],
        },
        ...
        "reproducibility": {
            ...
            "predecessor_327_page_prefix_preservation": prior_prefix,
        },
        "environment": {
        ...
        "checks": {
            "all_three_chapter25_unit_receipts_present_and_pass": True,
            ...
            "exact_through_s252_driver_and_folio_anchors": True,
            "official_volume2_folio_range_1_236_present": True,
            "official_accounting_102_plus_236_equals_338": True,
            ...
            "rebuilt_volume2_first_216_predecessor_pages_exact_and_terminal_transition_page_discarded": True,
            "all_327_predecessor_content_text_geometry_fingerprints_exact": True,
        ...
        "next_gate": "Render all pages, replay the 327-page prior raster prefix, inspect every appended contact sheet, and finalize the visual receipt.",
    }
    receipt_path = ROOT / "qa" / "through-s252-complete-build.json"
