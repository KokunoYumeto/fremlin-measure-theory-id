#!/usr/bin/env python3
"""Deterministically build and package cumulative O007 sections 111-123.

The two passes rebuild the same bounded staging and distribution paths.  The
builder preserves every published S111-through-S122 release artifact, retains
the frozen PostScript figure authority and admitted PNG derivatives, applies
only the admitted S115 staging-copy PDF reflow, and adds the complete S123
semantic reader.  No distribution path is touched until the S123 backend
...
TARGET_HASHES = {
    ...
    "122": "898783f7dc36acb07721f891525c215a53788ae7974dacd27590f51449b847f7",
    "123": "0dbed47213a2ba03ff3f55226aa2f9e141742234313ad45762742df9542fc985",
}
AUTHORITY_HASHES = {
...
# source-ordered cumulative Indonesian text produces 55 deterministic A4 pages.
PDF_PAGES = 55
...
def deterministic_zip(package: Path, destination: Path) -> None:
    """Write an exact S123 cumulative ZIP with the frozen source-date timestamp."""
...
ADMITTED_CANDIDATE_QA_INPUTS = {
    ...
    "mt123-reader-qa-candidate.json",
    "mt123-pdf-visual-qa.json",
    "mt123-browser-visual-qa.json",
}


def qa_member(relative: Path, admitted: bool = False) -> bool:
    return (
...
def verify_mt123_inputs(lane: Path) -> None:
...
def backend_admission_phase(
    ...
    if not isinstance(catalog, dict) or not isinstance(artifacts_and_event, dict):
        raise RuntimeError(f"S123 backend receipt lacks phase records: {receipt_path}")
    ...
    if inventory_page_span != "10-56" or inventory_page_count != 47:
        raise RuntimeError(f"S123 backend inventory page union differs: {receipt_path}")

    admission_evidence = payload.get("admission_evidence")
    if not isinstance(admission_evidence, dict):
        raise RuntimeError(f"S123 backend receipt lacks admission evidence state: {receipt_path}")
    ...
    if event_established is not False:
        raise RuntimeError(
            f"S123 backend event improperly establishes reader admission: {receipt_path}"
        )
    if not isinstance(catalog_claimed, bool) or not isinstance(
        ...
        raise RuntimeError(
            f"S123 catalog/evidence reader-admission states disagree: {receipt_path}"
        )
    ...
    if target_admitted is False and reader_admitted is False:
        phase = "pending"
        admitted_page_span = "10-52"
        admitted_page_count = 43
        if status not in (None, "in_progress", "pending"):
            raise RuntimeError(f"S123 pending backend status differs: {receipt_path}")
        ...
        if status not in (None, "admitted"):
            raise RuntimeError(f"S123 admitted backend status differs: {receipt_path}")
    else:
        raise RuntimeError(
            "S123 backend phase mixes target and reader admission states: "
    ...
    if recorded_spans and admitted_page_span not in recorded_spans:
        raise RuntimeError(f"S123 admitted backend page span differs: {receipt_path}")
    if recorded_counts and admitted_page_count not in recorded_counts:
        raise RuntimeError(f"S123 admitted backend page count differs: {receipt_path}")
    return {
...
def verify_current_receipts(lane: Path) -> dict[str, Any]:
    """Fail closed until all S123 source, semantic, and backend gates pass."""
    ...
    for name in (
        ...
        if payload.get("unit_id") != UNIT_IDS["123"]:
            raise RuntimeError(f"current-unit receipt has wrong unit_id: {path}")
        if TARGET_HASHES["123"] not in json.dumps(payload, sort_keys=True):
        ...
        if name == "mt123-backend-validation.json":
            if payload.get("schema") != "o007-fremlin-mt123-backend-validation-v1":
                raise RuntimeError(f"S123 backend receipt schema differs: {path}")
            ...
            if (
                ...
                raise RuntimeError(f"S123 backend receipt checks are not all true: {path}")
            backend_phase = backend_admission_phase(payload, path)
    if backend_phase is None:
        raise RuntimeError("S123 backend admission phase was not verified")
...
def backend_gate_commands(phase: str) -> dict[str, list[str]]:
    ...
    generate = [sys.executable, "backend/generate_mt123.py", "--check"]
    validate = [sys.executable, "backend/validate_mt123.py"]
    if phase == "admitted":
        generate.append("--admit")
        validate.append("--expect-admitted")
    elif phase != "pending":
        raise RuntimeError(f"unknown S123 backend admission phase: {phase!r}")
...
def verify_backend_layout(lane: Path) -> dict[str, dict[str, object]]:
    """Require the complete validated S123 unit and catalog record surfaces."""
    backend = lane / "backend"
    unit = backend / "mt123"
    catalog = backend / "catalog-v1.3"
    for path in (backend / "generate_mt123.py", backend / "validate_mt123.py"):
        require_file(path)
    ...
    return {"mt123": tree_summary(unit), "catalog-v1.3": tree_summary(catalog)}
...
def exact_prior_html(lane: Path, number: str, candidate: Path) -> None:
    ...
    if candidate.read_bytes() != prior.read_bytes():
        raise RuntimeError(f"regenerated S{number} HTML differs from its admitted reader")
...
def copy_pdf_inputs(lane: Path, stage: Path) -> None:
    ...
    for name in (
        "sections111-115-121-122-123-id.tex",
...
def build_once(
    ...
    stage_name = "fremlin-v1-s111-s112-s113-s114-s115-s121-s122-s123-id"
    ...
    master = lane / "reader" / "pdf" / "sections111-115-121-122-123-id.tex"
    root_html_source = (
        lane / "reader" / "html" / "index-111-115-121-122-123-id.html"
    )
    ...
    packaged_backend_bindings = {
        "mt123": tree_summary(package / "backend" / "mt123"),
        "catalog-v1.3": tree_summary(package / "backend" / "catalog-v1.3"),
    }
    if packaged_backend_bindings != backend_bindings:
        raise RuntimeError("packaged S123 backend source records differ byte-for-byte")
    ...
    copy_tree(
        ...
        lambda relative: qa_member(
            relative, backend_phase["admission_phase"] == "admitted"
        ),
    )
...
def reproducibility_fingerprint(result: dict[str, Any]) -> dict[str, str]:
    return {
        ...
        **{
        ...
        "backend_mt123": str(
            result["backend_source_records"]["mt123"]["inventory_sha256"]
        ),
        "backend_catalog_v1_3": str(
            result["backend_source_records"]["catalog-v1.3"]["inventory_sha256"]
        ),
...
def main() -> int:
    ...
    verify_mt123_inputs(lane)
    required_files = [
        *(lane / "source" / "id-ID" / f"mt{number}.tex" for number in UNIT_IDS),
        lane / "reader" / "pdf" / "sections111-115-121-122-123-id.tex",
        ...
        *(lane / "scripts" / f"render_mt{number}_html.py" for number in UNIT_IDS),
        lane / "qa" / "mt123-backend-validation.json",
        lane / "qa" / "mt123-intake-census.json",
        lane / "qa" / "mt123-semantic-review.json",
        lane / "qa" / "mt123-structural-qa.json",
    ...
    stage = (
        lane
        / "build"
        / "fremlin-v1-s111-s112-s113-s114-s115-s121-s122-s123-id"
    )
    ...
    if preserved_before != preserved_after:
        raise RuntimeError("a pre-existing S111-through-S122 release artifact changed")

    qa_dir = lane / "qa"
    shutil.copyfile(package / "BUILD_METADATA.json", qa_dir / "mt123-build-metadata.json")
    shutil.copyfile(
        package / "PACKAGE_MANIFEST.tsv", qa_dir / "mt123-PACKAGE_MANIFEST.tsv"
    )
    ...
    (qa_dir / "mt123-SHA256SUMS.txt").write_text(
        "".join(
    ...
    evidence_names = {
        ...
        **{
            f"html-{number}.log": f"mt123-html{number}-render.log"
            for number in UNIT_IDS
        },
    }
    ...
    receipt: dict[str, Any] = {
        ...
        "backend_preflight": {
            ...
            "unit_records": 453,
            "catalog_records": 47,
            "catalog_unique_page_span": "10-56",
            "catalog_unique_page_count": 47,
    ...
    if backend_phase["admission_phase"] == "pending":
        write_immutable_json(
            qa_dir / "mt123-build-receipt-candidate.json", receipt
        )
    write_json(qa_dir / "mt123-build-receipt.json", receipt)
