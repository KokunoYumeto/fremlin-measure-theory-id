#!/usr/bin/env python3
"""Raster, structural, and visual QA for the reader through complete Chapter 26.

The admitted 419-page through-Chapter-25 reader and its passing visual receipt are
immutable controlling evidence.  Every cumulative page is rendered at 96 dpi;
all predecessor raster hashes must replay exactly, while the appended pages
are saved in complete contact sheets for explicit owner visual inspection.  The
default invocation records automated evidence only.  Finalization remains a
separate owner action after every saved sheet has actually been viewed.
"""
...
# Chapter 25 itself delegates the shared PDF-QA primitives to the Chapter 24
# harness; load that defining module directly rather than assuming the imported
# names are re-exported by the Chapter 25 wrapper.
base = load_module("o007_ch24_pdf_qa", Path(__file__).with_name("qa_volume1_through_chapter24_pdf.py"))
build_module = load_module("o007_chapter26_build", Path(__file__).with_name("build_volume1_through_chapter26.py"))

PDF = ROOT / "output" / "pdf" / "fondasi-teori-ukuran-jilid-1-dan-jilid-2-hingga-akhir-bab-26-id.pdf"
BUILD_RECEIPT = ROOT / "qa" / "through-chapter26-complete-build.json"
PRIOR_VISUAL_RECEIPT = ROOT / "qa" / "through-chapter25-pdf-visual-qa.json"
PRIOR_PDF = ROOT / "output" / "pdf" / "fondasi-teori-ukuran-jilid-1-dan-jilid-2-hingga-akhir-bab-25-id.pdf"
RECEIPT = ROOT / "qa" / "through-chapter26-pdf-visual-qa.json"
CONTACT_DIR = ROOT / "qa" / "rendered" / "through-chapter26-pdf-visual"
TMP_PARENT = ROOT / "tmp" / "pdfs"

EXPECTED_BUILD_RECEIPT_BYTES: int | None = 94_853
EXPECTED_BUILD_RECEIPT_SHA256: str | None = "81e8777bc52a4578712e2b40799dc3fafc316ce7d20edf970d7b2b53d5fa4b1d"
EXPECTED_PDF_BYTES: int | None = 3_426_613
EXPECTED_PDF_SHA256: str | None = "81bba1acf43824d1863f96bd484e872a7f6b40ab98405371e5c436634be04125"
EXPECTED_PAGES: int | None = 477
PRIOR_RECEIPT_BYTES = 299_281
PRIOR_RECEIPT_SHA256 = "3bd3d546011deb5e74a9d6e23988b9a3b1c502c2e8008cf0f8c78a557c4e5811"
...
EXPECTED_UNIT_IDS = [row[1] for row in build_module.CHAPTER26_UNITS]
...
def contact_sheet(page_paths: list[Path], page_numbers: list[int], output: Path, sequence: int) -> dict[str, Any]:
    ...
    for index, (path, number) in enumerate(zip(page_paths, page_numbers, strict=True)):
        ...
        label = f"Cumulative {number} | Volume II through Chapter 26 physical {volume2_page}"
...
def verify_build_receipt() -> tuple[dict[str, Any], dict[str, Any], int]:
    require(
        isinstance(EXPECTED_BUILD_RECEIPT_BYTES, int)
        and isinstance(EXPECTED_BUILD_RECEIPT_SHA256, str)
        and isinstance(EXPECTED_PDF_BYTES, int)
        and isinstance(EXPECTED_PDF_SHA256, str)
        and isinstance(EXPECTED_PAGES, int)
        and isinstance(APPENDED_PAGES, int),
        "Chapter 26 build/PDF constants are unbound; patch them from the completed deterministic build before QA",
    )
    ...
    require(build.get("schema") == "o007-fremlin-volume1-plus-volume2-through-chapter26-pdf-build-v1", "build schema differs")
    ...
    expected_accounting = {
        "volume1_pages": 102,
        "volume2_last_printed_page": 342,
        "selected_total_pages": 444,
    ...
    prefix = reproducibility.get("predecessor_419_page_prefix_preservation", {})
    require(
    ...
    units = build.get("chapter26_unit_receipts")
    require(isinstance(units, list) and [row.get("unit_id") for row in units] == EXPECTED_UNIT_IDS, "Chapter 26 unit order differs")
    for row in units:
...
def audit() -> dict[str, Any]:
    ...
    require(
        prior.get("schema") == "o007-volume1-plus-volume2-through-chapter25-pdf-visual-qa-v1"
    ...
    with tempfile.TemporaryDirectory(prefix="o007-through-chapter26-pdf-", dir=TMP_PARENT) as temp_name:
        temp = Path(temp_name)
        prefix = temp / "page"
        ...
        for offset in range(0, len(inspection_pages), 9):
            ...
            output = CONTACT_DIR / f"through-chapter26-contact-{sequence:02d}.png"
    ...
    automated_checks = {
        ...
        "build_receipt_and_seven_unit_bindings_exact": True,
        "pdfinfo_and_pypdf_page_count_exact": len(raster_rows) == EXPECTED_PAGES,
        "metadata_scope_model_license_and_444_of_672_exact": not metadata_mismatches,
        "all_pages_a4_zero_rotation": not geometry_mismatches,
        "all_419_predecessor_geometries_exact": not prior_geometry_mismatches,
        ...
        "all_419_predecessor_pixel_hashes_exact": not replay_mismatches,
    ...
    report: dict[str, Any] = {
        "schema": "o007-volume1-plus-volume2-through-chapter26-pdf-visual-qa-v1",
        "status": "automated_pass_visual_inspection_pending",
        "checked_at": "2026-08-29",
        "production_model": PRODUCTION_MODEL,
        "scope": {
            "included": ["Volume I complete", "Volume II official pages 1-342", "Chapter 26 complete through Section 266"],
            "excluded": ["Volume II Chapters 27-28 and appendices"],
            "locale": "id-ID",
            "license": "Design Science License for Fremlin-derived material",
            "official_source_page_accounting": "444 of 672 (Volume I 102 + Volume II official pages 1-342)",
        ...
        "prior_boundary_replay": {
            ...
            "all_419_predecessor_pages_reused_pixel_exact": True,
        },
        "inspection_plan": {
        ...
        "next_gate": "Owner inspects every saved through-Chapter26 contact sheet, then finalizes this receipt for admission.",
