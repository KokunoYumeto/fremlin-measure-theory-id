#!/usr/bin/env python3
"""Deterministically build and package cumulative O007 sections through 132.
...
only the admitted S115 staging-copy PDF reflow, and adds the complete S132
semantic reader.  No distribution path is touched until the S132 backend
...
TARGET_HASHES = {
    ...
    "132": "84da1785a751ab999a41dbbfffab37a91cdd0ae83948d1c341162eae48fbc814",
}
AUTHORITY_HASHES = {
    ...
    "132": "5bb8e80daa8d659ba21fd24c1c123eb17c3f76ac57d4102438acbb2622659ed6",
}

# A4 output is measured from pdfinfo after each build; official printed-page
# accounting is frozen independently by the S132 pagination evidence.
PDF_PAGES = None
CURRENT_OFFICIAL_PAGE_SPAN = "59-62"
CURRENT_OFFICIAL_PAGE_COUNT = 4
...
def deterministic_zip(package: Path, destination: Path) -> None:
    """Write an exact S132 cumulative ZIP with the frozen source-date timestamp."""
...
DURABLE_QA_INPUTS = {
    ...
    "mt132-backend-validation.json",
    "mt132-intake-census.json",
    "mt132-pagination-evidence.json",
    "mt132-semantic-review.json",
    "mt132-structural-qa.json",
    "mt132-terminology-gate.json",
    "mt132-visual-browser-qa.json",
    "mt132-browser-visual-qa.json",
    "mt132-pdf-visual-qa.json",
    "mt132-pdf-visual-qa-r3.json",
    "mt132-browser-visual-qa-r3.json",
...
ADMITTED_CANDIDATE_QA_INPUTS = {
    ...
    "mt132-build-receipt-candidate-r1.json",
    "mt132-reader-qa-candidate-r1.json",
    "mt132-pdf-visual-qa-r1.json",
    "mt132-browser-visual-qa-r1.json",
    "mt132-reader-qa-candidate-r3.json",
    "mt132-pdf-visual-qa-r3.json",
    "mt132-browser-visual-qa-r3.json",
}
...
def control_member(source: Path, relative: Path) -> bool:
    ...
    if relative.as_posix() == "CP0010_MT132_ADMISSION.md":
...
def verify_mt132_inputs(lane: Path) -> None:
...
def backend_admission_phase(
    ...
    """Accept the S132 backend's typed pending/admitted transition.

    S132's validator reports the current catalog unit under ``catalog.unit``;
    older builders exposed a larger catalog/evidence object.  Read the
    compact S132 shape and retain a conservative compatibility path for the
    latter without treating a backend event as reader admission.
    """
    catalog = payload.get("catalog")
    if not isinstance(catalog, dict):
        raise RuntimeError(f"S132 backend receipt lacks catalog phase: {receipt_path}")
    unit = catalog.get("unit")
    if isinstance(unit, dict):
        source_pages = unit.get("source_pages")
        if source_pages != CURRENT_OFFICIAL_PAGE_SPAN:
            raise RuntimeError(f"S132 backend source-page span differs: {receipt_path}")
        target_admitted = unit.get("target_admitted")
        status = unit.get("status")
    else:
        # Compatibility with a future expanded receipt shape.
        source_pages = catalog.get("current_unit_source_pages")
        target_admitted = catalog.get("current_unit_target_admitted")
        status = catalog.get("current_unit_status")
    if not isinstance(target_admitted, bool):
        raise RuntimeError(f"S132 backend admission flag is not boolean: {receipt_path}")
    if target_admitted:
        if status not in (None, "admitted"):
            raise RuntimeError(f"S132 admitted backend status differs: {receipt_path}")
        phase = "admitted"
        admitted_page_span = CUMULATIVE_OFFICIAL_PAGE_SPAN
        admitted_page_count = CUMULATIVE_OFFICIAL_PAGE_COUNT
    else:
        if status not in (None, "in_progress", "pending"):
            raise RuntimeError(f"S132 pending backend status differs: {receipt_path}")
        phase = "pending"
        admitted_page_span = "10-58"
        admitted_page_count = 49

    # The backend event is evidence of a stable-ID replay only.  It must never
    # be interpreted as visual/reader admission.
    artifacts_and_event = payload.get("artifacts_and_event")
    if not isinstance(artifacts_and_event, dict):
        artifacts_and_event = {
            "reader_package_admission_established_by_backend_event": False
        }
    if artifacts_and_event.get("reader_package_admission_established_by_backend_event") not in (None, False):
        raise RuntimeError(f"S132 backend event improperly establishes reader admission: {receipt_path}")
    inventory_page_span = CUMULATIVE_OFFICIAL_PAGE_SPAN
    inventory_page_count = CUMULATIVE_OFFICIAL_PAGE_COUNT
    reader_admitted = target_admitted
    admitted_boundary = {
        "current_unit_source_pages": CURRENT_OFFICIAL_PAGE_SPAN,
        "current_unit_source_page_count": CURRENT_OFFICIAL_PAGE_COUNT,
        "cumulative_official_page_span": CUMULATIVE_OFFICIAL_PAGE_SPAN,
        "cumulative_official_page_count": CUMULATIVE_OFFICIAL_PAGE_COUNT,
    }
    return {
        "admission_phase": phase,
        "receipt_path": "qa/mt132-backend-validation.json",
...
def verify_current_receipts(lane: Path) -> dict[str, Any]:
    """Fail closed until all S132 source, terminology, semantic, and backend gates pass."""
    ...
    for name in (
        ...
        if payload.get("unit_id") != UNIT_IDS["132"]:
            raise RuntimeError(f"current-unit receipt has wrong unit_id: {path}")
        if name != "mt132-terminology-gate.json" and TARGET_HASHES["132"] not in json.dumps(payload, sort_keys=True):
        ...
        if name == "mt132-backend-validation.json":
            if payload.get("schema") != "o007-fremlin-mt132-backend-validation-v1":
                raise RuntimeError(f"S132 backend receipt schema differs: {path}")
            ...
            if (
                ...
                raise RuntimeError(f"S132 backend receipt checks are not all true: {path}")
            backend_phase = backend_admission_phase(payload, path)
    if backend_phase is None:
        raise RuntimeError("S132 backend admission phase was not verified")
    return backend_phase


def verify_s132_support_receipts(lane: Path) -> None:
    """Bind the intake and official-pagination witnesses before any build."""
    intake_path = lane / "qa" / "mt132-intake-census.json"
    pagination_path = lane / "qa" / "mt132-pagination-evidence.json"
    intake = json.loads(intake_path.read_text(encoding="utf-8"))
    pagination = json.loads(pagination_path.read_text(encoding="utf-8"))
    if intake.get("unit_id") != UNIT_IDS["132"] or intake.get("status") != "pass":
        raise RuntimeError(f"S132 intake census is not a passing current-unit witness: {intake_path}")
    authority = intake.get("authority", {})
    if authority.get("sha256") != AUTHORITY_HASHES["132"]:
        raise RuntimeError(f"S132 intake authority hash differs: {intake_path}")
    intake_pages = intake.get("pagination", {})
    if (
        intake_pages.get("official_printed_pages") != CURRENT_OFFICIAL_PAGE_SPAN
        or intake_pages.get("page_count_inclusive") != CURRENT_OFFICIAL_PAGE_COUNT
        or intake_pages.get("cumulative_s111_s132_pages") != CUMULATIVE_OFFICIAL_PAGE_SPAN
        or intake_pages.get("cumulative_unique_official_pages") != CUMULATIVE_OFFICIAL_PAGE_COUNT
    ):
        raise RuntimeError(f"S132 intake pagination differs: {intake_path}")
    printed = pagination.get("printed_pagination", {})
    if (
        pagination.get("unit_id") != UNIT_IDS["132"]
        or printed.get("section_start_page") != 59
        or printed.get("section_end_page") != 62
        or printed.get("page_count_inclusive") != CURRENT_OFFICIAL_PAGE_COUNT
        or printed.get("cumulative_unique_official_pages") != CUMULATIVE_OFFICIAL_PAGE_COUNT
    ):
        raise RuntimeError(f"S132 official pagination evidence differs: {pagination_path}")


def backend_gate_commands(phase: str) -> dict[str, list[str]]:
    """Select the no-write checks matching the receipt's transition phase."""
    generate = [sys.executable, "backend/generate_mt132.py", "--check"]
    validate = [sys.executable, "backend/validate_mt132.py"]
    if phase == "admitted":
        generate.append("--admit")
        validate.append("--expect-admitted")
    elif phase != "pending":
        raise RuntimeError(f"unknown S132 backend admission phase: {phase!r}")
...
def verify_backend_layout(lane: Path) -> dict[str, dict[str, object]]:
    """Require the complete validated S132 unit and catalog record surfaces."""
    backend = lane / "backend"
    unit = backend / "mt132"
    catalog = backend / "catalog-v1.5"
    for path in (backend / "generate_mt132.py", backend / "validate_mt132.py"):
        require_file(path)
    ...
    return {"mt132": tree_summary(unit), "catalog-v1.5": tree_summary(catalog)}
...
def backend_record_counts(lane: Path) -> dict[str, int]:
    ...
    unit = lane / "backend" / "mt132"
    catalog = lane / "backend" / "catalog-v1.5"
    return {
...
def refined_prior_html(number: str, prior_bytes: bytes) -> bytes:
    ...
    # S131 already carries the refined title.  Match the old title only when
    # it is not the prefix of ``Ukuran``; this keeps the historical package
    # byte-stable while allowing the next cumulative build to replay it.
    old_exact = re.compile(PRIOR_HTML_OLD_TITLE + rb"(?!an)")
    old_count = len(old_exact.findall(prior_bytes))
    new_count = prior_bytes.count(PRIOR_HTML_NEW_TITLE)
    if old_count not in (0, PRIOR_HTML_TITLE_OCCURRENCES):
        raise RuntimeError(f"admitted S{number} HTML title occurrence count differs")
    if old_count == PRIOR_HTML_TITLE_OCCURRENCES:
        expected = old_exact.sub(PRIOR_HTML_NEW_TITLE, prior_bytes)
    elif new_count >= PRIOR_HTML_TITLE_OCCURRENCES:
        expected = prior_bytes
    else:
        raise RuntimeError(f"admitted S{number} HTML title occurrence count differs")
    for old, new, count in PRIOR_HTML_TERM_REFINEMENTS.get(number, ()):
        old_term_count = expected.count(old)
        if old_term_count == 0 and expected.count(new) >= count:
            # S131 already carries this audited migration; replaying it is
            # intentionally idempotent for the next cumulative boundary.
            continue
        if old_term_count != count:
            raise RuntimeError(
...
def copy_pdf_inputs(lane: Path, stage: Path) -> None:
    ...
    for name in (
        "sections111-115-121-122-123-131-132-id.tex",
...
def build_once(
    ...
    stage_name = "fremlin-v1-s111-s112-s113-s114-s115-s121-s122-s123-s131-s132-id"
    ...
    master = lane / "reader" / "pdf" / "sections111-115-121-122-123-131-132-id.tex"
    root_html_source = (
        lane / "reader" / "html" / "index-111-115-121-122-123-131-132-id.html"
    )
    ...
    packaged_backend_bindings = {
        "mt132": tree_summary(package / "backend" / "mt132"),
        "catalog-v1.5": tree_summary(package / "backend" / "catalog-v1.5"),
    }
    if packaged_backend_bindings != backend_bindings:
        raise RuntimeError("packaged S132 backend source records differ byte-for-byte")
    ...
    metadata: dict[str, Any] = {
        ...
        "official_pagination": {
            "current_unit": {
                "source_pages": CURRENT_OFFICIAL_PAGE_SPAN,
                "page_count": CURRENT_OFFICIAL_PAGE_COUNT,
            },
            "cumulative": {
                "source_pages": CUMULATIVE_OFFICIAL_PAGE_SPAN,
                "unique_page_count": CUMULATIVE_OFFICIAL_PAGE_COUNT,
            },
            "accounting_basis": "official printed pages; A4 PDF pages are measured independently in pdf.pages",
        },
        "pdf": pdf_layout,
        ...
        "s132_reader_census": {
            "official_printed_pages": CURRENT_OFFICIAL_PAGE_SPAN,
            "official_page_count": CURRENT_OFFICIAL_PAGE_COUNT,
            "cumulative_official_page_span": CUMULATIVE_OFFICIAL_PAGE_SPAN,
            "cumulative_official_page_count": CUMULATIVE_OFFICIAL_PAGE_COUNT,
            "formulas": 381,
            "semantic_ids": 26,
...
def reproducibility_fingerprint(result: dict[str, Any]) -> dict[str, str]:
    return {
        ...
        **{
        ...
        "backend_mt132": str(
            result["backend_source_records"]["mt132"]["inventory_sha256"]
        ),
        "backend_catalog_v1_5": str(
            result["backend_source_records"]["catalog-v1.5"]["inventory_sha256"]
        ),
...
def main() -> int:
    ...
    verify_mt132_inputs(lane)
    required_files = [
        *(lane / "source" / "id-ID" / f"mt{number}.tex" for number in UNIT_IDS),
        lane / "reader" / "pdf" / "sections111-115-121-122-123-131-132-id.tex",
        ...
        *(lane / "scripts" / f"render_mt{number}_html.py" for number in UNIT_IDS),
        lane / "qa" / "mt132-backend-validation.json",
        lane / "qa" / "mt132-intake-census.json",
        lane / "qa" / "mt132-pagination-evidence.json",
        lane / "qa" / "mt132-semantic-review.json",
        lane / "qa" / "mt132-structural-qa.json",
        lane / "qa" / "mt132-terminology-gate.json",
    ...
    verify_s132_support_receipts(lane)
    ...
    if args.preflight_only:
        print(
            json.dumps(
                {
                    "schema": "o007-s132-reader-build-preflight-v1",
                    "pass": True,
                    "package_name": PACKAGE_NAME,
    ...
    stage = (
        lane
        / "build"
        / "fremlin-v1-s111-s112-s113-s114-s115-s121-s122-s123-s131-s132-id"
    )
    ...
    if preserved_before != preserved_after:
        raise RuntimeError("a pre-existing S111-through-S131 release artifact changed")

    qa_dir = lane / "qa"
    shutil.copyfile(package / "BUILD_METADATA.json", qa_dir / "mt132-build-metadata.json")
    shutil.copyfile(
        package / "PACKAGE_MANIFEST.tsv", qa_dir / "mt132-PACKAGE_MANIFEST.tsv"
    )
    ...
    (qa_dir / "mt132-SHA256SUMS.txt").write_text(
        "".join(
    ...
    evidence_names = {
        "backend-generate-check.log": "mt132-backend-generate-check.log",
        "backend-validate.log": "mt132-backend-validate.log",
        "tex-pass1.log": "mt132-tex-pass1.log",
        "tex-pass2.log": "mt132-tex-pass2.log",
        "dvipdfmx.log": "mt132-dvipdfmx.log",
        "pdfinfo.log": "mt132-pdfinfo.log",
        **{
            f"html-{number}.log": f"mt132-html{number}-render.log"
            for number in UNIT_IDS
        },
    }
    ...
    receipt: dict[str, Any] = {
        ...
        "official_pagination": {
            "current_unit_source_pages": CURRENT_OFFICIAL_PAGE_SPAN,
            "current_unit_page_count": CURRENT_OFFICIAL_PAGE_COUNT,
            "cumulative_source_pages": CUMULATIVE_OFFICIAL_PAGE_SPAN,
            "cumulative_unique_page_count": CUMULATIVE_OFFICIAL_PAGE_COUNT,
            "a4_pdf_pages_measured": second["pdf"]["pages"],
        },
        "artifacts": second,
        "paths": {
    ...
    if backend_phase["admission_phase"] == "pending":
        write_immutable_json(
            qa_dir / "mt132-build-receipt-candidate-r1.json", receipt
        )
    write_json(qa_dir / "mt132-build-receipt.json", receipt)
