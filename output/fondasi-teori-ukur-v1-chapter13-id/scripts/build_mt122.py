#!/usr/bin/env python3
"""Deterministically build and package cumulative O007 sections 111-115 and 121-122.

The two passes rebuild the same bounded staging and distribution paths.  The
builder preserves every published S111-through-S121 release artifact, keeps the
...
cumulative reader preserves the admitted S111-through-S121 release artifacts
exactly and adds the complete S122 semantic reader.
"""
...
def deterministic_zip(package: Path, destination: Path) -> None:
    """Write an exact S122 cumulative ZIP with the frozen source-date timestamp."""
...
def verify_mt122_inputs(lane: Path) -> None:
    ...
    for number in UNIT_IDS:
        ...
        if sha256(target) != TARGET_HASHES[number]:
            boundary = "reviewed translation candidate" if number == "122" else "admitted translation"
...
def verify_current_receipts(lane: Path) -> None:
    """Fail closed until all three S122 source/backend/semantic gates pass."""
    for name in (
        "mt122-structural-qa.json",
        "mt122-semantic-review.json",
        "mt122-backend-validation.json",
        ...
        if payload.get("unit_id") != UNIT_IDS["122"]:
            raise RuntimeError(f"current-unit receipt has wrong unit_id: {path}")
        if TARGET_HASHES["122"] not in json.dumps(payload, sort_keys=True):
...
def exact_prior_html(lane: Path, number: str, candidate: Path) -> None:
    release_name = (
        "fondasi-teori-ukur-v1-s111-s112-s113-s114-s115-s121-id"
        if number == "121"
        else "fondasi-teori-ukur-v1-s111-s112-s113-s114-s115-id"
    )
    prior = (
        lane
        / "output"
    ...
    if candidate.read_bytes() != prior.read_bytes():
        raise RuntimeError(
            f"regenerated S{number} HTML differs from its admitted reader"
        )
...
def copy_pdf_inputs(lane: Path, stage: Path) -> None:
    ...
    for name in ("sections111-115-121-122-id.tex", "mt113-dvipdfmx-images.tex"):
...
def build_once(
    ...
    reset_directory(lane, stage, "fremlin-v1-s111-s112-s113-s114-s115-s121-s122-id")
    ...
    master = lane / "reader" / "pdf" / "sections111-115-121-122-id.tex"
    support = lane / "reader" / "pdf" / "mt113-dvipdfmx-images.tex"
    root_html_source = lane / "reader" / "html" / "index-111-115-121-122-id.html"
    ...
    renderer_122 = lane / "scripts" / "render_mt122_html.py"
    ...
    target_122 = lane / "source" / "id-ID" / "mt122.tex"
    html_commands = {
        "111": [
            sys.executable,
...
def reproducibility_fingerprint(result: dict[str, Any]) -> dict[str, str]:
    return {
        ...
        "html_122": result["html"]["122"]["sha256"],
...
def main() -> int:
    ...
    required_files = [
        ...
        *(lane / "reader" / "static" / name for name in ("reader.css", "reader-v2.css", "reader-v3.css")),
        ...
        lane / "scripts" / "render_mt122_html.py",
        lane / "qa" / "mt122-backend-validation.json",
        lane / "qa" / "mt122-semantic-review.json",
        lane / "qa" / "mt122-structural-qa.json",
    ...
    stage = lane / "build" / "fremlin-v1-s111-s112-s113-s114-s115-s121-s122-id"
    ...
    if preserved_before != preserved_after:
        raise RuntimeError("a pre-existing S111-through-S121 release artifact changed")

    qa_dir = lane / "qa"
    shutil.copyfile(package / "BUILD_METADATA.json", qa_dir / "mt122-build-metadata.json")
    shutil.copyfile(package / "PACKAGE_MANIFEST.tsv", qa_dir / "mt122-PACKAGE_MANIFEST.tsv")
    ...
    (qa_dir / "mt122-SHA256SUMS.txt").write_text(
    ...
    write_json(qa_dir / "mt122-build-receipt.json", receipt)
