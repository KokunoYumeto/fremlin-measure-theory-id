#!/usr/bin/env python3
"""Publish the admitted O007 444/672 checkpoint to its existing GitHub line.
...
complete Chapter 26 boundary.  ``--preflight`` performs no Git command,
...
PREDECESSOR_TAG = "v0.18.0-v2-through-ch25"
PREDECESSOR_BOUNDARY_COMMIT = "a1b93b7f0ca5197f2ca05e5a0e1cfb1c0b5ebb4e"
PREDECESSOR_RECEIPT_COMMIT = "9867e5146527183d40d93f34a680f125fa1b35d3"
PREDECESSOR_MAIN_COMMIT = "108b8500489eeabba6e0432e05098226ac0d128a"
PREDECESSOR_RECEIPT_RELATIVE = "qa/PUBLICATION_RECEIPT_V0180_V2_THROUGH_CH25.json"
PREDECESSOR_RECEIPT_PATH = ROOT / PREDECESSOR_RECEIPT_RELATIVE
PREDECESSOR_RECEIPT_BYTES = 4_854
...
PACKAGE_RELATIVE = "qa/through-chapter26-release-package.json"
PACKAGE_PATH = ROOT / PACKAGE_RELATIVE
TREE_RELATIVE = "qa/THROUGH_CHAPTER26_RELEASE_TREE.tsv"
TREE_PATH = ROOT / TREE_RELATIVE
RECEIPT_RELATIVE = "qa/PUBLICATION_RECEIPT_V0190_V2_THROUGH_CH26.json"
RECEIPT_PATH = ROOT / RECEIPT_RELATIVE
PUBLIC_VALIDATION_RELATIVE = "qa/through-chapter26-public-overlay-validation.json"
PUBLIC_MANIFEST_RELATIVE = "qa/through-chapter26-PUBLIC_SOURCE_TREE_MANIFEST.tsv"
PUBLIC_MAP_RELATIVE = "qa/through-chapter26-PUBLIC_SANITIZATION_MAP.json"
...
BACKEND_VALIDATION_RELATIVE = "backend/through-chapter26-backend-validation.json"
...
PDF_RELATIVE = (
    "output/pdf/fondasi-teori-ukuran-jilid-1-dan-jilid-2-hingga-akhir-bab-26-id.pdf"
)
EXPECTED_PDF_BYTES = 3_426_613
EXPECTED_PDF_SHA256 = "81bba1acf43824d1863f96bd484e872a7f6b40ab98405371e5c436634be04125"
EXPECTED_PDF_REFLOW_PAGES = 477
...
EXPECTED_PUBLIC_ASSET_NAMES = (
    "00_READ_FIRST_FONDASI_TEORI_UKURAN_V1_DAN_V2_HINGGA_BAB_26.pdf",
    "fondasi-teori-ukuran-v1-dan-v2-hingga-bab26-id-v0.19.0.zip",
    "SHA256SUMS-v0.19.0-v2-through-ch26.txt",
)
...
REQUIRED_BOUNDARY_EVIDENCE = {
    ...
    AGGREGATE_RELATIVE,
    "qa/through-chapter26-complete-build.json",
    "qa/through-chapter26-pdf-visual-qa.json",
    HTML_BUILD_RELATIVE,
...
def validate_predecessor_receipt() -> dict[str, Any]:
    require(
        ...
        "immutable GitHub v0.18 predecessor receipt identity differs",
    )
    value = load_json(PREDECESSOR_RECEIPT_PATH, "GitHub v0.18 predecessor receipt")
    scope = value.get("scope", {})
    require(
        value.get("destination") == "github"
        and value.get("version") == "0.18.0-v2-through-ch25"
        ...
        and scope.get("official_pages_complete") == 389
        and scope.get("volume2_last_included_page") == 287
        and scope.get("volume2_chapter25_complete") is True
        and value.get("release", {}).get("prerelease") is True,
        "GitHub v0.18 predecessor receipt semantics differ",
    )
    return value


def validate_admission() -> dict[str, Any]:
    value = load_json(ADMISSION_PATH, "final through-Chapter-26 admission receipt")
    ...
    require(
        ...
        "final through-Chapter-26 admission has not been issued",
    )
    ...
    require(
        ...
        "admission version or 444/672 boundary differs",
    )
    ...
    require(
        ...
        and publication.get("zenodo_predecessor_doi") == "10.5281/zenodo.22149439"
    ...
    exact_file(ADMISSION_RECORD_PATH, record, "CP0019 admission record")
...
def validate_package(
    ...
    value = load_json(PACKAGE_PATH, "final through-Chapter-26 package receipt")
    ...
    require(tuple(binding.name for binding in assets) == EXPECTED_PUBLIC_ASSET_NAMES,
            "package release-asset names differ from the Chapter-26 v0.19.0 contract")
    reader = assets[0]
    require(
    ...
    exact_file(backend_path, backend_row, "package-bound Chapter-26 backend validation")
    backend = load_json(backend_path, "package-bound Chapter-26 backend validation")
    require(
        backend.get("schema") == "o007-through-chapter26-backend-validation-v1"
        and backend.get("status") == "pass" and backend.get("pass") is True
        and backend.get("catalog_counts", {}).get("resources") == 267
        and backend.get("catalog_counts", {}).get("units") == 70
        and backend.get("schema_valid_record_count") == 5_508
        and backend.get("output_inventory", {}).get("file_count") == 217
        and isinstance(backend.get("manifests"), dict)
        and "catalog-v1.14" in backend["manifests"],
        "package backend is not the passing catalog-v1.14 checkpoint",
    )
    ...
    content_relative, content = exact_relative_binding(
        value.get("content_admission"), "package-bound CP0019 admission record",
    )
    require(
        ...
        and all(content.get(key) == admission["content_admission"].get(key)
                for key in ("path", "bytes", "sha256")),
        "package and admission CP0019 bindings disagree",
    )
    aggregate_relative, aggregate = exact_relative_binding(
    ...
    require(isinstance(zenodo, dict) and zenodo.get("doi") == "10.5281/zenodo.22149439",
    ...
    require(
        zenodo_relative == "qa/ZENODO_PUBLICATION_RECEIPT_V0180_V2_THROUGH_CH25.json"
        and (zenodo_receipt.get("bytes"), zenodo_receipt.get("sha256"))
        == (4_368, "bcbd756191b9e7b9e1cfa4a550105dc7cef4ac9390d458f79cafae9acf9c8224"),
...
def release_name() -> str:
    return "Fondasi Teori Ukuran Bahasa Indonesia — 444/672 halaman"
...
def release_body(pdf_name: str) -> str:
    return (
        ...
        "Cakupan kumulatif: Jilid 1 lengkap serta Jilid 2 dari bagian pendahuluan sampai akhir Bab 26, "
        "yaitu halaman resmi Jilid 2 nomor 1–342 dan total 444 dari 672 halaman resmi korpus terpilih. "
        "Bab 26 lengkap; Bab 27 dan sesudahnya belum termasuk. PDF pembaca ditempatkan pertama, "
...
def stage_boundary(contract: ReleaseContract) -> tuple[str, str, int]:
    ...
    require(engine.run_git("rev-parse", "HEAD") == PREDECESSOR_MAIN_COMMIT,
            "HEAD is not the exact receipt-bound pre-v0.19 main commit")
    ...
    with tempfile.TemporaryDirectory(prefix="o007-through-chapter26-git-") as name:
    ...
    require(engine.run_git("write-tree") != engine.run_git("rev-parse", "HEAD^{tree}"),
            "through-Chapter-26 boundary contains no staged change")
    engine.run_git("commit", "--no-verify", "-m",
                   "Admit Indonesian Fremlin checkpoint through complete Chapter 26")
...
def write_receipt(
    ...
    value = {
        ...
        "verification": {
            ...
            "catalog_v1_14_package_binding_replayed": True,
            "canonical_private_evidence_files_mutated": False,
            "volume2_contiguous_pages_1_342_disclosed": True,
    ...
    if RECEIPT_PATH.exists():
        ...
        require(old == current, "existing through-Chapter-26 GitHub receipt differs")
        return RECEIPT_PATH.read_bytes()
    temporary = RECEIPT_PATH.with_name(RECEIPT_PATH.name + ".tmp-v019-through-chapter26")
...
def commit_receipt(token: str, boundary: str, payload: bytes) -> str:
    ...
    if head != boundary:
        ...
        with tempfile.TemporaryDirectory(prefix="o007-through-chapter26-receipt-") as name:
        ...
        engine.run_git("commit", "--no-verify", "-m",
                       "Record public checkpoint through complete Chapter 26")
...
if __name__ == "__main__":
    try:
        ...
        print(f"ERROR: fail-closed through-Chapter-26 GitHub publication: {exc}",
              file=sys.stderr)
        raise SystemExit(1)
