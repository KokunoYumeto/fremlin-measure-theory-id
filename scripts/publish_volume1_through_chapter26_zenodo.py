#!/usr/bin/env python3
"""Publish the admitted O007 444/672 checkpoint in its Zenodo concept.

Only one new version of concept DOI 10.5281/zenodo.22059798 is permitted, and
only from exact public record 10.5281/zenodo.22149439.  The local Chapter-26
...
import publish_volume1_through_chapter26_github as gate
...
PREDECESSOR_RECORD_ID = 22_149_439
PREDECESSOR_DOI = "10.5281/zenodo.22149439"
PREDECESSOR_VERSION = "0.18.0-v2-through-ch25"
...
PREDECESSOR_RECEIPT_RELATIVE = "qa/ZENODO_PUBLICATION_RECEIPT_V0180_V2_THROUGH_CH25.json"
PREDECESSOR_RECEIPT_PATH = ROOT / PREDECESSOR_RECEIPT_RELATIVE
PREDECESSOR_RECEIPT_BYTES = 4_368
PREDECESSOR_RECEIPT_SHA256 = (
    "bcbd756191b9e7b9e1cfa4a550105dc7cef4ac9390d458f79cafae9acf9c8224"
)
RECEIPT_RELATIVE = "qa/ZENODO_PUBLICATION_RECEIPT_V0190_V2_THROUGH_CH26.json"
...
def validate_static_contract() -> None:
    require(gate.VERSION == VERSION, "through-Chapter-26 gate version differs")
    require(gate.TAG == TAG, "through-Chapter-26 gate tag differs")
    require(gate.MODEL == MODEL, "through-Chapter-26 gate model provenance differs")
    require(gate.expected_coverage() == EXPECTED_COVERAGE,
            "through-Chapter-26 gate coverage differs from 444/672")
...
def asset_bindings(contract: gate.ReleaseContract) -> dict[str, gate.AssetBinding]:
    ...
    for name, binding in bindings.items():
        ...
        require(
            ...
            f"through-Chapter-26 package-bound asset identity differs: {name}",
        )
    return bindings
...
def metadata() -> dict[str, object]:
    description = (
        "<p><strong>Prarilis parsial: 444 dari 672 halaman resmi korpus "
        ...
        "dan Bab 21–26 secara lengkap.</p>"
        "<p>Cakupan Jilid 2 pada checkpoint ini adalah halaman resmi 1–342. "
        "Bab 26 lengkap; Bab 27 dan sesudahnya belum termasuk, sehingga korpus "
    ...
    return {
        ...
        "notes": (
            "Prarilis parsial 444/672 halaman resmi: Jilid 1 lengkap dan Jilid 2 "
            "dari bagian pendahuluan sampai akhir Bab 26, mencakup halaman resmi "
            "Jilid 2 nomor 1–342. Bab 26 lengkap; Bab 27 dan sesudahnya belum "
...
def predecessor_receipt() -> dict[str, Any]:
    ...
    try:
        ...
        raise PublicationError("Zenodo v0.18 predecessor receipt is unreadable") from exc
    require(isinstance(value, dict), "Zenodo v0.18 predecessor receipt is malformed")
    ...
    require(
        ...
        "Zenodo v0.18 predecessor receipt semantics differ",
    )
...
def choose_candidate(token: str) -> tuple[dict[str, Any], str, bool]:
    ...
    require(len(exact_public) <= 1,
            "multiple exact public through-Chapter-26 versions exist")
    if exact_public:
        require(not drafts,
                "exact public through-Chapter-26 version coexists with a concept draft")
        return exact_public[0], "already_published_exact_through_chapter26", True
    ...
    return draft, "one_newversion_from_22149439", False
...
def write_receipt(
    ...
    value = {
        ...
        "record": {
            ...
            "partial_coverage": "444/672 official pages; Volume II pages 1-342 included",
        },
        "asset_order": [binding.name for binding in contract.assets],
        "assets": assets,
        "transaction_route": "existing_concept_newversion_from_22149439",
        "runtime_route": route,
        "verification": {
    ...
    if RECEIPT_PATH.exists():
        try:
            ...
            raise PublicationError(
                "existing through-Chapter-26 Zenodo receipt is unreadable"
            ) from exc
        ...
        require(old == current, "existing through-Chapter-26 Zenodo receipt differs")
...
if __name__ == "__main__":
    try:
        ...
        print(f"ERROR: fail-closed through-Chapter-26 Zenodo publication: {exc}",
              file=sys.stderr)
        raise SystemExit(1)
