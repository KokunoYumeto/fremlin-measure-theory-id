#!/usr/bin/env python3
"""Fail-closed validator for the cumulative O007 catalog-v1.11 checkpoint."""
...
import generate_through_chapter24_checkpoint as generator
import validate_volume1_chapter22_checkpoint as common
from o007_backend_core import sha256_bytes


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_RECEIPT = ROOT / "backend/chapter24-backend-validation.json"
...
def validate_predecessor_preservation(
    ...
    if catalog["corpus"] != prior["corpus"]:
        raise ValueError("catalog-v1.10 corpus record changed")
    if catalog["rights"] != prior["rights"]:
        raise ValueError("catalog-v1.10 DSL rights record changed")
    if catalog["units"][:len(prior["units"])] != prior["units"]:
        raise ValueError("catalog-v1.10 unit prefix changed")

    current_resources = catalog["resources"][:len(prior["resources"])]
    if len(current_resources) != len(prior["resources"]):
        raise ValueError("catalog-v1.10 resource prefix length changed")
    ...
    for before, after in zip(prior["resources"], current_resources):
        ...
        if after != expected:
            raise ValueError(
                f"catalog-v1.10 resource prefix changed beyond sanctioned path repair: {before['id']}"
            )
    ...
    return {
        "prior_catalog": "backend/catalog-v1.10",
...
def validate_catalog_state(
    ...
    if [row["id"] for row in new_units] != list(generator.CHAPTER24_UNIT_IDS):
        raise ValueError("Chapter 24 catalog unit order differs")
    ...
    if chapter_pages != set(range(138, 204)):
        raise ValueError("Chapter 24 official-page union is not exactly 138-203")
    ...
    expected_ids = list(prior_volume2["admitted_unit_ids"]) + list(generator.CHAPTER24_UNIT_IDS)
    if (
        volume2["status"] != "in_progress"
        or volume2["admitted_source_page_span"] != "1-203"
        or volume2["admitted_unique_source_page_count"] != 203
        or volume2["admitted_unit_ids"] != expected_ids
    ):
        raise ValueError("Volume-II front-matter-through-Chapter-24 accounting differs")
    ...
    required = {
        ...
        "O007-RESOURCE-CH24-MODEL-PROVENANCE",
        *(
            value
            for state in states
            for value in (
    ...
    if not required <= resource_ids:
        raise ValueError("Chapter 24 evidence resources are incomplete")
    return {
        "official_section_starts": starts,
        "frontmatter_through_chapter23_pages": "1-137",
        "chapter24_pages": "138-203",
        "chapter24_unique_page_count": len(chapter_pages),
        ...
        "required_chapter24_resource_count": len(required),
    }
...
def validate_correction_and_term_bindings(
    ...
    if {row["id"] for row in correction_records} != expected_ids or len(correction_records) != len(expected_ids):
        raise ValueError("generated Chapter 24 correction set differs")
    formula_links = [
        correction_id
        for state in states
    ...
    correction_resource = resources["O007-RESOURCE-CH24-SOURCE-CORRECTIONS"]
    ...
    terminology_resource = resources["O007-RESOURCE-CH24-TERMINOLOGY-DECISIONS"]
    ...
    return {
        ...
        "chapter24_correction_records": len(correction_records),
...
def validate_model_and_rights(catalog: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    resources = {row["id"]: row for row in catalog["resources"]}
    model = resources["O007-RESOURCE-CH24-MODEL-PROVENANCE"]
...
def validate() -> dict[str, Any]:
    ...
    manifests["catalog-v1.11"] = common.validate_output_directory(
    ...
    return {
        ...
        "generator": "backend/generate_through_chapter24_checkpoint.py",
