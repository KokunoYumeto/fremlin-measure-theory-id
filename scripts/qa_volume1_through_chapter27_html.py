#!/usr/bin/env python3
"""Replay the complete 81-route through-Chapter-27 reader in Chromium.
...
import qa_volume1_through_chapter26_html as predecessor
import render_volume1_through_chapter27_html as renderer


ROOT = Path(__file__).resolve().parents[1]
READER = ROOT / "output" / "fondasi-teori-ukuran-v1-through-chapter27-id" / "html"
BUILD_RECEIPT = ROOT / "qa" / "through-chapter27-html-build.json"
RECEIPT = ROOT / "qa" / "through-chapter27-html-reader-qa.json"
MODEL = "OpenAI Codex gpt-5.6-sol, Ultra"
CHECKED_AT = "2026-08-29"

EXPECTED_ROUTES = tuple(predecessor.EXPECTED_ROUTES) + ("27", "271", "272", "273", "274", "275", "276")
...
def validate_static_tree() -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    ...
    require(len(EXPECTED_ROUTES) == 81, "internal expected route count differs")
    ...
    require(build.get("schema") == "o007-volume1-through-volume2-chapter27-html-build-v1", "build schema differs")
    require(build.get("checks", {}).get("routes") == 81, "build route count differs")
    coverage = build.get("coverage", {})
    require(
        coverage.get("official_pages_complete") == 509
        and coverage.get("corpus_official_pages") == 672
        and coverage.get("volume_2_chapter_27") == "complete"
        and coverage.get("volume_2_contiguous_source_pages") == [1, 407],
...
def run_browser_in_bounded_batches(
    ...
    retaining one target across 162 Chapter-27 observations can accumulate
    ...
    require(len(observations) == 162, "merged browser observation count differs")
...
def build_receipt() -> dict[str, Any]:
    ...
    return {
        "schema": "o007-volume1-through-volume2-chapter27-html-browser-qa-v1",
        ...
        "scope": {
            ...
            "included": [
                ...
                "Volume II Chapter 27 complete, official pages 343-407",
            ],
            "excluded": ["Volume II Chapter 28 and appendices"],
            "official_source_page_accounting": "509 of 672 (Volume I 102 + Volume II pages 1-407)",
            "chapter_27_status": "complete",
            "html_routes_in_materialized_tree": 81,
        },
        "inputs": {
        ...
        "coverage": {
            ...
            "unique_current_routes_with_desktop_and_mobile_evidence": 81,
            "route_viewport_observations": 162,
        ...
        "checks": {
            ...
            "all_81_routes_exercised_at_desktop_and_mobile": True,
            "math_source_rendered_assistive_parity_every_route": True,
            "all_reader_facing_chapter27_target_math_replayed": True,
            "all_non_body_chapter27_math_explicitly_receipted": True,
        ...
        "next_gate": (
            ...
            "and aggregate receipts into CP0020; this receipt does not self-admit."
        ),
    }
