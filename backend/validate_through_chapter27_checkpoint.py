#!/usr/bin/env python3
"""Fail-closed validator for the cumulative complete-Chapter-27 backend."""
...
import generate_through_chapter27_checkpoint as generator
import validate_volume1_chapter22_checkpoint as common
from o007_backend_core import sha256_bytes


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_RECEIPT = ROOT / "backend/through-chapter27-backend-validation.json"
...
def validate_predecessor_preservation(
    ...
    if catalog["corpus"] != prior["corpus"]:
        raise ValueError("catalog-v1.14 corpus record changed")
    if catalog["rights"] != prior["rights"]:
        raise ValueError("catalog-v1.14 DSL rights record changed")
    ...
    if len(current_units) != len(prior["units"]):
        raise ValueError("catalog-v1.14 unit prefix length changed")
    for before, after in zip(prior["units"], current_units):
        ...
        if after != expected:
            raise ValueError(f"catalog-v1.14 unit changed outside CP0019 admission replay: {unit_id}")
    if exercise_check != {"catalog": "backend/catalog-v1.14", "active_exercises": 841, "changed": False}:
        raise ValueError("catalog-v1.14 predecessor exercise census check differs")

    current_resources = catalog["resources"][:len(prior["resources"])]
    if len(current_resources) != len(prior["resources"]):
        raise ValueError("catalog-v1.14 resource prefix length changed")
    ...
    for before, after in zip(prior["resources"], current_resources):
        ...
        if after != expected:
            raise ValueError(f"catalog-v1.14 resource changed beyond sanctioned snapshot repair: {before['id']}")
    if {row["resource_id"] for row in repairs} != set(generator.INHERITED_SNAPSHOT_SPECS):
        raise ValueError("catalog-v1.14 stale-path repair set differs")
    ...
    return {
        "prior_catalog": "backend/catalog-v1.14",
        ...
        "cp0019_status_promotions": list(generator.PREDECESSOR_PENDING_UNIT_IDS),
...
def validate_catalog_state(
    ...
    if [row["id"] for row in new_units] != list(generator.COMPLETE_CHAPTER27_NEW_UNIT_IDS):
        raise ValueError("complete-Chapter-27 catalog unit order differs")
    expected_titles = [
        ("Probability theory", "Teori probabilitas"),
        ("Distributions", "Distribusi"),
        ("Independence", "Kebebasan"),
        ("The strong law of large numbers", "Hukum kuat bilangan besar"),
        ("The Central Limit Theorem", "Teorema limit pusat"),
        ("Martingales", "Martingal"),
        ("Martingale difference sequences", "Barisan selisih martingal"),
    ]
    ...
    if [(row["source_title"], row["target_working_title"]) for row in new_units] != expected_titles:
        raise ValueError("complete-Chapter-27 source/Indonesian title mapping differs")
    ...
    if len(inherited_ids) != 43 or set(inherited_records) != set(inherited_ids):
    ...
    if new_pages != set(range(343, 408)):
        raise ValueError("complete Chapter 27 official-page union is not exactly 343-407")
    starts = generator.official_section_starts()
    volume2 = next(row for row in catalog["volumes"] if row["id"] == generator.VOLUME_ID)
    expected_ids = inherited_ids + list(generator.COMPLETE_CHAPTER27_NEW_UNIT_IDS)
    if (
        volume2["status"] != "in_progress"
        or volume2["admitted_source_page_span"] != "1-407"
        or volume2["admitted_unique_source_page_count"] != 407
        or volume2["admitted_unit_ids"] != expected_ids
    ):
        raise ValueError("Volume-II complete-Chapter-27 accounting differs")
    ...
    if cumulative_exercises != 952 or cumulative_hints != 233:
        raise ValueError("corrected cumulative exercise/hint census differs")
    chapter_ids = set(generator.COMPLETE_CHAPTER27_NEW_UNIT_IDS)
    chapter_units = [row for row in catalog["units"] if row["id"] in chapter_ids]
    if len(chapter_units) != 7:
        raise ValueError("complete Chapter 27 catalog unit set differs")
    ...
    if chapter_exercises != 111 or chapter_hints != 37:
        raise ValueError("complete Chapter 27 exercise/hint census differs")
    ...
    if not required <= resource_ids:
        raise ValueError("complete-Chapter-27 evidence resources are incomplete")
    return {
        "official_section_starts": starts,
        "boundary_label": "COMPLETE CHAPTER 27",
        "chapter27_complete": True,
        "predecessor_through_chapter26_pages": "1-342",
        "new_chapter27_pages": "343-407",
        "new_unique_official_page_count": len(new_pages),
        "complete_chapter27_pages": "343-407",
        "complete_chapter27_unique_page_count": 65,
        ...
        "pre_chapter27_active_exercises": 841,
        "complete_chapter27_active_exercises": chapter_exercises,
        "cumulative_active_exercises": cumulative_exercises,
        "pre_chapter27_explicit_hints": 196,
        "complete_chapter27_explicit_hints": chapter_hints,
        ...
        "required_complete_chapter27_resource_count": len(required),
    }
...
def validate_unit_closure(
    ...
    if state.config.slug == "mt274":
        whheader_rows = [
            row for row in datasets["exercises"]
            if row.get("semantic_anchor") == "274Xf"
            and str(row.get("source_text", "")).startswith("\\wheader{274Xf}")
        ]
        if len(whheader_rows) != 1:
            raise ValueError("mt274 active \\wheader exercise 274Xf is not preserved exactly once")
...
def validate_correction_and_term_bindings(
    ...
    if {row["id"] for row in correction_records} != expected_ids or len(correction_records) != len(expected_ids):
        raise ValueError("generated complete-Chapter-27 correction set differs")
    ...
    correction_resource = resources["O007-RESOURCE-CH27-COMPLETE-SOURCE-CORRECTIONS"]
    ...
    terminology_resource = resources["O007-RESOURCE-CH27-COMPLETE-TERMINOLOGY-DECISIONS"]
    ...
    return {
        ...
        "chapter27_correction_records": len(correction_records),
...
def validate_model_and_rights(catalog: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    resources = {row["id"]: row for row in catalog["resources"]}
    model = resources["O007-RESOURCE-CH27-COMPLETE-MODEL-PROVENANCE"]
...
def validate() -> dict[str, Any]:
    ...
    manifests["catalog-v1.15"] = common.validate_output_directory(
    ...
    return {
        ...
        "generator": "backend/generate_through_chapter27_checkpoint.py",
