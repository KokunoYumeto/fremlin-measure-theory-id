...
def main() -> int:
    ...
    if sha256_bytes(source_bytes) != EXPECTED_SOURCE_SHA256 or len(source_bytes) != 43014:
        raise SystemExit("frozen mt121 authority identity mismatch")
    if sha256_bytes(target_bytes) != EXPECTED_TARGET_SHA256 or len(target_bytes) != EXPECTED_TARGET_BYTES:
        raise SystemExit("final mt121 target identity mismatch or target is not frozen")
    corrections_bytes = CORRECTIONS_PATH.read_bytes()
    intake_bytes = INTAKE_PATH.read_bytes()
    source_review_bytes = SOURCE_REVIEW_PATH.read_bytes()
    if sha256_bytes(corrections_bytes) != EXPECTED_CORRECTIONS_SHA256:
        raise SystemExit("S121 correction ledger identity mismatch")
    if sha256_bytes(intake_bytes) != EXPECTED_INTAKE_SHA256:
        raise SystemExit("S121 intake census identity mismatch")
    if sha256_bytes(source_review_bytes) != EXPECTED_SOURCE_REVIEW_SHA256:
        raise SystemExit("S121 source review identity mismatch")
    source, target = source_bytes.decode("utf-8"), target_bytes.decode("utf-8")
    if len(source.splitlines()) != 1057 or len(target.splitlines()) != EXPECTED_TARGET_LINES:
        raise SystemExit("S121 source/target line identity mismatch")
    ...
    counts = {
        ...
        "semantic_shorthand_relations": 11,
    ...
    if SCHEMA_PATH.read_bytes() != schema_before:
        raise ValueError("S121 generator must preserve schema-v1.1.json byte-identically")
    if (BACKEND / "o007_backend_core.py").read_bytes() != core_before:
        raise ValueError("S121 generator must preserve o007_backend_core.py byte-identically")
    if (BACKEND / "o007_nested_math.py").read_bytes() != scanner_before:
        raise ValueError("S121 generator must preserve o007_nested_math.py byte-identically")
