#!/usr/bin/env python3
"""Replay the complete 67-route through-Chapter-25 reader in Chromium.
...
import qa_volume1_through_s252_html as predecessor


ROOT = Path(__file__).resolve().parents[1]
READER = ROOT / "output" / "fondasi-teori-ukuran-v1-through-chapter25-id" / "html"
BUILD_RECEIPT = ROOT / "qa" / "through-chapter25-html-build.json"
RECEIPT = ROOT / "qa" / "through-chapter25-html-reader-qa.json"
MODEL = "OpenAI Codex gpt-5.6-sol, Ultra"
CHECKED_AT = "2026-08-28"

EXPECTED_ROUTES = tuple(predecessor.EXPECTED_ROUTES) + ("253", "254", "255", "256", "257")
VIEWPORTS = predecessor.VIEWPORTS
EXPECTED_MATH = {"253": 1173, "254": 2180, "255": 934, "256": 902, "257": 197}
...
def validate_static_tree() -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    ...
    require(len(EXPECTED_ROUTES) == 67, "internal expected route count differs")
    require(set(routes) == set(EXPECTED_ROUTES), f"route surface differs: {routes!r}")

    parsed: dict[Path, predecessor.chapter24.ReferenceParser] = {}
    ...
    require(build.get("schema") == "o007-volume1-through-volume2-chapter25-html-build-v1", "build schema differs")
    require(build.get("checks", {}).get("routes") == 67, "build route count differs")
    coverage = build.get("coverage", {})
    require(
        coverage.get("official_pages_complete") == 389
        and coverage.get("corpus_official_pages") == 672
        and coverage.get("volume_2_chapter_25") == "complete"
        and coverage.get("volume_2_contiguous_source_pages") == [1, 287],
    ...
    require(
        adjustments["256"].get("canonical_target_math_atoms") == 904
        and adjustments["256"].get("reader_math_exclusions") == 2
        and len(adjustments["256"].get("reader_math_exclusion_receipts", [])) == 2,
        "mt256 print-title math accounting differs",
    )
    ...
    static_state = {
        ...
        "mt256_print_title_metadata_atoms_explicitly_receipted": 2,
    }
...
def build_receipt() -> dict[str, Any]:
    ...
    return {
        "schema": "o007-volume1-through-volume2-chapter25-html-browser-qa-v1",
        ...
        "scope": {
            ...
            "included": [
                ...
                "Volume II Chapter 25 complete, official pages 204-287",
            ],
            "excluded": ["Volume II Chapters 26-28 and appendices"],
            "official_source_page_accounting": "389 of 672 (Volume I 102 + Volume II pages 1-287)",
            "chapter_25_status": "complete",
            "html_routes_in_materialized_tree": 67,
        },
        "inputs": {
        ...
        "coverage": {
            ...
            "unique_current_routes_with_desktop_and_mobile_evidence": 67,
            "route_viewport_observations": 134,
        ...
        "checks": {
            ...
            "all_67_routes_exercised_at_desktop_and_mobile": True,
            "math_source_rendered_assistive_parity_every_route": True,
            "all_reader_facing_chapter25_target_math_replayed": True,
            "all_non_body_chapter25_math_explicitly_receipted": True,
        ...
        "next_gate": (
            ...
            "and aggregate receipts into CP0018; this receipt does not self-admit."
        ),
    }
