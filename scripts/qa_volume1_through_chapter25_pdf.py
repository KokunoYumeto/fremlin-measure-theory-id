#!/usr/bin/env python3
"""Raster, structural, and visual QA for the reader through complete Chapter 25.

The admitted 363-page through-S252 reader and its passing visual receipt are
immutable controlling evidence.  Every cumulative page is rendered at 96 dpi;
all predecessor raster hashes must replay exactly, while the 56 appended pages
...
build_module = load_module("o007_chapter25_build", Path(__file__).with_name("build_volume1_through_chapter25.py"))

PDF = ROOT / "output" / "pdf" / "fondasi-teori-ukuran-jilid-1-dan-jilid-2-hingga-akhir-bab-25-id.pdf"
BUILD_RECEIPT = ROOT / "qa" / "through-chapter25-complete-build.json"
PRIOR_VISUAL_RECEIPT = ROOT / "qa" / "through-s252-pdf-visual-qa.json"
PRIOR_PDF = ROOT / "output" / "pdf" / "fondasi-teori-ukuran-jilid-1-dan-jilid-2-hingga-bagian-252-id.pdf"
RECEIPT = ROOT / "qa" / "through-chapter25-pdf-visual-qa.json"
CONTACT_DIR = ROOT / "qa" / "rendered" / "through-chapter25-pdf-visual"
TMP_PARENT = ROOT / "tmp" / "pdfs"

EXPECTED_BUILD_RECEIPT_BYTES = 79_893
EXPECTED_BUILD_RECEIPT_SHA256 = "c2c67672d201796a24fa1f9663e0847533079f4f92c7fc616d717a90e6364365"
...
def contact_sheet(page_paths: list[Path], page_numbers: list[int], output: Path, sequence: int) -> dict[str, Any]:
    ...
    for index, (path, number) in enumerate(zip(page_paths, page_numbers, strict=True)):
        ...
        label = f"Cumulative {number} | Volume II through Chapter 25 physical {volume2_page}"
...
def verify_build_receipt() -> tuple[dict[str, Any], dict[str, Any], int]:
    ...
    require(build.get("schema") == "o007-fremlin-volume1-plus-volume2-through-chapter25-pdf-build-v1", "build schema differs")
    ...
    expected_accounting = {
        "volume1_pages": 102,
        "volume2_last_printed_page": 287,
        "selected_total_pages": 389,
    ...
    prefix = reproducibility.get("predecessor_363_page_prefix_preservation", {})
    require(
...
def audit() -> dict[str, Any]:
    ...
    require(
        prior.get("schema") == "o007-volume1-plus-volume2-through-s252-pdf-visual-qa-v1"
    ...
    with tempfile.TemporaryDirectory(prefix="o007-through-chapter25-pdf-", dir=TMP_PARENT) as temp_name:
        temp = Path(temp_name)
        prefix = temp / "page"
        ...
        for offset in range(0, len(inspection_pages), 9):
            ...
            output = CONTACT_DIR / f"through-chapter25-contact-{sequence:02d}.png"
    ...
    automated_checks = {
        ...
        "build_receipt_and_eight_unit_bindings_exact": True,
        "pdfinfo_and_pypdf_page_count_419": len(raster_rows) == EXPECTED_PAGES,
        "metadata_scope_model_license_and_389_of_672_exact": not metadata_mismatches,
        "all_pages_a4_zero_rotation": not geometry_mismatches,
        "all_363_predecessor_geometries_exact": not prior_geometry_mismatches,
        ...
        "all_363_predecessor_pixel_hashes_exact": not replay_mismatches,
        "contact_sheets_cover_all_56_appended_pages": [page for row in contact_rows for page in row["pages"]] == inspection_pages,
    ...
    report: dict[str, Any] = {
        "schema": "o007-volume1-plus-volume2-through-chapter25-pdf-visual-qa-v1",
        "status": "automated_pass_visual_inspection_pending",
        "checked_at": "2026-08-28",
        "production_model": PRODUCTION_MODEL,
        "scope": {
            "included": ["Volume I complete", "Volume II official pages 1-287", "Chapter 25 complete through Section 257"],
            "excluded": ["Volume II Chapters 26-28 and appendices"],
            "locale": "id-ID",
            "license": "Design Science License for Fremlin-derived material",
            "official_source_page_accounting": "389 of 672 (Volume I 102 + Volume II official pages 1-287)",
        ...
        "prior_boundary_replay": {
            ...
            "all_363_predecessor_pages_reused_pixel_exact": True,
        },
        "inspection_plan": {
        ...
        "next_gate": "Owner inspects every saved through-Chapter25 contact sheet, then finalizes this receipt for admission.",
...
def finalize(findings: str | None) -> dict[str, Any]:
    ...
    if findings is None:
        findings = (
            "All 56 appended surfaces were inspected across seven contact sheets; no clipping, overlap, "
